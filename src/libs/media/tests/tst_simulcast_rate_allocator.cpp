...
public:
    ...
    MOCK_METHOD(void, OnRatesUpdated, (size_t, const std::vector<uint32_t> &, int), (override));
    MOCK_METHOD(Vp8EncoderConfig, UpdateConfiguration, (size_t), (override));
    MOCK_METHOD(void, OnEncodeDone, (size_t, uint32_t, size_t, bool, int, CodecSpecificInfo *), (override));
};
} // namespace
...
public:
    ...
    template <size_t S>
    void ExpectEqual(uint32_t (&expected)[S], const VideoBitrateAllocation &actual)
    {
        // EXPECT_EQ(S, actual.size());
        uint32_t sum = 0;
        for (size_t i = 0; i < S; ++i)
        {
            ...
            EXPECT_EQ(expected[i] * 1000U, layer_bitrate) << "Mismatch at index " << i;
            sum += layer_bitrate;
        }
    ...
    void SetupCodec3SL3TL(const std::vector<bool> &active_streams)
    {
        ...
        OCTK_DCHECK_GE(active_streams.size(), num_simulcast_layers);
    ...
    VideoBitrateAllocation GetAllocation(uint32_t target_bitrate)
    {
        return allocator_->Allocate(
            VideoBitrateAllocationParameters(DataRate::KilobitsPerSec(target_bitrate), kDefaultFrameRate));
    }

    VideoBitrateAllocation GetAllocation(DataRate target_rate, DataRate stable_rate)
    {
        return allocator_->Allocate(VideoBitrateAllocationParameters(target_rate, stable_rate, kDefaultFrameRate));
    }

    DataRate MinRate(size_t layer_index) const
    {
        return DataRate::KilobitsPerSec(codec_.simulcastStream[layer_index].minBitrate);
    }

    DataRate TargetRate(size_t layer_index) const
    {
        return DataRate::KilobitsPerSec(codec_.simulcastStream[layer_index].targetBitrate);
    }

    DataRate MaxRate(size_t layer_index) const
    {
        return DataRate::KilobitsPerSec(codec_.simulcastStream[layer_index].maxBitrate);
    }
...
{
    codec_.active = true;
    for (uint32_t bitrate = codec_.minBitrate; bitrate <= codec_.maxBitrate; ++bitrate)
    {
...
TEST_F(SimulcastRateAllocatorTest, SignalsBwLimited)
{
    ...
    EXPECT_TRUE(GetAllocation(codec_.simulcastStream[0].minBitrate - 10).is_bw_limited());
    EXPECT_TRUE(GetAllocation(codec_.simulcastStream[0].targetBitrate).is_bw_limited());
    EXPECT_TRUE(
        GetAllocation(codec_.simulcastStream[0].targetBitrate + codec_.simulcastStream[1].minBitrate).is_bw_limited());
    EXPECT_FALSE(GetAllocation(codec_.simulcastStream[0].targetBitrate + codec_.simulcastStream[1].targetBitrate +
                               static_cast<uint32_t>(codec_.simulcastStream[2].minBitrate * kDefaultHysteresis + 0.5))
                     .is_bw_limited());
...
TEST_F(SimulcastRateAllocatorTest, SingleSimulcastWithinLimits)
{
    ...
    for (uint32_t bitrate = kMinBitrateKbps; bitrate <= kLegacyScreenshareMaxBitrateKbps; ++bitrate)
    {
        uint32_t expected[] = {bitrate};
        ExpectEqual(expected, GetAllocation(bitrate));
    }
...
TEST_F(SimulcastRateAllocatorTest, Regular3TLTemporalRateAllocation)
{
    ...
    // 40/20/40.
    EXPECT_EQ(static_cast<uint32_t>(0.4 * kMinBitrateKbps), alloc.GetBitrate(0, 0) / 1000);
    EXPECT_EQ(static_cast<uint32_t>(0.2 * kMinBitrateKbps), alloc.GetBitrate(0, 1) / 1000);
    EXPECT_EQ(static_cast<uint32_t>(0.4 * kMinBitrateKbps), alloc.GetBitrate(0, 2) / 1000);
}

TEST_F(SimulcastRateAllocatorTest, BaseHeavy3TLTemporalRateAllocation)
{
    ExplicitKeyValueConfig field_trials("WebRTC-UseBaseHeavyVP8TL3RateAllocation/Enabled/");
    ...
    const VideoBitrateAllocation alloc = GetAllocation(kMinBitrateKbps);
    // 60/20/20.
    EXPECT_EQ(static_cast<uint32_t>(0.6 * kMinBitrateKbps), alloc.GetBitrate(0, 0) / 1000);
    EXPECT_EQ(static_cast<uint32_t>(0.2 * kMinBitrateKbps), alloc.GetBitrate(0, 1) / 1000);
    EXPECT_EQ(static_cast<uint32_t>(0.2 * kMinBitrateKbps), alloc.GetBitrate(0, 2) / 1000);
}
...
TEST_F(SimulcastRateAllocatorTest, OneToThreeStreams)
{
    ...
    uint32_t kMinInitialRateTwoLayers = codec_.simulcastStream[0].targetBitrate +
...

    {
        ...
        uint32_t expected[] = {codec_.simulcastStream[0].targetBitrate,
                               kMinInitialRateTwoLayers - codec_.simulcastStream[0].targetBitrate,
                               0};
        ExpectEqual(expected, GetAllocation(bitrate));
    }

    {
        ...
        const uint32_t bitrate = codec_.simulcastStream[0].targetBitrate + codec_.simulcastStream[1].maxBitrate;
        uint32_t expected[] = {codec_.simulcastStream[0].targetBitrate, codec_.simulcastStream[1].maxBitrate, 0};
        ExpectEqual(expected, GetAllocation(bitrate));
    }

    uint32_t kMinInitialRateThreeLayers = codec_.simulcastStream[0].targetBitrate +
...

    {
        ...
        uint32_t expected[] = {codec_.simulcastStream[0].targetBitrate,
                               codec_.simulcastStream[1].targetBitrate,
                               static_cast<uint32_t>(codec_.simulcastStream[2].minBitrate * kDefaultHysteresis)};
        ExpectEqual(expected, GetAllocation(bitrate));
    }

    {
        // Third maxed out.
        const uint32_t bitrate = codec_.simulcastStream[0].targetBitrate + codec_.simulcastStream[1].targetBitrate +
...

    {
        ...
        // the lower streams.
        const uint32_t bitrate = codec_.simulcastStream[0].maxBitrate + codec_.simulcastStream[1].maxBitrate +
                                 codec_.simulcastStream[2].maxBitrate;
        uint32_t expected[] = {codec_.simulcastStream[0].targetBitrate,
                               codec_.simulcastStream[1].targetBitrate,
                               codec_.simulcastStream[2].maxBitrate};
...
TEST_F(SimulcastRateAllocatorTest, ThreeStreamsInactive)
{
    ...
    const uint32_t min_bitrate = codec_.simulcastStream[0].minBitrate + codec_.simulcastStream[1].minBitrate +
                                 codec_.simulcastStream[2].minBitrate;
    // Enough bitrate to allocate target to all streams.
    const uint32_t target_bitrate = codec_.simulcastStream[0].targetBitrate + codec_.simulcastStream[1].targetBitrate +
                                    codec_.simulcastStream[2].targetBitrate;
    // Enough bitrate to allocate max to all streams.
    const uint32_t max_bitrate = codec_.simulcastStream[0].maxBitrate + codec_.simulcastStream[1].maxBitrate +
                                 codec_.simulcastStream[2].maxBitrate;
...
TEST_F(SimulcastRateAllocatorTest, TwoStreamsLowInactive)
{
    ...
    const uint32_t kActiveStreamTargetBitrate = codec_.simulcastStream[1].targetBitrate;
...
TEST_F(SimulcastRateAllocatorTest, TwoStreamsHighInactive)
{
    ...
    const uint32_t kActiveStreamTargetBitrate = codec_.simulcastStream[0].targetBitrate;
...
TEST_F(SimulcastRateAllocatorTest, ThreeStreamsMiddleInactive)
{
...

    {
        ...
        ExpectEqual(expected, GetAllocation(codec_.simulcastStream[0].targetBitrate));
    }

    {
        // The lowest stream gets its max bitrate, but not enough for the high
        // stream.
        const uint32_t bitrate = codec_.simulcastStream[0].targetBitrate + codec_.simulcastStream[2].minBitrate - 1;
...

    {
        ...
        const uint32_t bitrate = codec_.simulcastStream[0].targetBitrate + codec_.simulcastStream[2].targetBitrate;
        uint32_t expected[] = {codec_.simulcastStream[0].targetBitrate, 0, codec_.simulcastStream[2].targetBitrate};
        ExpectEqual(expected, GetAllocation(bitrate));
    }

    {
        // Lowest stream gets its target bitrate, high stream gets its max bitrate.
        uint32_t bitrate = codec_.simulcastStream[0].targetBitrate + codec_.simulcastStream[2].maxBitrate;
        uint32_t expected[] = {codec_.simulcastStream[0].targetBitrate, 0, codec_.simulcastStream[2].maxBitrate};
        ExpectEqual(expected, GetAllocation(bitrate));
...
{
    ExplicitKeyValueConfig field_trials("WebRTC-StableTargetRate/"
                                        "enabled:true,"
    ...
    const DataRate volatile_rate = (TargetRate(0) + TargetRate(1) + MinRate(2)) * 1.1;

    {
        // On the first call to a new SimulcastRateAllocator instance, hysteresis
        // is disabled, but stable rate still caps layers.
        uint32_t expected[] = {TargetRate(0).kbps<uint32_t>(), MaxRate(1).kbps<uint32_t>()};
        ExpectEqual(expected, GetAllocation(volatile_rate, TargetRate(0) + MinRate(1)));
    }

    {
        // Let stable rate go to a bitrate below what is needed for two streams.
        uint32_t expected[] = {MaxRate(0).kbps<uint32_t>(), 0};
        ExpectEqual(expected, GetAllocation(volatile_rate, TargetRate(0) + MinRate(1) - DataRate::BitsPerSec(1)));
    }
...

    {
        ...
        uint32_t expected[] = {TargetRate(0).kbps<uint32_t>(), MaxRate(1).kbps<uint32_t>()};
        ExpectEqual(expected, GetAllocation(volatile_rate, (TargetRate(0) + MinRate(1)) * 1.1));
    }

    {
        // Enough to enable all thee layers.
        uint32_t expected[] = {TargetRate(0).kbps<uint32_t>(),
                               TargetRate(1).kbps<uint32_t>(),
...

    {
        ...
        uint32_t expected[] = {TargetRate(0).kbps<uint32_t>(),
                               TargetRate(1).kbps<uint32_t>(),
                               (volatile_rate - TargetRate(0) - TargetRate(1)).kbps<uint32_t>()};
        ExpectEqual(expected, GetAllocation(volatile_rate, TargetRate(0) + TargetRate(1) + MinRate(2)));
    }
}
...
public:
    void SetupConferenceScreenshare(bool use_simulcast, bool active = true)
    {
        codec_.mode = VideoCodecMode::kScreensharing;
        codec_.minBitrate = kMinBitrateKbps;
        codec_.maxBitrate = kLegacyScreenshareMaxBitrateKbps + kSimulcastScreenshareMaxBitrateKbps;
        if (use_simulcast)
        {
            ...
            codec_.simulcastStream[1].minBitrate = kSimulcastScreenshareMinBitrateKbps;
            codec_.simulcastStream[1].targetBitrate = kSimulcastScreenshareMaxBitrateKbps;
...
INSTANTIATE_TEST_SUITE_P(ScreenshareTest, ScreenshareRateAllocationTest, ::testing::Bool());
...
TEST_P(ScreenshareRateAllocationTest, ConferenceBitrateBelowTl0)
{
    ...
    VideoBitrateAllocation allocation = allocator_->Allocate(
        VideoBitrateAllocationParameters(kLegacyScreenshareTargetBitrateKbps * 1000, kFramerateFps));

    // All allocation should go in TL0.
    EXPECT_EQ(kLegacyScreenshareTargetBitrateKbps, allocation.get_sum_kbps());
    EXPECT_EQ(kLegacyScreenshareTargetBitrateKbps, allocation.GetBitrate(0, 0) / 1000);
...
TEST_P(ScreenshareRateAllocationTest, ConferenceBitrateAboveTl0)
{
    ...
    uint32_t target_bitrate_kbps = (kLegacyScreenshareTargetBitrateKbps + kLegacyScreenshareMaxBitrateKbps) / 2;
    VideoBitrateAllocation allocation = allocator_->Allocate(
        VideoBitrateAllocationParameters(target_bitrate_kbps * 1000, kFramerateFps));

    // Fill TL0, then put the rest in TL1.
    EXPECT_EQ(target_bitrate_kbps, allocation.get_sum_kbps());
    EXPECT_EQ(kLegacyScreenshareTargetBitrateKbps, allocation.GetBitrate(0, 0) / 1000);
    EXPECT_EQ(target_bitrate_kbps - kLegacyScreenshareTargetBitrateKbps, allocation.GetBitrate(0, 1) / 1000);
    EXPECT_EQ(allocation.is_bw_limited(), GetParam());
}
...
TEST_F(ScreenshareRateAllocationTest, ConferenceBitrateAboveTl1)
{
    ...
    VideoBitrateAllocation allocation = allocator_->Allocate(
        VideoBitrateAllocationParameters(kLegacyScreenshareMaxBitrateKbps * 2000, kFramerateFps));

    // Fill both TL0 and TL1, but no more.
    EXPECT_EQ(kLegacyScreenshareMaxBitrateKbps, allocation.get_sum_kbps());
    EXPECT_EQ(kLegacyScreenshareTargetBitrateKbps, allocation.GetBitrate(0, 0) / 1000);
    EXPECT_EQ(kLegacyScreenshareMaxBitrateKbps - kLegacyScreenshareTargetBitrateKbps,
...
TEST_F(ScreenshareRateAllocationTest, Hysteresis)
{
    ...
    const uint32_t default_enable_rate_bps = codec_.simulcastStream[0].targetBitrate +
                                             codec_.simulcastStream[1].minBitrate;
    const uint32_t enable_rate_with_hysteresis_bps = (default_enable_rate_bps * 135) / 100;

    {
        // On the first call to a new SimulcastRateAllocator instance, hysteresis
        // is disabled.
        const uint32_t bitrate = default_enable_rate_bps;
        uint32_t expected[] = {codec_.simulcastStream[0].targetBitrate, codec_.simulcastStream[1].minBitrate};
        ExpectEqual(expected, GetAllocation(bitrate));
    }
...

    {
        ...
        uint32_t expected[] = {codec_.simulcastStream[0].targetBitrate,
                               enable_rate_with_hysteresis_bps - codec_.simulcastStream[0].targetBitrate};
        ExpectEqual(expected, GetAllocation(bitrate));
    }
