...
OCTK_CXX14_CONSTEXPR TimeDelta kFrameTimeout = TimeDelta::Millis(100);
...
const VideoEncoder::ResolutionBitrateLimits kEncoderBitrateLimits540p(960 * 540, 100 * 1000, 100 * 1000, 2000 * 1000);
const VideoEncoder::ResolutionBitrateLimits kEncoderBitrateLimits720p(1280 * 720, 200 * 1000, 200 * 1000, 4000 * 1000);

uint8_t kOptimalSps[] = {0,
                         0,
                         0,
                         1,
                         H264::NaluType::kSps,
...
const uint8_t kCodedFrameVp8Qp25[] = {0x10, 0x02, 0x00, 0x9d, 0x01, 0x2a, 0x10, 0x00, 0x10, 0x00,
...
void PassAFrame(TaskQueue *encoder_queue,
                FrameCadenceAdapterInterface::Callback *video_stream_encoder_callback,
                int64_t ntp_time_ms)
{
    encoder_queue->PostTask(
        [video_stream_encoder_callback, ntp_time_ms]
        { video_stream_encoder_callback->OnFrame(Timestamp::Millis(ntp_time_ms), false, CreateSimpleNV12Frame()); });
}
...
public:
    FakeNV12NativeBuffer(Event *event, int width, int height)
        : nv12_buffer_(NV12Buffer::Create(width, height))
        , event_(event)
    {
    }
...
{
public:
    CpuOveruseDetectorProxy(const RtcContext &env, CpuOveruseMetricsObserver *metrics_observer)
        : OveruseFrameDetector(env, metrics_observer)
        , last_target_framerate_fps_(-1)
        , framerate_updated_event_(true /* manual_reset */, false /* initially_signaled */)
    {
    }
...
private:
    Mutex lock_;
    int last_target_framerate_fps_ OCTK_ATTRIBUTE_GUARDED_BY(lock_);
    Event framerate_updated_event_;
};

class FakeVideoSourceRestrictionsListener : public VideoSourceRestrictionsListener
{
public:
    FakeVideoSourceRestrictionsListener()
        : was_restrictions_updated_(false)
...
{
    return Field("max_framerate_fps", &VideoSinkWants::max_framerate_fps, fps_matcher);
}

auto WantsMaxPixels(Matcher<int> max_pixel_matcher)
{
    return Field("max_pixel_count", &VideoSinkWants::max_pixel_count, AllOf(max_pixel_matcher, Gt(0)));
}
...
auto FpsInRangeForPixelsInBalanced(int last_frame_pixels)
{
    ...
    return Field("max_framerate_fps", &VideoSinkWants::max_framerate_fps, fps_range_matcher);
}

auto FpsEqResolutionEqTo(const VideoSinkWants &other_wants)
{
    return AllOf(WantsFps(Eq(other_wants.max_framerate_fps)), WantsMaxPixels(Eq(other_wants.max_pixel_count)));
}
...
{
    return AllOf(WantsFps(Lt(other_wants.max_framerate_fps)), WantsMaxPixels(Eq(other_wants.max_pixel_count)));
}

auto FpsGtResolutionEq(const VideoSinkWants &other_wants)
{
    return AllOf(WantsFps(Gt(other_wants.max_framerate_fps)), WantsMaxPixels(Eq(other_wants.max_pixel_count)));
}

auto FpsEqResolutionLt(const VideoSinkWants &other_wants)
{
    return AllOf(WantsFps(Eq(other_wants.max_framerate_fps)), WantsMaxPixels(Lt(other_wants.max_pixel_count)));
}

auto FpsEqResolutionGt(const VideoSinkWants &other_wants)
{
    return AllOf(WantsFps(Eq(other_wants.max_framerate_fps)), WantsMaxPixels(Gt(other_wants.max_pixel_count)));
}

class VideoStreamEncoderUnderTest : public VideoStreamEncoder
{
public:
    VideoStreamEncoderUnderTest(const RtcContext &env,
                                TimeController *time_controller,
                                std::unique_ptr<FrameCadenceAdapterInterface> cadence_adapter,
                                std::unique_ptr<webrtc::TaskQueue, webrtc::TaskQueueDeleter> encoder_queue,
    ...
    void SetSourceAndWaitForRestrictionsUpdated(rtc::VideoSourceInterface<VideoFrame> *source,
    ...
    void SetSourceAndWaitForFramerateUpdated(rtc::VideoSourceInterface<VideoFrame> *source,
                                             const DegradationPreference &degradation_preference)
    {
        overuse_detector_proxy_->framerate_updated_event()->Reset();
        SetSource(source, degradation_preference);
        overuse_detector_proxy_->framerate_updated_event()->Wait(TimeDelta::Seconds(5));
    }
    ...
    {
        OnBitrateUpdated(target_bitrate,
    ...
    void WaitUntilTaskQueueIsIdle() { time_controller_->AdvanceTime(TimeDelta::Zero()); }
...
// by 4.
class CroppingVideoStreamFactory : public VideoEncoderConfig::VideoStreamFactoryInterface
{
public:
    CroppingVideoStreamFactory() { }

private:
    std::vector<VideoStream> CreateEncoderStreams(const FieldTrialsView & /*field_trials*/,
                                                  int frame_width,
                                                  int frame_height,
...
public:
    ...
    void IncomingCapturedFrame(const VideoFrame &video_frame) override
    {
        ...
        if (adaption_enabled())
        {
            RTC_DLOG(LS_INFO) << "IncomingCapturedFrame: AdaptFrameResolution()"
                              << "w=" << video_frame.width() << "h=" << video_frame.height();
            if (adapter_.AdaptFrameResolution(video_frame.width(),
                                              video_frame.height(),
    ...
    void OnOutputFormatRequest(int width, int height)
    {
        ...
        adapter_.OnOutputFormatRequest(target_aspect_ratio, max_pixel_count, max_fps);
    }

    void AddOrUpdateSink(rtc::VideoSinkInterface<VideoFrame> *sink, const VideoSinkWants &wants) override
    {
        MutexLock lock(&mutex_);
        VideoSinkWants prev_wants = sink_wants_locked();
        bool did_adapt = prev_wants.max_pixel_count != wants.max_pixel_count ||
    ...
    bool adaptation_enabled_ OCTK_ATTRIBUTE_GUARDED_BY(mutex_);
    VideoSinkWants last_wants_ OCTK_ATTRIBUTE_GUARDED_BY(mutex_);
...
public:
    ...
    void SetDroppedFrameCallback(std::function<void(DropReason)> callback) { on_frame_dropped_ = std::move(callback); }
...
private:
    ...
    mutable Mutex lock_;
    std::optional<VideoSendStream::Stats> mock_stats_ OCTK_ATTRIBUTE_GUARDED_BY(lock_);
...
public:
    ...
    GlobalSimulatedTimeController *GetTimeController() { return &time_controller_; }
...
private:
    ...
    public:
        ...
        void OnBitrateAllocationUpdated(const VideoBitrateAllocation &allocation) override { }
        void OnVideoLayersAllocationUpdated(VideoLayersAllocation allocation) override { }
        Result OnEncodedImage(const EncodedImage &encoded_image, const CodecSpecificInfo *codec_specific_info) override
        {
...
public:
    ...
    MOCK_METHOD(void, SetZeroHertzModeEnabled, (std::optional<ZeroHertzModeParams>), (override));
    MOCK_METHOD(void, OnFrame, (const VideoFrame &), (override));
    MOCK_METHOD(std::optional<uint32_t>, GetInputFrameRateFps, (), (override));
    MOCK_METHOD(void, UpdateLayerQualityConvergence, (size_t spatial_index, bool converged), (override));
    MOCK_METHOD(void, UpdateLayerStatus, (size_t spatial_index, bool enabled), (override));
    MOCK_METHOD(void, UpdateVideoSourceRestrictions, (std::optional<double>), (override));
    MOCK_METHOD(void, ProcessKeyFrameRequest, (), (override));
};

class MockEncoderSelector : public VideoEncoderFactory::EncoderSelectorInterface
{
public:
    MOCK_METHOD(void, OnCurrentEncoder, (const SdpVideoFormat &format), (override));
    MOCK_METHOD(std::optional<SdpVideoFormat>, OnAvailableBitrate, (const DataRate &rate), (override));
    MOCK_METHOD(std::optional<SdpVideoFormat>, OnResolutionChange, (const RenderResolution &resolution), (override));
...
{
public:
    MOCK_METHOD(void, AddOrUpdateSink, (rtc::VideoSinkInterface<VideoFrame> *, const VideoSinkWants &), (override));
    MOCK_METHOD(void, RemoveSink, (rtc::VideoSinkInterface<VideoFrame> *), (override));
...
public:
    ...
    VideoStreamEncoderTest()
        ...
        , stats_proxy_(new MockableSendStatisticsProxy(time_controller_.GetClock(),
                                                       video_send_config_,
                                                       VideoEncoderConfig::ContentType::kRealtimeVideo,
                                                       field_trials_))
    ...
    void SetUp() override
    {
        ...
        video_send_config_.encoder_settings.bitrate_allocator_factory = &bitrate_allocator_factory_;
                     ...
                     int num_cores = 1)
    {
        ...
        video_stream_encoder_->SetSource(&video_source_, webrtc::DegradationPreference::MAINTAIN_FRAMERATE);
        video_stream_encoder_->SetStartBitrate(kTargetBitrate.bps());
        video_stream_encoder_->ConfigureEncoder(std::move(video_encoder_config), kMaxPayloadLength, nullptr);
                      ...
                      int num_cores = 1)
    {
        ...
        VideoEncoderConfig video_encoder_config;
        test::FillEncoderConfiguration(PayloadStringToCodecType(payload_name), num_streams, &video_encoder_config);
        ...
        ConfigureEncoder(std::move(video_encoder_config), allocation_callback_type, num_cores);
    }

    VideoFrame CreateFrame(int64_t ntp_time_ms, Event *destruction_event) const
    {
        return VideoFrame::Builder()
            .set_video_frame_buffer(rtc::make_ref_counted<TestBuffer>(destruction_event, codec_width_, codec_height_))
    ...
    VideoFrame CreateFrameWithUpdatedPixel(int64_t ntp_time_ms, Event *destruction_event, int offset_x) const
    {
        return VideoFrame::Builder()
            .set_video_frame_buffer(rtc::make_ref_counted<TestBuffer>(destruction_event, codec_width_, codec_height_))
            .set_ntp_time_ms(ntp_time_ms)
    ...
    VideoFrame CreateFakeNativeFrame(int64_t ntp_time_ms, Event *destruction_event, int width, int height) const
    {
        return VideoFrame::Builder()
            .set_video_frame_buffer(rtc::make_ref_counted<FakeNativeBuffer>(destruction_event, width, height))
            .set_ntp_time_ms(ntp_time_ms)
            .set_timestamp_ms(ntp_time_ms)
            .set_rotation(kVideoRotation_0)
            .build();
    ...
    VideoFrame CreateFakeNV12NativeFrame(int64_t ntp_time_ms, Event *destruction_event, int width, int height) const
    {
        return VideoFrame::Builder()
            .set_video_frame_buffer(rtc::make_ref_counted<FakeNV12NativeBuffer>(destruction_event, width, height))
            .set_ntp_time_ms(ntp_time_ms)
            .set_timestamp_ms(ntp_time_ms)
            .set_rotation(kVideoRotation_0)
            .build();
    ...
    VideoFrame CreateFakeNativeFrame(int64_t ntp_time_ms, Event *destruction_event) const
    {
        return CreateFakeNativeFrame(ntp_time_ms, destruction_event, codec_width_, codec_height_);
    }

    void VerifyAllocatedBitrate(const VideoBitrateAllocation &expected_bitrate)
    {
        video_stream_encoder_
            ->OnBitrateUpdatedAndWaitForManagedResources(kTargetBitrate, kTargetBitrate, kTargetBitrate, 0, 0, 0);
    ...
    {
    public:
        explicit TestEncoder(const RtcContext &env)
            : FakeEncoder(env)
        {
        }
        ...
        VideoEncoder::EncoderInfo GetEncoderInfo() const override
        {
            ...
            if (initialized_ == EncoderState::kInitialized)
            {
                if (quality_scaling_)
                {
                    info.scaling_settings = VideoEncoder::ScalingSettings(kQpLow, kQpHigh, kMinPixelsPerFrame);
                }
            ...
            info.apply_alignment_to_all_simulcast_layers = apply_alignment_to_all_simulcast_layers_;
        ...
        void SetRequestedResolutionAlignment(uint32_t requested_resolution_alignment)
        {
            MutexLock lock(&local_mutex_);
        ...
        void SetResolutionBitrateLimits(std::vector<ResolutionBitrateLimits> thresholds)
        {
            MutexLock lock(&local_mutex_);
        ...
        {
            const std::vector<VideoFrameType> frame_type = {keyframe ? VideoFrameType::kVideoFrameKey
        ...
        void InjectEncodedImage(const EncodedImage &image, const CodecSpecificInfo *codec_specific_info)
        {
            MutexLock lock(&local_mutex_);
        ...
        std::optional<VideoEncoder::RateControlParameters> GetAndResetLastRateControlSettings()
        {
    ...
    private:
        ...
        int32_t InitEncode(const VideoCodec *config, const Settings &settings) override
        {
            ...
            if (config->codecType == kVideoCodecVP8)
            {
                ...
                frame_buffer_controller_ = factory.Create(*config, settings, &fec_controller_override_);
            }
                ...
                for (size_t ti = 0; ti < kMaxTemporalStreams; ++ti)
                {
                    if (parameters.bitrate.HasBitrate(si, ti))
                    {
                        adjusted_rate_allocation.SetBitrate(
                            si,
                            ti,
                            static_cast<uint32_t>(parameters.bitrate.GetBitrate(si, ti) * rate_factor_));
                    }
                }
            }
        ...
        bool quality_scaling_ OCTK_ATTRIBUTE_GUARDED_BY(local_mutex_) = true;
        ...
        double rate_factor_ OCTK_ATTRIBUTE_GUARDED_BY(local_mutex_) = 1.0;
        uint32_t last_framerate_ OCTK_ATTRIBUTE_GUARDED_BY(local_mutex_) = 0;
        std::optional<VideoEncoder::RateControlParameters> last_rate_control_settings_;
        VideoFrame::UpdateRect last_update_rect_ OCTK_ATTRIBUTE_GUARDED_BY(local_mutex_) = {0, 0, 0, 0};
        std::vector<VideoFrameType> last_frame_types_;
        bool expect_null_frame_ = false;
        EncodedImageCallback *encoded_image_callback_ OCTK_ATTRIBUTE_GUARDED_BY(local_mutex_) = nullptr;
    ...
    public:
        ...
        void CheckLastFrameSizeMatches(uint32_t expected_width, uint32_t expected_height)
        {
            uint32_t width = 0;
            uint32_t height = 0;
            {
        ...
        void ExpectDroppedFrame() { EXPECT_FALSE(WaitForFrame(TimeDelta::Millis(100))); }
        ...
        std::optional<absl::variant<FrameInstrumentationSyncData, FrameInstrumentationData>>
    ...
    private:
        Result OnEncodedImage(const EncodedImage &encoded_image, const CodecSpecificInfo *codec_specific_info) override
        {
            ...
            {
                last_frame_instrumentation_data_ = codec_specific_info->frame_instrumentation_data;
            }
        ...
        void OnVideoLayersAllocationUpdated(VideoLayersAllocation allocation) override
        {
            ...
            for (const auto &layer : allocation.active_spatial_layers)
            {
                ...
                for (const auto target_bitrate : layer.target_bitrate_per_temporal_layer)
                {
        ...
        VideoBitrateAllocation last_bitrate_allocation_ OCTK_ATTRIBUTE_GUARDED_BY(&mutex_);
        int number_of_bitrate_allocations_ OCTK_ATTRIBUTE_GUARDED_BY(&mutex_) = 0;
        VideoLayersAllocation last_layers_allocation_ OCTK_ATTRIBUTE_GUARDED_BY(&mutex_);
        int number_of_layers_allocations_ OCTK_ATTRIBUTE_GUARDED_BY(&mutex_) = 0;
        std::optional<absl::variant<FrameInstrumentationSyncData, FrameInstrumentationData>>
            last_frame_instrumentation_data_ OCTK_ATTRIBUTE_GUARDED_BY(&mutex_);
    };

    class VideoBitrateAllocatorProxyFactory : public VideoBitrateAllocatorFactory
    {
    public:
        ...
        std::unique_ptr<VideoBitrateAllocator> Create(const RtcContext &env, const VideoCodec &codec) override
        {
            MutexLock lock(&mutex_);
            codec_config_ = codec;
    ...
    private:
        ...
        mutable Mutex mutex_;
        VideoCodec codec_config_ OCTK_ATTRIBUTE_GUARDED_BY(mutex_);
    };

    Clock *clock() { return time_controller_.GetClock(); }
    void AdvanceTime(TimeDelta duration) { time_controller_.AdvanceTime(duration); }
...
{
    video_send_config_.encoder_settings.enable_frame_instrumentation_generator = true;
    ConfigureEncoder(video_encoder_config_.Copy());
    video_stream_encoder_
        ->OnBitrateUpdatedAndWaitForManagedResources(kTargetBitrate, kTargetBitrate, kTargetBitrate, 0, 0, 0);

    // We need a QP for the encoded frame.
    fake_encoder_.SetEncodedImageData(EncodedImageBuffer::Create(kCodedFrameVp8Qp25, sizeof(kCodedFrameVp8Qp25)));
    video_source_.IncomingCapturedFrame(CreateFrame(1, codec_width_, codec_height_));
    WaitForEncodedFrame(1);
...
TEST_F(VideoStreamEncoderTest, FrameInstrumentationGeneratorDoesNotStashDroppedFrames)
{
    ...
    video_send_config_.encoder_settings.enable_frame_instrumentation_generator = true;
    ConfigureEncoder(video_encoder_config_.Copy());
    video_stream_encoder_->OnBitrateUpdatedAndWaitForManagedResources(kLowRate, kLowRate, kLowRate, 0, 0, 0);
...
TEST_F(VideoStreamEncoderTest, FrameInstrumentationGeneratorHandlesQueuedFrames)
{
    video_send_config_.encoder_settings.enable_frame_instrumentation_generator = true;
    ConfigureEncoder(video_encoder_config_.Copy());
    ...
    video_stream_encoder_
        ->OnBitrateUpdatedAndWaitForManagedResources(DataRate::Zero(), DataRate::Zero(), DataRate::Zero(), 0, 0, 0);
    video_stream_encoder_->WaitUntilTaskQueueIsIdle();
...
TEST_F(VideoStreamEncoderTest, DoesNotPopulateFrameInstrumentationDataWhenSetNotTo)
{
    video_send_config_.encoder_settings.enable_frame_instrumentation_generator = false;
    ConfigureEncoder(video_encoder_config_.Copy());
    video_stream_encoder_
        ->OnBitrateUpdatedAndWaitForManagedResources(kTargetBitrate, kTargetBitrate, kTargetBitrate, 0, 0, 0);
...
TEST_F(VideoStreamEncoderTest, FrameInstrumentationGeneratorNotResetOnConfigurationUnlessEncoderIsToo)
{
    ...
    video_stream_encoder_->ConfigureEncoder(video_encoder_config_.Copy(), kMaxPayloadLength, nullptr);
...
TEST_F(VideoStreamEncoderTest, DropsFramesBeforeFirstOnBitrateUpdated)
{
    ...
    video_source_.IncomingCapturedFrame(CreateFrame(kFrame1TimestampMs, &frame_destroyed_event));
...
TEST_F(VideoStreamEncoderTest, DropsPendingFramesOnSlowEncode)
{
    ...
    video_stream_encoder_->SetSource(&source, DegradationPreference::MAINTAIN_FRAMERATE);
    video_stream_encoder_
        ->OnBitrateUpdatedAndWaitForManagedResources(kTargetBitrate, kTargetBitrate, kTargetBitrate, 0, 0, 0);

    int dropped_count = 0;
    stats_proxy_->SetDroppedFrameCallback([&dropped_count](VideoStreamEncoderObserver::DropReason)
                                          { ++dropped_count; });
...
TEST_F(VideoStreamEncoderTest, NativeFrameWithoutI420SupportGetsDelivered)
{
    ...
    Event frame_destroyed_event;
    video_source_.IncomingCapturedFrame(CreateFakeNativeFrame(1, &frame_destroyed_event));
    WaitForEncodedFrame(1);
    EXPECT_EQ(VideoFrameBuffer::Type::kNative, fake_encoder_.GetLastInputPixelFormat());
...
TEST_F(VideoStreamEncoderTest, NativeFrameWithoutI420SupportGetsCroppedIfNecessary)
{
    // Use the cropping factory.
    video_encoder_config_.video_stream_factory = rtc::make_ref_counted<CroppingVideoStreamFactory>();
    video_stream_encoder_->ConfigureEncoder(std::move(video_encoder_config_), kMaxPayloadLength);
    video_stream_encoder_->WaitUntilTaskQueueIsIdle();
    ...
    video_source_.IncomingCapturedFrame(
        CreateFakeNativeFrame(2, &frame_destroyed_event, codec_width_ + 1, codec_height_ + 1));
    WaitForEncodedFrame(2);
    EXPECT_EQ(VideoFrameBuffer::Type::kNative, fake_encoder_.GetLastInputPixelFormat());
    EXPECT_EQ(fake_encoder_.config().width, fake_encoder_.GetLastInputWidth());
    EXPECT_EQ(fake_encoder_.config().height, fake_encoder_.GetLastInputHeight());
...
TEST_F(VideoStreamEncoderTest, NonI420FramesShouldNotBeConvertedToI420)
{
    ...
    video_source_.IncomingCapturedFrame(CreateNV12Frame(1, codec_width_, codec_height_));
    WaitForEncodedFrame(1);
    EXPECT_EQ(VideoFrameBuffer::Type::kNV12, fake_encoder_.GetLastInputPixelFormat());
    video_stream_encoder_->Stop();
}
...
TEST_F(VideoStreamEncoderTest, NativeFrameGetsDelivered_NoFrameTypePreference)
{
    ...
    Event frame_destroyed_event;
    video_source_.IncomingCapturedFrame(
        CreateFakeNV12NativeFrame(1, &frame_destroyed_event, codec_width_, codec_height_));
    WaitForEncodedFrame(1);
    EXPECT_EQ(VideoFrameBuffer::Type::kNative, fake_encoder_.GetLastInputPixelFormat());
    video_stream_encoder_->Stop();
}

TEST_F(VideoStreamEncoderTest, NativeFrameGetsDelivered_PixelFormatPreferenceMatches)
{
    video_stream_encoder_
        ->OnBitrateUpdatedAndWaitForManagedResources(kTargetBitrate, kTargetBitrate, kTargetBitrate, 0, 0, 0);
...
TEST_F(VideoStreamEncoderTest, DropsFramesWhenCongestionWindowPushbackSet)
{
    ...
    video_stream_encoder_
        ->OnBitrateUpdatedAndWaitForManagedResources(kTargetBitrate, kTargetBitrate, kTargetBitrate, 0, 0, 0.5);
...
TEST_F(VideoStreamEncoderTest, ConfigureEncoderTriggersOnEncoderConfigurationChanged)
{
    ...
    video_stream_encoder_->ConfigureEncoder(std::move(video_encoder_config), kMaxPayloadLength);
...
TEST_F(VideoStreamEncoderTest, EncoderInstanceDestroyedBeforeAnotherInstanceCreated)
{
    video_stream_encoder_
        ->OnBitrateUpdatedAndWaitForManagedResources(kTargetBitrate, kTargetBitrate, kTargetBitrate, 0, 0, 0);
    ...
    video_stream_encoder_->ConfigureEncoder(std::move(video_encoder_config), kMaxPayloadLength / 2);
...
TEST_F(VideoStreamEncoderTest, BitrateLimitsChangeReconfigureRateAllocator)
{
    ...
    video_stream_encoder_->ConfigureEncoder(video_encoder_config.Copy(), kMaxPayloadLength);
    ...
    // received.
    EXPECT_EQ(1, sink_.number_of_reconfigurations());
    EXPECT_EQ(kTargetBitrate.bps(), bitrate_allocator_factory_.codec_config().maxBitrate * 1000);
    EXPECT_EQ(kStartBitrate.bps(), bitrate_allocator_factory_.codec_config().startBitrate * 1000);
    ...
    EXPECT_EQ(kTargetBitrate.bps() * 2, bitrate_allocator_factory_.codec_config().maxBitrate * 1000);
    EXPECT_EQ(kStartBitrate.bps() * 2, bitrate_allocator_factory_.codec_config().startBitrate * 1000);
...
TEST_F(VideoStreamEncoderTest, IntersectionOfEncoderAndAppBitrateLimitsUsedWhenBothProvided)
{
    ...
    video_encoder_config.simulcast_layers[0].min_bitrate_bps = (kMinEncBitrateKbps + 1) * 1000;
    video_stream_encoder_->ConfigureEncoder(video_encoder_config.Copy(), kMaxPayloadLength);
    ...
    video_source_.IncomingCapturedFrame(CreateFrame(1, nullptr));
    WaitForEncodedFrame(1);
    EXPECT_EQ(kMaxEncBitrateKbps, bitrate_allocator_factory_.codec_config().maxBitrate);
    EXPECT_EQ(kMinEncBitrateKbps + 1, bitrate_allocator_factory_.codec_config().minBitrate);
    ...
    video_source_.IncomingCapturedFrame(CreateFrame(2, nullptr));
    WaitForEncodedFrame(2);
    EXPECT_EQ(kMaxEncBitrateKbps - 1, bitrate_allocator_factory_.codec_config().maxBitrate);
    EXPECT_EQ(kMinEncBitrateKbps, bitrate_allocator_factory_.codec_config().minBitrate);

    video_stream_encoder_->Stop();
}

TEST_F(VideoStreamEncoderTest, EncoderAndAppLimitsDontIntersectEncoderLimitsIgnored)
{
    video_stream_encoder_
        ->OnBitrateUpdatedAndWaitForManagedResources(kTargetBitrate, kTargetBitrate, kTargetBitrate, 0, 0, 0);
    ...
    video_encoder_config.simulcast_layers[0].min_bitrate_bps = kMinAppBitrateKbps * 1000;
    video_stream_encoder_->ConfigureEncoder(video_encoder_config.Copy(), kMaxPayloadLength);

    video_source_.IncomingCapturedFrame(CreateFrame(1, nullptr));
    WaitForEncodedFrame(1);
    EXPECT_EQ(kMaxAppBitrateKbps, bitrate_allocator_factory_.codec_config().maxBitrate);
    EXPECT_EQ(kMinAppBitrateKbps, bitrate_allocator_factory_.codec_config().minBitrate);

    video_stream_encoder_->Stop();
}

TEST_F(VideoStreamEncoderTest, EncoderRecommendedMaxAndMinBitratesUsedForGivenResolution)
{
    video_stream_encoder_
        ->OnBitrateUpdatedAndWaitForManagedResources(kTargetBitrate, kTargetBitrate, kTargetBitrate, 0, 0, 0);

    const VideoEncoder::ResolutionBitrateLimits encoder_bitrate_limits_270p(480 * 270,
                                                                            34 * 1000,
                                                                            12 * 1000,
                                                                            1234 * 1000);
    const VideoEncoder::ResolutionBitrateLimits encoder_bitrate_limits_360p(640 * 360,
                                                                            43 * 1000,
                                                                            21 * 1000,
                                                                            2345 * 1000);
    fake_encoder_.SetResolutionBitrateLimits({encoder_bitrate_limits_270p, encoder_bitrate_limits_360p});
    ...
    video_source_.IncomingCapturedFrame(CreateFrame(3, (640 + 480) / 2, (360 + 270) / 2));
    WaitForEncodedFrame(3);
    EXPECT_EQ(static_cast<uint32_t>(encoder_bitrate_limits_360p.min_bitrate_bps),
              bitrate_allocator_factory_.codec_config().minBitrate * 1000);
    EXPECT_EQ(static_cast<uint32_t>(encoder_bitrate_limits_360p.max_bitrate_bps),
...
TEST_F(VideoStreamEncoderTest, EncoderRecommendedMaxBitrateCapsTargetBitrate)
{
    ...
    const uint32_t kDefaultTargetBitrateFor720pKbps =
        bitrate_allocator_factory_.codec_config().simulcastStream[0].targetBitrate;

    // Set the max recommended encoder bitrate to something lower than the default
    // target bitrate.
    const VideoEncoder::ResolutionBitrateLimits encoder_bitrate_limits(1280 * 720,
                                                                       10 * 1000,
                                                                       10 * 1000,
    ...
    EXPECT_EQ(bitrate_allocator_factory_.codec_config().simulcastStream[0].targetBitrate * 1000,
              static_cast<uint32_t>(encoder_bitrate_limits.max_bitrate_bps));

    video_stream_encoder_->Stop();
}

TEST_F(VideoStreamEncoderTest, EncoderMaxAndMinBitratesUsedForTwoStreamsHighestActive)
{
    const VideoEncoder::ResolutionBitrateLimits kEncoderLimits270p(480 * 270, 34 * 1000, 12 * 1000, 1234 * 1000);
    const VideoEncoder::ResolutionBitrateLimits kEncoderLimits360p(640 * 360, 43 * 1000, 21 * 1000, 2345 * 1000);
    fake_encoder_.SetResolutionBitrateLimits({kEncoderLimits270p, kEncoderLimits360p});
...
TEST_F(VideoStreamEncoderTest, DefaultEncoderMaxAndMinBitratesUsedForTwoStreamsHighestActive)
{
    // Two streams, highest stream active.
    VideoEncoderConfig config;
    const int kNumStreams = 2;
    ...
    const std::optional<VideoEncoder::ResolutionBitrateLimits> kDefaultLimits270p =
        EncoderInfoSettings::GetDefaultSinglecastBitrateLimitsForResolution(kVideoCodecVP8, 480 * 270);
    video_source_.IncomingCapturedFrame(CreateFrame(1, 480, 270));
    video_stream_encoder_->WaitUntilTaskQueueIsIdle();
    EXPECT_EQ(fake_encoder_.config().numberOfSimulcastStreams, kNumStreams);
    ...
    const std::optional<VideoEncoder::ResolutionBitrateLimits> kDefaultLimits360p =
        EncoderInfoSettings::GetDefaultSinglecastBitrateLimitsForResolution(kVideoCodecVP8, 640 * 360);
    video_source_.IncomingCapturedFrame(CreateFrame(2, 640, 360));
    video_stream_encoder_->WaitUntilTaskQueueIsIdle();
    ...
    const std::optional<VideoEncoder::ResolutionBitrateLimits> kDefaultLimits540p =
        EncoderInfoSettings::GetDefaultSinglecastBitrateLimitsForResolution(kVideoCodecVP8, 960 * 540);
    video_source_.IncomingCapturedFrame(CreateFrame(4, 960, 540));
    video_stream_encoder_->WaitUntilTaskQueueIsIdle();
...
TEST_F(VideoStreamEncoderTest, EncoderMaxAndMinBitratesUsedForThreeStreamsMiddleActive)
{
    const VideoEncoder::ResolutionBitrateLimits kEncoderLimits270p(480 * 270, 34 * 1000, 12 * 1000, 1234 * 1000);
    const VideoEncoder::ResolutionBitrateLimits kEncoderLimits360p(640 * 360, 43 * 1000, 21 * 1000, 2345 * 1000);
    const VideoEncoder::ResolutionBitrateLimits kEncoderLimits720p(1280 * 720, 54 * 1000, 31 * 1000, 3456 * 1000);
    fake_encoder_.SetResolutionBitrateLimits({kEncoderLimits270p, kEncoderLimits360p, kEncoderLimits720p});
...
TEST_F(VideoStreamEncoderTest, EncoderMaxAndMinBitratesNotUsedForThreeStreamsLowestActive)
{
    const VideoEncoder::ResolutionBitrateLimits kEncoderLimits270p(480 * 270, 34 * 1000, 12 * 1000, 1234 * 1000);
    const VideoEncoder::ResolutionBitrateLimits kEncoderLimits360p(640 * 360, 43 * 1000, 21 * 1000, 2345 * 1000);
    const VideoEncoder::ResolutionBitrateLimits kEncoderLimits720p(1280 * 720, 54 * 1000, 31 * 1000, 3456 * 1000);
...
TEST_F(VideoStreamEncoderTest, EncoderMaxBitrateCappedByConfigForTwoStreamsHighestActive)
{
    const VideoEncoder::ResolutionBitrateLimits kEncoderLimits270p(480 * 270, 34 * 1000, 12 * 1000, 1234 * 1000);
    const VideoEncoder::ResolutionBitrateLimits kEncoderLimits360p(640 * 360, 43 * 1000, 21 * 1000, 2345 * 1000);
    fake_encoder_.SetResolutionBitrateLimits({kEncoderLimits270p, kEncoderLimits360p});
    ...
    EXPECT_EQ(static_cast<uint32_t>(kEncoderLimits360p.min_bitrate_bps),
              fake_encoder_.config().simulcastStream[1].minBitrate * 1000);
    EXPECT_EQ(static_cast<uint32_t>(kMaxBitrateBps), fake_encoder_.config().simulcastStream[1].maxBitrate * 1000);
...
TEST_F(VideoStreamEncoderTest, SwitchSourceDeregisterEncoderAsSink)
{
    ...
    video_stream_encoder_->SetSource(&new_video_source, webrtc::DegradationPreference::MAINTAIN_FRAMERATE);
...
TEST_F(VideoStreamEncoderTest, FrameRateLimitPropagatedToSinkWantsBeforeFirstFrame)
{
...
TEST_F(VideoStreamEncoderTest, FrameRateLimitIgnoredOnInactiveStreams)
{
    ...
    video_stream_encoder_->ConfigureEncoder(std::move(encoder_config), kMaxPayloadLength);
    EXPECT_EQ(video_source_.sink_wants().max_framerate_fps, 15);
    video_stream_encoder_->Stop();
}
...
TEST_F(VideoStreamEncoderTest, FrameRateLimitCanBeReset)
{
    ...
    video_encoder_config_.simulcast_layers[0].max_framerate = VideoStream().max_framerate;
    video_stream_encoder_->ConfigureEncoder(video_encoder_config_.Copy(), kMaxPayloadLength);
...
TEST_F(VideoStreamEncoderTest, RequestInSinkWantsBeforeFirstFrame)
{
    ...
    EXPECT_EQ(video_source_.sink_wants().requested_resolution, VideoSinkWants::FrameSize(320, 160));

    video_encoder_config_.simulcast_layers[0].scale_resolution_down_to->height = 320;
    video_encoder_config_.simulcast_layers[0].scale_resolution_down_to->width = 640;
    video_stream_encoder_->ConfigureEncoder(video_encoder_config_.Copy(), kMaxPayloadLength);

    EXPECT_EQ(video_source_.sink_wants().requested_resolution, VideoSinkWants::FrameSize(640, 320));
...
TEST_F(VideoStreamEncoderTest, RequestInWrongAspectRatioWithAdapter)
{
    ...
    video_stream_encoder_->SetSource(&source, webrtc::DegradationPreference::MAINTAIN_FRAMERATE);

    ASSERT_THAT(video_encoder_config_.simulcast_layers, SizeIs(1));
    video_encoder_config_.simulcast_layers[0].scale_resolution_down_to = {.width = 30, .height = 30};
    video_stream_encoder_->ConfigureEncoder(video_encoder_config_.Copy(), kMaxPayloadLength);
...
TEST_F(VideoStreamEncoderTest, EncodingActiveFlagPropagatedToSinkWantsBeforeFirstFrame)
{
    ASSERT_THAT(video_encoder_config_.simulcast_layers, SizeIs(1));
...
TEST_F(VideoStreamEncoderTest, CorrectlyAdjustsAv1Bitrate)
{
    ...
    video_stream_encoder_
        ->OnBitrateUpdatedAndWaitForManagedResources(kTargetBitrate, 2 * kTargetBitrate, 2 * kTargetBitrate, 0, 0, 0);

    video_source_.IncomingCapturedFrame(CreateFrame(CurrentTimeMs(), codec_width_, codec_height_));
    WaitForEncodedFrame(CurrentTimeMs());

    // Before enough data has been gathered, some default pushback is applied.
    VideoEncoder::RateControlParameters rate_settings = *fake_encoder_.GetAndResetLastRateControlSettings();
    // Allow 5% diff from target bitrate.
    const double allowed_error_bps = rate_settings.target_bitrate.get_sum_bps() * 0.05;
    EXPECT_NEAR(rate_settings.bitrate.get_sum_bps(),
                rate_settings.target_bitrate.get_sum_bps() / EncoderBitrateAdjuster::kDefaultUtilizationFactor,
                allowed_error_bps);
    ...
    video_stream_encoder_->OnBitrateUpdatedAndWaitForManagedResources(kTargetBitrate - DataRate::BitsPerSec(500),
                                                                      2 * kTargetBitrate,
                                                                      2 * kTargetBitrate,
                                                                      0,
                                                                      0,
    ...
    EXPECT_NEAR(rate_settings.bitrate.get_sum_bps(), rate_settings.target_bitrate.get_sum_bps(), allowed_error_bps);

    video_stream_encoder_->Stop();
}

class ResolutionAlignmentTest : public VideoStreamEncoderTest,
                                public ::testing::WithParamInterface<::testing::tuple<int, std::vector<double>>>
{
public:
...
INSTANTIATE_TEST_SUITE_P(AlignmentAndScaleFactors,
                         ResolutionAlignmentTest,
                         ::testing::Combine(::testing::Values(1, 2, 3, 4, 5, 6, 16, 22), // requested_alignment_
...
TEST_P(ResolutionAlignmentTest, SinkWantsAlignmentApplied)
{
    ...
    video_stream_encoder_
        ->OnBitrateUpdatedAndWaitForManagedResources(kAvailableBitrate, kAvailableBitrate, kAvailableBitrate, 0, 0, 0);
    ...
    for (int i = 0; i < codec.numberOfSimulcastStreams; ++i)
    {
        ...
        EXPECT_EQ(codec.width * codec.simulcastStream[i].height, codec.height * codec.simulcastStream[i].width);
    }
...
TEST_F(VideoStreamEncoderTest, TestCpuDowngrades_BalancedMode)
{
    ...
    video_stream_encoder_
        ->OnBitrateUpdatedAndWaitForManagedResources(kTargetBitrate, kTargetBitrate, kTargetBitrate, 0, 0, 0);
    video_stream_encoder_->SetSource(&video_source_, webrtc::DegradationPreference::BALANCED);
    ...
    do
    {
        ...
        EXPECT_THAT(
            ...
            FpsInRangeForPixelsInBalanced(*video_source_.last_sent_width() * *video_source_.last_sent_height()));
    } while (video_source_.sink_wants().max_pixel_count < last_wants.max_pixel_count ||
             video_source_.sink_wants().max_framerate_fps < last_wants.max_framerate_fps);
    ...
    EXPECT_EQ(loop_count - 1, stats_proxy_->GetStats().number_of_cpu_adapt_changes);
    EXPECT_EQ(kMinPixelsPerFrame, *video_source_.last_sent_width() * *video_source_.last_sent_height());
    EXPECT_EQ(kMinBalancedFramerateFps, video_source_.sink_wants().max_framerate_fps);
    ...
    for (int i = 0; i < loop_count - 1; ++i)
    {
        ...
        EXPECT_THAT(
            video_source_.sink_wants(),
            FpsInRangeForPixelsInBalanced(*video_source_.last_sent_width() * *video_source_.last_sent_height()));
        EXPECT_TRUE(video_source_.sink_wants().max_pixel_count > last_wants.max_pixel_count ||
                    video_source_.sink_wants().max_framerate_fps > last_wants.max_framerate_fps);
    }
    ...
    EXPECT_FALSE(stats_proxy_->GetStats().cpu_limited_resolution);
    EXPECT_FALSE(stats_proxy_->GetStats().cpu_limited_framerate);
    EXPECT_EQ((loop_count - 1) * 2, stats_proxy_->GetStats().number_of_cpu_adapt_changes);

    video_stream_encoder_->Stop();
}

TEST_F(VideoStreamEncoderTest, SinkWantsNotChangedByResourceLimitedBeforeDegradationPreferenceChange)
{
    video_stream_encoder_->OnBitrateUpdated(kTargetBitrate, kTargetBitrate, kTargetBitrate, 0, 0, 0);
    EXPECT_THAT(video_source_.sink_wants(), UnlimitedSinkWants());
    ...
    video_source_.set_adaptation_enabled(true);
    video_stream_encoder_->SetSource(&video_source_, webrtc::DegradationPreference::MAINTAIN_RESOLUTION);
    EXPECT_THAT(video_source_.sink_wants(), UnlimitedSinkWants());
    video_source_.IncomingCapturedFrame(CreateFrame(ntp_time, kFrameWidth, kFrameHeight));
    ...
    video_stream_encoder_->SetSourceAndWaitForRestrictionsUpdated(&video_source_,
    ...
    EXPECT_FALSE(video_source_.sink_wants().target_pixel_count);
    EXPECT_LT(video_source_.sink_wants().max_pixel_count, kFrameWidth * kFrameHeight);
    EXPECT_THAT(video_source_.sink_wants(), FpsUnlimited());
...
TEST_F(VideoStreamEncoderTest, SinkWantsStoredByDegradationPreference)
{
    ...
    video_source_.IncomingCapturedFrame(CreateFrame(frame_timestamp, kFrameWidth, kFrameHeight));
    ...
    test::FrameForwarder new_video_source;
    video_stream_encoder_->SetSourceAndWaitForRestrictionsUpdated(&new_video_source,
                                                                  webrtc::DegradationPreference::MAINTAIN_RESOLUTION);
    // Give the encoder queue time to process the change in degradation preference
    // by waiting for an encoded frame.
    new_video_source.IncomingCapturedFrame(CreateFrame(frame_timestamp, kFrameWidth, kFrameWidth));
    ...
    video_stream_encoder_->TriggerCpuOveruse();
    new_video_source.IncomingCapturedFrame(CreateFrame(frame_timestamp, kFrameWidth, kFrameHeight));
    WaitForEncodedFrame(frame_timestamp);
    frame_timestamp += kFrameIntervalMs;
    ...
    sink_.WaitForEncodedFrame(frame_timestamp);
    frame_timestamp += kFrameIntervalMs;
    EXPECT_LT(new_video_source.sink_wants().max_pixel_count, kFrameWidth * kFrameHeight);
    EXPECT_FALSE(new_video_source.sink_wants().target_pixel_count);
...
TEST_F(VideoStreamEncoderTest, SwitchingSourceKeepsCpuAdaptation)
{
    ...
    video_stream_encoder_->SetSource(&new_video_source, webrtc::DegradationPreference::DISABLED);
...
TEST_F(VideoStreamEncoderTest, SwitchingSourceKeepsQualityAdaptation)
{
    ...
    // Set new source with adaptation still enabled.
    test::FrameForwarder new_video_source;
    video_stream_encoder_->SetSource(&new_video_source, webrtc::DegradationPreference::BALANCED);
    ...
    video_stream_encoder_->SetSource(&new_video_source, webrtc::DegradationPreference::MAINTAIN_RESOLUTION);
...
TEST_F(VideoStreamEncoderTest, QualityAdaptationStatsAreResetWhenScalerIsDisabled)
{
    video_stream_encoder_
        ->OnBitrateUpdatedAndWaitForManagedResources(kTargetBitrate, kTargetBitrate, kTargetBitrate, 0, 0, 0);
    ...
    int64_t timestamp_ms = kFrameIntervalMs;
    video_source_.set_adaptation_enabled(true);
    video_source_.IncomingCapturedFrame(CreateFrame(timestamp_ms, kWidth, kHeight));
    WaitForEncodedFrame(timestamp_ms);
    EXPECT_FALSE(stats_proxy_->GetStats().cpu_limited_resolution);
...
TEST_F(VideoStreamEncoderTest, StatsTracksCpuAdaptationStatsWhenSwitchingSource_Balanced)
{
    video_stream_encoder_
        ->OnBitrateUpdatedAndWaitForManagedResources(kTargetBitrate, kTargetBitrate, kTargetBitrate, 0, 0, 0);
    ...
    // Enable BALANCED preference, no initial limitation.
    test::FrameForwarder source;
    video_stream_encoder_->SetSource(&source, webrtc::DegradationPreference::BALANCED);
    ...
    // from BALANCED.
    video_stream_encoder_->SetSourceAndWaitForRestrictionsUpdated(&source,
                                                                  webrtc::DegradationPreference::MAINTAIN_FRAMERATE);
    source.IncomingCapturedFrame(CreateFrame(sequence, kWidth, kHeight));
    WaitForEncodedFrame(sequence++);
    stats = stats_proxy_->GetStats();
    ...
    video_stream_encoder_->SetSourceAndWaitForRestrictionsUpdated(&source, webrtc::DegradationPreference::BALANCED);
    source.IncomingCapturedFrame(CreateFrame(sequence, kWidth, kHeight));
    WaitForEncodedFrame(sequence++);
    stats = stats_proxy_->GetStats();
    EXPECT_FALSE(stats.cpu_limited_resolution);
...
TEST_F(VideoStreamEncoderTest, StatsTracksCpuAdaptationStatsWhenSwitchingSource)
{
    video_stream_encoder_
        ->OnBitrateUpdatedAndWaitForManagedResources(kTargetBitrate, kTargetBitrate, kTargetBitrate, 0, 0, 0);
    ...
    new_video_source.IncomingCapturedFrame(CreateFrame(sequence, kWidth, kHeight));
    WaitForEncodedFrame(sequence++);
    stats = stats_proxy_->GetStats();
    EXPECT_TRUE(stats.cpu_limited_resolution);
    EXPECT_FALSE(stats.cpu_limited_framerate);
...
TEST_F(VideoStreamEncoderTest, ScalingUpAndDownDoesNothingWithMaintainResolution)
{
    const int kWidth = 1280;
    const int kHeight = 720;
    video_stream_encoder_
...
TEST_F(VideoStreamEncoderTest, SkipsSameAdaptDownRequest_MaintainFramerateMode)
{
    const int kWidth = 1280;
    const int kHeight = 720;
    video_stream_encoder_
    ...
    video_stream_encoder_->TriggerCpuOveruse();
    EXPECT_THAT(source.sink_wants(), FpsUnlimitedResolutionMatches(Lt(kWidth * kHeight)));
...
TEST_F(VideoStreamEncoderTest, FpsCountReturnsToZeroForFewerAdaptationsUpThanDown)
{
    ...
    EXPECT_THAT(source.sink_wants(), FpsMatchesResolutionMax(Lt(kDefaultFramerate)));
    EXPECT_FALSE(stats_proxy_->GetStats().bw_limited_resolution);
...
TEST_F(VideoStreamEncoderTest, FpsCountReturnsToZeroForFewerAdaptationsUpThanDownWithTwoResources)
{
    const int kWidth = 1280;
    const int kHeight = 720;
    const int64_t kFrameIntervalMs = 150;
    ...
    source.IncomingCapturedFrame(CreateFrame(timestamp_ms, kWidth, kHeight));
    sink_.WaitForEncodedFrame(timestamp_ms);
    EXPECT_THAT(source.sink_wants(), FpsUnlimitedResolutionLt(source.last_wants()));
    EXPECT_TRUE(stats_proxy_->GetStats().bw_limited_resolution);
    EXPECT_FALSE(stats_proxy_->GetStats().bw_limited_framerate);
    EXPECT_EQ(2, stats_proxy_->GetStats().number_of_quality_adapt_changes);
...
TEST_F(VideoStreamEncoderTest, NoChangeForInitialNormalUsage_MaintainFramerateMode)
{
    const int kWidth = 1280;
    const int kHeight = 720;
    video_stream_encoder_
...
TEST_F(VideoStreamEncoderTest, NoChangeForInitialNormalUsage_MaintainResolutionMode)
{
    const int kWidth = 1280;
    const int kHeight = 720;
    video_stream_encoder_
    ...
    test::FrameForwarder source;
    video_stream_encoder_->SetSource(&source, webrtc::DegradationPreference::MAINTAIN_RESOLUTION);
...
TEST_F(VideoStreamEncoderTest, NoChangeForInitialNormalUsage_DisabledMode)
{
    ...
    test::FrameForwarder source;
    video_stream_encoder_->SetSource(&source, webrtc::DegradationPreference::DISABLED);
...
TEST_F(VideoStreamEncoderTest, AdaptsResolutionForLowQuality_MaintainFramerateMode)
{
    const int kWidth = 1280;
    const int kHeight = 720;
    video_stream_encoder_
...
TEST_F(VideoStreamEncoderTest, AdaptsFramerateForLowQuality_MaintainResolutionMode)
{
    const int kWidth = 1280;
    const int kHeight = 720;
    const int kInputFps = 30;
    ...
    video_source_.IncomingCapturedFrame(CreateFrame(2, kWidth, kHeight));
    sink_.WaitForEncodedFrame(2);
    EXPECT_THAT(video_source_.sink_wants(), FpsUnlimitedResolutionMatches(Lt(kWidth * kHeight)));
    ...
    EXPECT_THAT(new_video_source.sink_wants(), FpsMatchesResolutionMax(Lt(kInputFps)));
...
TEST_F(VideoStreamEncoderTest, DoesNotScaleBelowSetResolutionLimit)
{
    ...
    {
        video_source_.IncomingCapturedFrame(CreateFrame(i * kFrameIntervalMs, kWidth, kHeight));
        ...
        EXPECT_TRUE(stats_proxy_->GetStats().bw_limited_resolution);
        EXPECT_EQ(downscales, stats_proxy_->GetStats().number_of_quality_adapt_changes);
        EXPECT_GT(downscales, 0);
    }
    video_stream_encoder_->Stop();
}

TEST_F(VideoStreamEncoderTest, AdaptsResolutionUpAndDownTwiceOnOveruse_MaintainFramerateMode)
{
    const int kWidth = 1280;
    const int kHeight = 720;
    video_stream_encoder_
...
TEST_F(VideoStreamEncoderTest, AdaptsResolutionUpAndDownTwiceForLowQuality_BalancedMode_NoFpsLimit)
{
    const int kWidth = 1280;
    const int kHeight = 720;
    video_stream_encoder_
...
{
    fake_encoder_.SetResolutionBitrateLimits({kEncoderBitrateLimits540p, kEncoderBitrateLimits720p});
    ...
    // of bitrate.
    video_stream_encoder_->TriggerQualityHigh();
    EXPECT_THAT(source.sink_wants(), FpsUnlimitedResolutionMatches(Lt(1280 * 720)));
...
protected:
    ...
    {
        video_stream_encoder_->OnBitrateUpdatedAndWaitForManagedResources(bitrate, bitrate, bitrate, 0, 0, 0);
    }
...
TEST_F(BalancedDegradationTest, AdaptDownTwiceIfMinFpsDiffLtThreshold)
{
    ...
    video_stream_encoder_->TriggerQualityLow();
    EXPECT_THAT(source_.sink_wants(), AllOf(WantsFps(Eq(24)), WantsMaxPixels(Le(230400))));
...
TEST_F(BalancedDegradationTest, InitialFrameDropAdaptsFpsAndResolutionInOneStep)
{
    test::ScopedKeyValueConfig field_trials(field_trials_,
                                            "WebRTC-Video-BalancedDegradationSettings/"
    ...
    InsertFrame();
    EXPECT_FALSE(WaitForFrame(TimeDelta::Seconds(1)));
    EXPECT_LT(source_.sink_wants().max_pixel_count, source_.last_wants().max_pixel_count);
    EXPECT_EQ(source_.sink_wants().max_framerate_fps, 24);
...
TEST_F(BalancedDegradationTest, NoAdaptUpInResolutionIfBwEstimateIsLessThanMinBitrate)
{
    test::ScopedKeyValueConfig field_trials(field_trials_,
                                            "WebRTC-Video-BalancedDegradationSettings/"
...
TEST_F(BalancedDegradationTest, NoAdaptUpInFpsAndResolutionIfBwEstimateIsLessThanMinBitrate)
{
    test::ScopedKeyValueConfig field_trials(field_trials_,
                                            "WebRTC-Video-BalancedDegradationSettings/"
...
TEST_F(VideoStreamEncoderTest, AdaptsResolutionOnOveruseAndLowQuality_MaintainFramerateMode)
{
    const int kWidth = 1280;
    const int kHeight = 720;
    video_stream_encoder_
    ...
    source.IncomingCapturedFrame(CreateFrame(timestamp_ms, kWidth, kHeight));
    WaitForEncodedFrame(timestamp_ms);
    EXPECT_THAT(source.sink_wants(), FpsUnlimitedResolutionGt(source.last_wants()));
    EXPECT_TRUE(stats_proxy_->GetStats().cpu_limited_resolution);
    EXPECT_TRUE(stats_proxy_->GetStats().bw_limited_resolution);
    EXPECT_EQ(3, stats_proxy_->GetStats().number_of_cpu_adapt_changes);
    EXPECT_EQ(2, stats_proxy_->GetStats().number_of_quality_adapt_changes);
...
TEST_F(VideoStreamEncoderTest, CpuLimitedHistogramIsReported)
{
    ...
    {
        video_source_.IncomingCapturedFrame(CreateFrame(ntp_timestamp_ms, kWidth, kHeight));
    ...
    EXPECT_METRIC_EQ(1, metrics::NumSamples("WebRTC.Video.CpuLimitedResolutionInPercent"));
    EXPECT_METRIC_EQ(1, metrics::NumEvents("WebRTC.Video.CpuLimitedResolutionInPercent", 50));
}

TEST_F(VideoStreamEncoderTest, CpuLimitedHistogramIsNotReportedForDisabledDegradation)
{
    video_stream_encoder_
        ->OnBitrateUpdatedAndWaitForManagedResources(kTargetBitrate, kTargetBitrate, kTargetBitrate, 0, 0, 0);
    const int kWidth = 640;
    ...
    video_stream_encoder_->SetSource(&video_source_, webrtc::DegradationPreference::DISABLED);
    ...
    EXPECT_EQ(0, metrics::NumSamples("WebRTC.Video.CpuLimitedResolutionInPercent"));
}
...
TEST_F(VideoStreamEncoderTest, ReportsVideoLayersAllocationForVP8Simulcast)
{
    ...
    VideoLayersAllocation last_layer_allocation = sink_.GetLastVideoLayersAllocation();
    // kLowTargetBitrate is only enough for one spatial layer.
    ASSERT_EQ(last_layer_allocation.active_spatial_layers.size(), 1u);

    VideoBitrateAllocation bitrate_allocation = fake_encoder_.GetAndResetLastRateControlSettings()->target_bitrate;
    // Check that encoder has been updated too, not just allocation observer.
    ...
    while (CurrentTimeMs() - start_time_ms < 10 * kProcessIntervalMs)
    {
        ...
        if (number_of_layers_allocation != sink_.number_of_layers_allocations())
        {
            ...
            VideoLayersAllocation new_allocation = sink_.GetLastVideoLayersAllocation();
            ...
            EXPECT_EQ(new_allocation.active_spatial_layers[0].target_bitrate_per_temporal_layer,
                      last_layer_allocation.active_spatial_layers[0].target_bitrate_per_temporal_layer);
...
TEST_F(VideoStreamEncoderTest, ReportsVideoLayersAllocationForVP8WithMiddleLayerDisabled)
{
    ...
    video_encoder_config.content_type = VideoEncoderConfig::ContentType::kRealtimeVideo;
    video_encoder_config.encoder_specific_settings =
        rtc::make_ref_counted<VideoEncoderConfig::Vp8EncoderSpecificSettings>(VideoEncoder::GetDefaultVp8Settings());
    for (auto &layer : video_encoder_config.simulcast_layers)
    {
    ...
    EXPECT_THAT(last_layer_allocation.active_spatial_layers[0].target_bitrate_per_temporal_layer, SizeIs(2));
...
TEST_F(VideoStreamEncoderTest, ReportsVideoLayersAllocationForVP8WithMiddleAndHighestLayerDisabled)
{
    fake_encoder_.SetTemporalLayersSupported(/*spatial_idx=*/0, true);
    fake_encoder_.SetTemporalLayersSupported(/*spatial_idx*/ 1, true);
    fake_encoder_.SetTemporalLayersSupported(/*spatial_idx*/ 2, true);
...
TEST_F(VideoStreamEncoderTest, ReportsVideoLayersAllocationForV9SvcWithTemporalLayerSupport)
{
    fake_encoder_.SetTemporalLayersSupported(/*spatial_idx=*/0, true);
    fake_encoder_.SetTemporalLayersSupported(/*spatial_idx*/ 1, true);
    VideoEncoderConfig video_encoder_config;
    ...
    EXPECT_THAT(last_layer_allocation.active_spatial_layers[1].target_bitrate_per_temporal_layer, SizeIs(2));
    EXPECT_EQ(last_layer_allocation.active_spatial_layers[1].width, 1280);
    ...
    // rate.
    EXPECT_EQ(last_layer_allocation.active_spatial_layers[1].target_bitrate_per_temporal_layer[1], kTargetBitrate);
    video_stream_encoder_->Stop();
}
...
TEST_F(VideoStreamEncoderTest, ReportsVideoLayersAllocationForV9SvcWithoutTemporalLayerSupport)
{
    ...
    ASSERT_THAT(last_layer_allocation.active_spatial_layers, SizeIs(2));
    EXPECT_THAT(last_layer_allocation.active_spatial_layers[0].target_bitrate_per_temporal_layer, SizeIs(1));
    EXPECT_THAT(last_layer_allocation.active_spatial_layers[1].target_bitrate_per_temporal_layer, SizeIs(1));
    // Since full SVC is used, expect the top layer to utilize the full target
    // rate.
    EXPECT_EQ(last_layer_allocation.active_spatial_layers[1].target_bitrate_per_temporal_layer[0], kTargetBitrate);
    video_stream_encoder_->Stop();
}

TEST_F(VideoStreamEncoderTest, ReportsVideoLayersAllocationForVP9KSvcWithTemporalLayerSupport)
{
    fake_encoder_.SetTemporalLayersSupported(/*spatial_idx=*/0, true);
    fake_encoder_.SetTemporalLayersSupported(/*spatial_idx*/ 1, true);
    VideoEncoderConfig video_encoder_config;
    ...
    EXPECT_LT(last_layer_allocation.active_spatial_layers[1].target_bitrate_per_temporal_layer[1], kTargetBitrate);
    video_stream_encoder_->Stop();
}

TEST_F(VideoStreamEncoderTest, ReportsVideoLayersAllocationForV9SvcWithLowestLayerDisabled)
{
    fake_encoder_.SetTemporalLayersSupported(/*spatial_idx=*/0, true);
    fake_encoder_.SetTemporalLayersSupported(/*spatial_idx*/ 1, true);
    fake_encoder_.SetTemporalLayersSupported(/*spatial_idx*/ 2, true);
...
TEST_F(VideoStreamEncoderTest, ReportsVideoLayersAllocationForV9SvcWithHighestLayerDisabled)
{
    fake_encoder_.SetTemporalLayersSupported(/*spatial_idx=*/0, true);
    fake_encoder_.SetTemporalLayersSupported(/*spatial_idx*/ 1, true);
    fake_encoder_.SetTemporalLayersSupported(/*spatial_idx*/ 2, true);
...
TEST_F(VideoStreamEncoderTest, ReportsVideoLayersAllocationForV9SvcWithAllButHighestLayerDisabled)
{
    fake_encoder_.SetTemporalLayersSupported(/*spatial_idx=*/0, true);
    fake_encoder_.SetTemporalLayersSupported(/*spatial_idx*/ 1, true);
    fake_encoder_.SetTemporalLayersSupported(/*spatial_idx*/ 2, true);
    ...
    EXPECT_EQ(last_layer_allocation.active_spatial_layers[0].spatial_id, 0);
    EXPECT_EQ(last_layer_allocation.active_spatial_layers[0].target_bitrate_per_temporal_layer[1], kTargetBitrate);
    video_stream_encoder_->Stop();
}
...
TEST_F(VideoStreamEncoderTest, ReportsVideoLayersAllocationForH264)
{
    ...
    ASSERT_THAT(last_layer_allocation.active_spatial_layers, SizeIs(1));
    ASSERT_THAT(last_layer_allocation.active_spatial_layers[0].target_bitrate_per_temporal_layer, SizeIs(1));
    EXPECT_EQ(last_layer_allocation.active_spatial_layers[0].target_bitrate_per_temporal_layer[0], kTargetBitrate);
    EXPECT_EQ(last_layer_allocation.active_spatial_layers[0].width, 1280);
...
TEST_F(VideoStreamEncoderTest, ReportsUpdatedVideoLayersAllocationWhenBweChanges)
{
    ResetEncoder("VP8",
                 /*num_streams*/ 2,
                 1,
    ...
    // kLowTargetBitrate is only enough for one spatial layer.
    ASSERT_EQ(last_layer_allocation.active_spatial_layers.size(), 1u);
    EXPECT_EQ(last_layer_allocation.active_spatial_layers[0].target_bitrate_per_temporal_layer[0], kLowTargetBitrate);

    video_stream_encoder_->OnBitrateUpdatedAndWaitForManagedResources(kSimulcastTargetBitrate,
    ...
    EXPECT_GT(last_layer_allocation.active_spatial_layers[1].target_bitrate_per_temporal_layer[0], DataRate::Zero());

    video_stream_encoder_->Stop();
}

TEST_F(VideoStreamEncoderTest, ReportsUpdatedVideoLayersAllocationWhenResolutionChanges)
{
    ResetEncoder("VP8",
                 /*num_streams*/ 2,
                 1,
    ...
    WaitForEncodedFrame(CurrentTimeMs());
    EXPECT_EQ(sink_.number_of_layers_allocations(), 1);
    ASSERT_THAT(sink_.GetLastVideoLayersAllocation().active_spatial_layers, SizeIs(2));
    EXPECT_EQ(sink_.GetLastVideoLayersAllocation().active_spatial_layers[1].width, codec_width_);
    EXPECT_EQ(sink_.GetLastVideoLayersAllocation().active_spatial_layers[1].height, codec_height_);

    video_source_.IncomingCapturedFrame(CreateFrame(CurrentTimeMs(), codec_width_ / 2, codec_height_ / 2));
    WaitForEncodedFrame(CurrentTimeMs());
    EXPECT_EQ(sink_.number_of_layers_allocations(), 2);
    ASSERT_THAT(sink_.GetLastVideoLayersAllocation().active_spatial_layers, SizeIs(2));
    EXPECT_EQ(sink_.GetLastVideoLayersAllocation().active_spatial_layers[1].width, codec_width_ / 2);
    EXPECT_EQ(sink_.GetLastVideoLayersAllocation().active_spatial_layers[1].height, codec_height_ / 2);
...
TEST_F(VideoStreamEncoderTest, TemporalLayersNotDisabledIfSupported)
{
    ...
    const int kTl0Bps = kTargetBitrate.bps() *
                        webrtc::SimulcastRateAllocator::GetTemporalRateAllocation(kNumTemporalLayers,
                                                                                  /*temporal_id*/ 0,
...
TEST_F(VideoStreamEncoderTest, VerifyBitrateAllocationForTwoStreams)
{
    ...
    const int kS0Tl0Bps = kS0Bps * webrtc::SimulcastRateAllocator::GetTemporalRateAllocation(
                                       /*num_layers*/ 2,
                                       /*temporal_id*/ 0,
                                       /*base_heavy_tl3_alloc*/ false);
    const int kS0Tl1Bps = kS0Bps * webrtc::SimulcastRateAllocator::GetTemporalRateAllocation(
                                       /*num_layers*/ 2,
                                       /*temporal_id*/ 1,
                                       /*base_heavy_tl3_alloc*/ false);
...
TEST_F(VideoStreamEncoderTest, OveruseDetectorUpdatedOnReconfigureAndAdaption)
{
    ...
    EXPECT_EQ(video_stream_encoder_->overuse_detector_proxy_->GetLastTargetFramerate(), kDefaultFramerate);
    ...
    EXPECT_EQ(video_stream_encoder_->overuse_detector_proxy_->GetLastTargetFramerate(), kFramerate);
    ...
    video_stream_encoder_->TriggerCpuOveruse();
    video_stream_encoder_->WaitUntilTaskQueueIsIdle();
    int adapted_framerate = video_stream_encoder_->overuse_detector_proxy_->GetLastTargetFramerate();
...
TEST_F(VideoStreamEncoderTest, OveruseDetectorUpdatedRespectsFramerateAfterUnderuse)
{
    const int kFrameWidth = 1280;
    const int kFrameHeight = 720;
    ...
    EXPECT_EQ(video_stream_encoder_->overuse_detector_proxy_->GetLastTargetFramerate(), kLowFramerate);
    ...
    EXPECT_EQ(video_stream_encoder_->overuse_detector_proxy_->GetLastTargetFramerate(), adapted_framerate);
    ...
    video_stream_encoder_->TriggerCpuUnderuse();
    video_stream_encoder_->WaitUntilTaskQueueIsIdle();
    EXPECT_EQ(video_stream_encoder_->overuse_detector_proxy_->GetLastTargetFramerate(), kHighFramerate);

    video_stream_encoder_->Stop();
}

TEST_F(VideoStreamEncoderTest, OveruseDetectorUpdatedOnDegradationPreferenceChange)
{
    const int kFrameWidth = 1280;
    const int kFrameHeight = 720;
    const int kFramerate = 24;
    ...
    video_stream_encoder_->SetSourceAndWaitForFramerateUpdated(&source,
                                                               webrtc::DegradationPreference::MAINTAIN_FRAMERATE);
    EXPECT_EQ(video_stream_encoder_->overuse_detector_proxy_->GetLastTargetFramerate(), kFramerate);
...
TEST_F(VideoStreamEncoderTest, DropsFramesAndScalesWhenBitrateIsTooLow)
{
    ...
    EXPECT_TRUE_WAIT(video_source_.sink_wants().max_pixel_count < kWidth * kHeight, 5000);

    int last_pixel_count = video_source_.sink_wants().max_pixel_count;

    // Next frame is scaled.
    video_source_.IncomingCapturedFrame(CreateFrame(2, kWidth * 3 / 4, kHeight * 3 / 4));

    // Expect to drop this frame, the wait should time out.
    ExpectDroppedFrame();

    EXPECT_TRUE_WAIT(video_source_.sink_wants().max_pixel_count < last_pixel_count, 5000);

    video_stream_encoder_->Stop();
}

TEST_F(VideoStreamEncoderTest, NumberOfDroppedFramesLimitedWhenBitrateIsTooLow)
{
    const int kTooLowBitrateForFrameSizeBps = 10000;
    video_stream_encoder_->OnBitrateUpdatedAndWaitForManagedResources(
        DataRate::BitsPerSec(kTooLowBitrateForFrameSizeBps),
...
TEST_F(VideoStreamEncoderTest, InitialFrameDropOffWithMaintainResolutionPreference)
{
    const int kWidth = 640;
    const int kHeight = 360;
    video_stream_encoder_
...
{
    VideoEncoderConfig video_encoder_config;
    test::FillEncoderConfiguration(PayloadStringToCodecType("VP8"), 1, &video_encoder_config);
    ...
    video_stream_encoder_->OnBitrateUpdatedAndWaitForManagedResources(DataRate::KilobitsPerSec(30),
...
TEST_F(VideoStreamEncoderTest, InitialFrameDropActivatesWhenLayersChange)
{
    ...
    // Update the config.
    VideoEncoderConfig video_encoder_config;
    test::FillEncoderConfiguration(PayloadStringToCodecType("VP8"), 3, &video_encoder_config);
    video_encoder_config.video_stream_factory = nullptr;
    for (auto &layer : video_encoder_config.simulcast_layers)
    {
        layer.num_temporal_layers = 1;
...
TEST_F(VideoStreamEncoderTest, InitialFrameDropActivatesWhenSVCLayersChange)
{
    ...
    // Update the config.
    VideoEncoderConfig video_encoder_config;
    test::FillEncoderConfiguration(PayloadStringToCodecType("VP9"), 1, &video_encoder_config);
    VideoCodecVP9 vp9_settings = VideoEncoder::GetDefaultVp9Settings();
    vp9_settings.numberOfSpatialLayers = 3;
...
TEST_F(VideoStreamEncoderTest, EncoderMaxAndMinBitratesUsedIfMiddleStreamActive)
{
    const VideoEncoder::ResolutionBitrateLimits kEncoderLimits270p(480 * 270, 34 * 1000, 12 * 1000, 1234 * 1000);
    const VideoEncoder::ResolutionBitrateLimits kEncoderLimits360p(640 * 360, 43 * 1000, 21 * 1000, 2345 * 1000);
    const VideoEncoder::ResolutionBitrateLimits kEncoderLimits720p(1280 * 720, 54 * 1000, 31 * 1000, 2500 * 1000);
    fake_encoder_.SetResolutionBitrateLimits({kEncoderLimits270p, kEncoderLimits360p, kEncoderLimits720p});
...
TEST_F(VideoStreamEncoderTest, DefaultMaxAndMinBitratesUsedIfMiddleStreamActive)
{
    VideoEncoderConfig video_encoder_config;
    test::FillEncoderConfiguration(PayloadStringToCodecType("VP9"), 1, &video_encoder_config);
    VideoCodecVP9 vp9_settings = VideoEncoder::GetDefaultVp9Settings();
    ...
    const std::optional<VideoEncoder::ResolutionBitrateLimits> kLimits360p =
        EncoderInfoSettings::GetDefaultSinglecastBitrateLimitsForResolution(kVideoCodecVP9, 640 * 360);
    video_source_.IncomingCapturedFrame(CreateFrame(1, 1280, 720));
    video_stream_encoder_->WaitUntilTaskQueueIsIdle();
    EXPECT_EQ(fake_encoder_.config().numberOfSimulcastStreams, 1);
    EXPECT_EQ(fake_encoder_.config().codecType, VideoCodecType::kVideoCodecVP9);
    ...
    const std::optional<VideoEncoder::ResolutionBitrateLimits> kLimits270p =
        EncoderInfoSettings::GetDefaultSinglecastBitrateLimitsForResolution(kVideoCodecVP9, 480 * 270);
    video_source_.IncomingCapturedFrame(CreateFrame(2, 960, 540));
    video_stream_encoder_->WaitUntilTaskQueueIsIdle();
    EXPECT_EQ(fake_encoder_.config().numberOfSimulcastStreams, 1);
    EXPECT_EQ(fake_encoder_.config().codecType, VideoCodecType::kVideoCodecVP9);
...
{
    webrtc::test::ScopedKeyValueConfig field_trials(field_trials_, "WebRTC-DefaultBitrateLimitsKillSwitch/Enabled/");
    VideoEncoderConfig video_encoder_config;
    test::FillEncoderConfiguration(PayloadStringToCodecType("VP9"), 1, &video_encoder_config);
    VideoCodecVP9 vp9_settings = VideoEncoder::GetDefaultVp9Settings();
    vp9_settings.numberOfSpatialLayers = 3;
...
TEST_F(VideoStreamEncoderTest, SinglecastBitrateLimitsNotUsedForOneStream)
{
    ...
    const std::optional<VideoEncoder::ResolutionBitrateLimits> kLimits720p =
        EncoderInfoSettings::GetDefaultSinglecastBitrateLimitsForResolution(kVideoCodecVP9, 1280 * 720);
    video_source_.IncomingCapturedFrame(CreateFrame(1, 1280, 720));
    video_stream_encoder_->WaitUntilTaskQueueIsIdle();
    EXPECT_EQ(fake_encoder_.config().numberOfSimulcastStreams, 1);
    EXPECT_EQ(fake_encoder_.config().codecType, VideoCodecType::kVideoCodecVP9);
...
TEST_F(VideoStreamEncoderTest, EncoderMaxAndMinBitratesNotUsedIfLowestStreamActive)
{
    const VideoEncoder::ResolutionBitrateLimits kEncoderLimits180p(320 * 180, 34 * 1000, 12 * 1000, 1234 * 1000);
    const VideoEncoder::ResolutionBitrateLimits kEncoderLimits720p(1280 * 720, 54 * 1000, 31 * 1000, 2500 * 1000);
    fake_encoder_.SetResolutionBitrateLimits({kEncoderLimits180p, kEncoderLimits720p});
...
class VideoStreamEncoderInitialFrameDropperTest : public VideoStreamEncoderTest,
                                                  public ::testing::WithParamInterface<FrameResolutionChangeMethod>
{
public:
    VideoStreamEncoderInitialFrameDropperTest()
        : VideoStreamEncoderTest()
        , frame_resolution_change_method_(GetParam())
    {
    }
    ...
    void SetUp() override
    {
        ...
        {
            case FrameResolutionChangeMethod::MODIFY_SOURCE: break;
...
INSTANTIATE_TEST_SUITE_P(VideoStreamEncoderInitialFrameDropperTest,
                         VideoStreamEncoderInitialFrameDropperTest,
...
TEST_P(VideoStreamEncoderInitialFrameDropperTest, InitialFrameDropActivatesWhenResolutionIncreases)
{
    SetEncoderFrameSize(kWidth / 2, kHeight / 2);

    video_stream_encoder_
        ->OnBitrateUpdatedAndWaitForManagedResources(kTargetBitrate, kTargetBitrate, kTargetBitrate, 0, 0, 0);
    video_source_.IncomingCapturedFrame(CreateFrame(1, captureWidth, captureHeight));
    // Frame should not be dropped.
    WaitForEncodedFrame(1);

    video_stream_encoder_
        ->OnBitrateUpdatedAndWaitForManagedResources(kLowTargetBitrate, kLowTargetBitrate, kLowTargetBitrate, 0, 0, 0);
    video_source_.IncomingCapturedFrame(CreateFrame(2, captureWidth, captureHeight));
    ...
    video_source_.IncomingCapturedFrame(CreateFrame(3, captureWidth, captureHeight));
...
TEST_P(VideoStreamEncoderInitialFrameDropperTest, InitialFrameDropIsNotReactivatedWhenAdaptingUp)
{
    const int kWidth = 640;
    const int kHeight = 360;
    ...
    video_stream_encoder_
        ->OnBitrateUpdatedAndWaitForManagedResources(kTargetBitrate, kTargetBitrate, kTargetBitrate, 0, 0, 0);
    source.IncomingCapturedFrame(CreateFrame(timestamp, captureWidth, captureHeight));
    WaitForEncodedFrame(timestamp);
    timestamp += 9000;
    ...
    EXPECT_TRUE_WAIT(source.sink_wants().max_pixel_count < kWidth * kHeight, 5000);
...
TEST_P(VideoStreamEncoderInitialFrameDropperTest, FrameDroppedWhenResolutionIncreasesAndLinkAllocationIsLow)
{
    ...
    fake_encoder_.SetResolutionBitrateLimits(
        {VideoEncoder::ResolutionBitrateLimits(320 * 180, 0, 30000, 400000),
         VideoEncoder::ResolutionBitrateLimits(640 * 360, kMinStartBps360p, 30000, 800000)});
    ...
    EXPECT_TRUE_WAIT(video_source_.sink_wants().max_pixel_count < 640 * 360, 5000);
    video_stream_encoder_->Stop();
}

TEST_P(VideoStreamEncoderInitialFrameDropperTest, FrameNotDroppedWhenResolutionIncreasesAndLinkAllocationIsHigh)
{
    const int kMinStartBps360p = 222000;
    fake_encoder_.SetResolutionBitrateLimits(
        {VideoEncoder::ResolutionBitrateLimits(320 * 180, 0, 30000, 400000),
...
TEST_F(VideoStreamEncoderTest, QualityScalerAdaptationsRemovedWhenQualityScalingDisabled)
{
    webrtc::test::ScopedKeyValueConfig field_trials(field_trials_, "WebRTC-Video-QualityScaling/Disabled/");
    AdaptingFrameForwarder source(&time_controller_);
    source.set_adaptation_enabled(true);
    video_stream_encoder_->SetSource(&source, DegradationPreference::MAINTAIN_FRAMERATE);
    video_stream_encoder_
    ...
    // time.
    EXPECT_TRUE_WAIT(stats_proxy_->GetStats().bw_limited_resolution, 2000 * 2.5 * 2);
    timestamp_ms += kFrameIntervalMs;
    source.IncomingCapturedFrame(CreateFrame(timestamp_ms, kWidth, kHeight));
    WaitForEncodedFrame(timestamp_ms);
    video_stream_encoder_->WaitUntilTaskQueueIsIdle();
...
TEST_F(VideoStreamEncoderTest, ResolutionNotAdaptedForTooSmallFrame_MaintainFramerateMode)
{
...
TEST_F(VideoStreamEncoderTest, ResolutionNotAdaptedForTooSmallFrame_BalancedMode)
{
    const int kTooSmallWidth = 10;
    const int kTooSmallHeight = 10;
...
TEST_F(VideoStreamEncoderTest, FailingInitEncodeDoesntCauseCrash)
{
    ...
    const int kFrameWidth = 1280;
    const int kFrameHeight = 720;
    video_source_.IncomingCapturedFrame(CreateFrame(1, kFrameWidth, kFrameHeight));
    ExpectDroppedFrame();
    video_stream_encoder_->Stop();
}

// TODO(sprang): Extend this with fps throttling and any "balanced" extensions.
TEST_F(VideoStreamEncoderTest, AdaptsResolutionOnOveruse_MaintainFramerateMode)
{
    video_stream_encoder_
        ->OnBitrateUpdatedAndWaitForManagedResources(kTargetBitrate, kTargetBitrate, kTargetBitrate, 0, 0, 0);
    ...
    video_source_.IncomingCapturedFrame(CreateFrame(1 * kFrameIntervalMs, kFrameWidth, kFrameHeight));
    WaitForEncodedFrame(kFrameWidth, kFrameHeight);

    // Trigger CPU overuse, downscale by 3/4.
    video_stream_encoder_->TriggerCpuOveruse();
    video_source_.IncomingCapturedFrame(CreateFrame(2 * kFrameIntervalMs, kFrameWidth, kFrameHeight));
    WaitForEncodedFrame((kFrameWidth * 3) / 4, (kFrameHeight * 3) / 4);

    // Trigger CPU normal use, return to original resolution.
    video_stream_encoder_->TriggerCpuUnderuse();
    video_source_.IncomingCapturedFrame(CreateFrame(3 * kFrameIntervalMs, kFrameWidth, kFrameHeight));
    WaitForEncodedFrame(kFrameWidth, kFrameHeight);

    video_stream_encoder_->Stop();
}

TEST_F(VideoStreamEncoderTest, AdaptsFramerateOnOveruse_MaintainResolutionMode)
{
    const int kFrameWidth = 1280;
    const int kFrameHeight = 720;
    ...
    video_source_.IncomingCapturedFrame(CreateFrame(timestamp_ms, kFrameWidth, kFrameHeight));
    WaitForEncodedFrame(timestamp_ms);
    ...
    EXPECT_NEAR(num_frames_dropped, max_framerate_ - (max_framerate_ * 2 / 3), kErrorMargin);
    ...
    EXPECT_NEAR(num_frames_dropped, max_framerate_ - (max_framerate_ * 4 / 9), kErrorMargin);
...
TEST_F(VideoStreamEncoderTest, DoesntAdaptDownPastMinFramerate)
{
    ...
    do
    {
        ...
        video_stream_encoder_->TriggerCpuOveruse();
    } while (video_source_.sink_wants().max_framerate_fps < last_wants.max_framerate_fps);

    EXPECT_THAT(video_source_.sink_wants(), FpsMatchesResolutionMax(Eq(kMinFramerateFps)));

    video_stream_encoder_->Stop();
}

TEST_F(VideoStreamEncoderTest, AdaptsResolutionAndFramerateForLowQuality_BalancedMode)
{
    const int kWidth = 1280;
    const int kHeight = 720;
    const int64_t kFrameIntervalMs = 150;
    ...
    source.IncomingCapturedFrame(CreateFrame(timestamp_ms, kWidth, kHeight));
    WaitForEncodedFrame(timestamp_ms);
    EXPECT_THAT(source.sink_wants(), FpsMaxResolutionMatches(Lt(kWidth * kHeight)));
    EXPECT_TRUE(stats_proxy_->GetStats().bw_limited_resolution);
    EXPECT_FALSE(stats_proxy_->GetStats().bw_limited_framerate);
    EXPECT_EQ(1, stats_proxy_->GetStats().number_of_quality_adapt_changes);
    ...
    EXPECT_EQ(source.sink_wants().max_pixel_count, source.last_wants().max_pixel_count);
    EXPECT_TRUE(stats_proxy_->GetStats().bw_limited_resolution);
    EXPECT_FALSE(stats_proxy_->GetStats().bw_limited_framerate);
...
TEST_F(VideoStreamEncoderTest, AdaptWithTwoReasonsAndDifferentOrder_Resolution)
{
    const int kWidth = 640;
    const int kHeight = 360;
...
TEST_F(VideoStreamEncoderTest, AcceptsFullHdAdaptedDownSimulcastFrames)
{
    ...
    video_encoder_config.simulcast_layers[0].max_framerate = kFramerate;
    video_encoder_config.max_bitrate_bps = kTargetBitrate.bps();
    video_encoder_config.video_stream_factory = rtc::make_ref_counted<CroppingVideoStreamFactory>();
    video_stream_encoder_->ConfigureEncoder(std::move(video_encoder_config), kMaxPayloadLength);
    video_stream_encoder_->WaitUntilTaskQueueIsIdle();

    video_source_.set_adaptation_enabled(true);

    video_source_.IncomingCapturedFrame(CreateFrame(ntp_time_ms, kFrameWidth, kFrameHeight));
    WaitForEncodedFrame(kFrameWidth, kFrameHeight);
...
TEST_F(VideoStreamEncoderTest, DoesNotUpdateBitrateAllocationWhenSuspended)
{
    ...
    video_stream_encoder_
        ->OnBitrateUpdatedAndWaitForManagedResources(DataRate::Zero(), DataRate::Zero(), DataRate::Zero(), 0, 1, 0);
...
TEST_F(VideoStreamEncoderTest, DefaultCpuAdaptationThresholdsForSoftwareEncoder)
{
    const int kFrameWidth = 1280;
    const int kFrameHeight = 720;
    ...
    video_source_.IncomingCapturedFrame(CreateFrame(1, kFrameWidth, kFrameHeight));
    WaitForEncodedFrame(1);
    EXPECT_EQ(video_stream_encoder_->overuse_detector_proxy_->GetOptions().low_encode_usage_threshold_percent,
              default_options.low_encode_usage_threshold_percent);
    EXPECT_EQ(video_stream_encoder_->overuse_detector_proxy_->GetOptions().high_encode_usage_threshold_percent,
              default_options.high_encode_usage_threshold_percent);
    video_stream_encoder_->Stop();
}

TEST_F(VideoStreamEncoderTest, HigherCpuAdaptationThresholdsForHardwareEncoder)
{
    const int kFrameWidth = 1280;
    const int kFrameHeight = 720;
...
TEST_F(VideoStreamEncoderTest, CpuAdaptationThresholdsUpdatesWhenHardwareAccelerationChange)
{
    const int kFrameWidth = 1280;
    const int kFrameHeight = 720;
    ...
    video_source_.IncomingCapturedFrame(CreateFrame(2, kFrameWidth, kFrameHeight));
    WaitForEncodedFrame(2);
...
TEST_F(VideoStreamEncoderTest, DropsFramesWhenEncoderOvershoots)
{
    ...
    for (int i = 0; i < kNumFramesInRun; ++i)
    {
        ...
        if (!TimedWaitForEncodedFrame(timestamp_ms, 2 * TimeDelta::Seconds(1) / kFps))
        {
            ++num_dropped;
        }
    ...
    video_stream_encoder_->OnBitrateUpdatedAndWaitForManagedResources(kTargetBitrate + DataRate::KilobitsPerSec(1),
...
TEST_F(VideoStreamEncoderTest, SetsFrameTypes)
{
    ...
    video_source_.IncomingCapturedFrame(CreateFrame(1, nullptr));
    WaitForEncodedFrame(1);
    EXPECT_THAT(fake_encoder_.LastFrameTypes(), ::testing::ElementsAre(VideoFrameType{VideoFrameType::kVideoFrameKey}));
...
TEST_F(VideoStreamEncoderTest, SetsFrameTypesSimulcast)
{
    ...
    EXPECT_THAT(fake_encoder_.LastFrameTypes(),
                ::testing::ElementsAreArray(
                    {VideoFrameType::kVideoFrameKey, VideoFrameType::kVideoFrameKey, VideoFrameType::kVideoFrameKey}));
    ...
    EXPECT_THAT(
        ...
        ::testing::ElementsAreArray(
            {VideoFrameType::kVideoFrameDelta, VideoFrameType::kVideoFrameDelta, VideoFrameType::kVideoFrameDelta}));
...
TEST_F(VideoStreamEncoderTest, DoesNotRewriteH264BitstreamWithOptimalSps)
{
    ...
    fake_encoder_.SetEncodedImageData(EncodedImageBuffer::Create(kOptimalSps, sizeof(kOptimalSps)));

    video_source_.IncomingCapturedFrame(CreateFrame(1, nullptr));
    WaitForEncodedFrame(1);

    EXPECT_THAT(sink_.GetLastEncodedImageData(), testing::ElementsAreArray(kOptimalSps));
...
TEST_F(VideoStreamEncoderTest, RewritesH264BitstreamWithNonOptimalSps)
{
    ...
    uint8_t original_sps[] = {0, 0, 0, 1, H264::NaluType::kSps, 0x00, 0x00, 0x03, 0x03, 0xF4, 0x05, 0x03, 0xC7, 0xC0};
    ResetEncoder("H264", 1, 1, 1, false);
    ...
    fake_encoder_.SetEncodedImageData(EncodedImageBuffer::Create(original_sps, sizeof(original_sps)));
...
TEST_F(VideoStreamEncoderTest, CopiesVideoFrameMetadataAfterDownscale)
{
    const int kFrameWidth = 1280;
    const int kFrameHeight = 720;
    const DataRate kTargetBitrate = DataRate::KilobitsPerSec(300); // Too low for HD resolution.
...
TEST_F(VideoStreamEncoderTest, BandwidthAllocationLowerBound)
{
    ...
    DataRate allocation_sum = DataRate::BitsPerSec(rate_settings->bitrate.get_sum_bps());
...
TEST_F(VideoStreamEncoderTest, EncoderRatesPropagatedOnReconfigure)
{
    ...
    EXPECT_EQ(static_cast<int>(prev_rate_settings->framerate_fps), kDefaultFramerate);
    ...
    EXPECT_EQ(static_cast<int>(fake_encoder_.GetLastFramerate()), kDefaultFramerate);
    ...
    EXPECT_EQ(2, sink_.number_of_reconfigurations());
    auto current_rate_settings = fake_encoder_.GetAndResetLastRateControlSettings();
...
struct MockEncoderSwitchRequestCallback : public EncoderSwitchRequestCallback
{
    ...
    MOCK_METHOD(void,
                ...
                (const webrtc::SdpVideoFormat &format, bool allow_default_fallback),
                (override));
};
...
TEST_F(VideoStreamEncoderTest, EncoderSelectorCurrentEncoderIsSignaled)
{
    ...
    auto encoder_factory = std::make_unique<test::VideoEncoderProxyFactory>(&fake_encoder_, &encoder_selector);
    ...
    video_source_.IncomingCapturedFrame(CreateFrame(kDontCare, kDontCare, kDontCare));
    AdvanceTime(TimeDelta::Zero());
    video_stream_encoder_->Stop();
...
TEST_F(VideoStreamEncoderTest, EncoderSelectorBitrateSwitch)
{
    ...
    ON_CALL(encoder_selector, OnAvailableBitrate).WillByDefault(Return(SdpVideoFormat("AV1")));
...
TEST_F(VideoStreamEncoderTest, EncoderSelectorResolutionSwitch)
{
    ...
    EXPECT_CALL(encoder_selector, OnResolutionChange(RenderResolution(640, 480))).WillOnce(Return(std::nullopt));
...
TEST_F(VideoStreamEncoderTest, EncoderSelectorBrokenEncoderSwitch)
{
    ...
    StrictMock<MockEncoderSwitchRequestCallback> switch_callback;
    video_send_config_.encoder_settings.encoder_switch_request_callback = &switch_callback;
    auto encoder_factory = std::make_unique<test::VideoEncoderProxyFactory>(&video_encoder, &encoder_selector);
    video_send_config_.encoder_settings.encoder_factory = encoder_factory.get();
    ...
    ON_CALL(video_encoder, Encode).WillByDefault(Return(WEBRTC_VIDEO_CODEC_ENCODER_FAILURE));
    ON_CALL(encoder_selector, OnEncoderBroken).WillByDefault(Return(SdpVideoFormat("AV2")));
...
TEST_F(VideoStreamEncoderTest, SwitchEncoderOnInitFailureWithEncoderSelector)
{
    ...
    video_stream_encoder_->OnBitrateUpdatedAndWaitForManagedResources(kTargetBitrate,
                                                                      kTargetBitrate,
                                                                      kTargetBitrate,
                                                                      /*fraction_lost=*/0,
                                                                      /*round_trip_time_ms=*/0,
    ...
    ON_CALL(video_encoder, InitEncode(_, _)).WillByDefault(Return(WEBRTC_VIDEO_CODEC_ENCODER_FAILURE));
    ON_CALL(encoder_selector, OnEncoderBroken).WillByDefault(Return(SdpVideoFormat("AV2")));
...
TEST_F(VideoStreamEncoderTest, SwitchEncoderOnInitFailureWithoutEncoderSelector)
{
    NiceMock<MockVideoEncoder> video_encoder;
    StrictMock<MockEncoderSwitchRequestCallback> switch_callback;
    video_send_config_.encoder_settings.encoder_switch_request_callback = &switch_callback;
...
TEST_F(VideoStreamEncoderTest, NullEncoderReturnSwitch)
{
    ...
    StrictMock<MockEncoderSwitchRequestCallback> switch_callback;
    video_send_config_.encoder_settings.encoder_switch_request_callback = &switch_callback;
    auto encoder_factory = std::make_unique<test::VideoEncoderNullableProxyFactory>(
        /*encoder=*/nullptr,
        &encoder_selector);
    video_send_config_.encoder_settings.encoder_factory = encoder_factory.get();
...
TEST_F(VideoStreamEncoderTest, AllocationPropagatedToEncoderWhenTargetRateChanged)
{
    const int kFrameWidth = 320;
    const int kFrameHeight = 180;
...
TEST_F(VideoStreamEncoderTest, AllocationNotPropagatedToEncoderWhenTargetRateUnchanged)
{
    const int kFrameWidth = 320;
    const int kFrameHeight = 180;
...
TEST_F(VideoStreamEncoderTest, EncoderResetAccordingToParameterChange)
{
    ...
    const int number_layers = sizeof(downscale_factors) / sizeof(downscale_factors[0]);
    VideoEncoderConfig config;
...
TEST_F(VideoStreamEncoderTest, EncoderResolutionsExposedInSinglecast)
{
    ...
    EXPECT_THAT(video_source_.sink_wants().resolutions,
                ::testing::ElementsAreArray({VideoSinkWants::FrameSize(kFrameWidth, kFrameHeight)}));
...
TEST_F(VideoStreamEncoderTest, EncoderResolutionsExposedInSimulcast)
{
    ...
    const int kFrameWidth = 1280;
    const int kFrameHeight = 720;
    const VideoSinkWants::FrameSize kLayer0Size(kFrameWidth / kDownscaleFactors[0],
                                                kFrameHeight / kDownscaleFactors[0]);
    const VideoSinkWants::FrameSize kLayer1Size(kFrameWidth / kDownscaleFactors[1],
                                                kFrameHeight / kDownscaleFactors[1]);
    const VideoSinkWants::FrameSize kLayer2Size(kFrameWidth / kDownscaleFactors[2],
    ...
    // Expect encoded resolutions to match the expected simulcast layers.
    video_stream_encoder_->WaitUntilTaskQueueIsIdle();
    EXPECT_THAT(video_source_.sink_wants().resolutions, ::testing::ElementsAreArray({kLayer0Size, kLayer1Size}));
    ...
    // Expect encoded resolutions to match the expected simulcast layers.
    video_stream_encoder_->WaitUntilTaskQueueIsIdle();
    EXPECT_THAT(video_source_.sink_wants().resolutions, ::testing::ElementsAreArray({kLayer0Size}));
...
TEST_F(VideoStreamEncoderTest, QpPresent_QpKept)
{
    ...
    encoded_image.SetEncodedData(EncodedImageBuffer::Create(kCodedFrameVp8Qp25, sizeof(kCodedFrameVp8Qp25)));
...
{
    webrtc::test::ScopedKeyValueConfig field_trials(field_trials_, "WebRTC-QpParsingKillSwitch/Enabled/");
...
TEST_F(VideoStreamEncoderTest, QualityScalingNotAllowed_QualityScalingDisabled)
{
    VideoEncoderConfig video_encoder_config = video_encoder_config_.Copy();
    video_encoder_config.simulcast_layers[0].max_framerate = -1; // No max frame rate.
...
TEST_F(VideoStreamEncoderTest, QualityScalingNotAllowedAndQPIsTrusted_BandwidthScalerDisable)
{
    VideoEncoderConfig video_encoder_config = video_encoder_config_.Copy();
...
TEST_F(VideoStreamEncoderTest, QualityScalingNotAllowedAndQPIsNotTrusted_BandwidthScalerDisable)
{
    VideoEncoderConfig video_encoder_config = video_encoder_config_.Copy();
...
TEST_F(VideoStreamEncoderTest, EncoderProvideLimitsWhenQPIsNotTrusted)
{
    ...
    video_encoder_config.simulcast_layers[0].min_bitrate_bps = MinEncBitrateKbps * 1000;
    video_stream_encoder_->ConfigureEncoder(video_encoder_config.Copy(), kMaxPayloadLength);

    video_source_.IncomingCapturedFrame(CreateFrame(1, nullptr));
    WaitForEncodedFrame(1);
    EXPECT_EQ(MaxEncBitrateKbps, static_cast<int>(bitrate_allocator_factory_.codec_config().maxBitrate));
    EXPECT_EQ(MinEncBitrateKbps, static_cast<int>(bitrate_allocator_factory_.codec_config().minBitrate));
...
TEST_F(VideoStreamEncoderTest, EncoderDoesnotProvideLimitsWhenQPIsNotTrusted)
{
    ...
    video_stream_encoder_->OnBitrateUpdatedAndWaitForManagedResources(DataRate::BitsPerSec(target_encoder_bitrate),
    ...
    video_encoder_config.simulcast_layers[0].min_bitrate_bps = min_encoder_bitrate;
    video_stream_encoder_->ConfigureEncoder(video_encoder_config.Copy(), kMaxPayloadLength);

    video_source_.IncomingCapturedFrame(CreateFrame(1, nullptr));
    WaitForEncodedFrame(1);
    EXPECT_EQ(max_encoder_bitrate / 1000, static_cast<int>(bitrate_allocator_factory_.codec_config().maxBitrate));
    EXPECT_EQ(min_encoder_bitrate / 1000, static_cast<int>(bitrate_allocator_factory_.codec_config().minBitrate));

    video_stream_encoder_->Stop();
}

TEST_F(VideoStreamEncoderTest, RespectsLowerThanDefaultMinBitrateWhenQPIsNotTrusted)
{
    // Set QP not trusted in encoder info.
    fake_encoder_.SetIsQpTrusted(false);
    ...
    video_encoder_config.max_bitrate_bps = (max_encoder_bitrate + min_encoder_bitrate) / 2;
    video_encoder_config.simulcast_layers[0].min_bitrate_bps = min_encoder_bitrate;
    video_stream_encoder_->ConfigureEncoder(video_encoder_config.Copy(), kMaxPayloadLength);

    video_source_.IncomingCapturedFrame(CreateFrame(1, nullptr));
    WaitForEncodedFrame(1);
    EXPECT_EQ(video_encoder_config.max_bitrate_bps / 1000,
...
TEST_F(VideoStreamEncoderTest, NormalComplexityWithMoreThanTwoCores)
{
    ...
    video_stream_encoder_
        ->OnBitrateUpdatedAndWaitForManagedResources(kTargetBitrate, kTargetBitrate, kTargetBitrate, 0, 0, 0);
    video_source_.IncomingCapturedFrame(CreateFrame(1, /*width=*/320, /*height=*/180));
    WaitForEncodedFrame(1);
    EXPECT_EQ(fake_encoder_.LastEncoderComplexity(), VideoCodecComplexity::kComplexityNormal);
    video_stream_encoder_->Stop();
}

TEST_F(VideoStreamEncoderTest, NormalComplexityWhenLowTierOptimizationsAreDisabled)
{
    webrtc::test::ScopedKeyValueConfig field_trials(field_trials_, "WebRTC-VP9-LowTierOptimizations/Disabled/");
...
TEST_F(VideoStreamEncoderTest, LowComplexityWithTwoCores)
{
    ...
    video_source_.IncomingCapturedFrame(CreateFrame(1, /*width=*/320, /*height=*/180));
    WaitForEncodedFrame(1);
    EXPECT_EQ(fake_encoder_.LastEncoderComplexity(), VideoCodecComplexity::kComplexityLow);
    video_stream_encoder_->Stop();
}
...
TEST_F(VideoStreamEncoderTest, QualityScalingAllowedAndQPIsTrusted_BandwidthScalerDisable)
{
    VideoEncoderConfig video_encoder_config = video_encoder_config_.Copy();
...
TEST_F(VideoStreamEncoderTest, QualityScalingAllowedAndQPIsNotTrusted_BandwidthScalerEnabled)
{
    VideoEncoderConfig video_encoder_config = video_encoder_config_.Copy();
...
TEST_F(VideoStreamEncoderTest, RequestsRefreshFrameAfterEarlyDroppedNativeFrame)
{
...
TEST_F(VideoStreamEncoderTest, RecreatesEncoderWhenEnableVp9SpatialLayer)
{
    ...
    video_encoder_config.encoder_specific_settings =
        rtc::make_ref_counted<VideoEncoderConfig::Vp9EncoderSpecificSettings>(vp9_settings);
    video_encoder_config.spatial_layers = GetSvcConfig(1280,
                                                       720,
                                                       /*fps=*/30.0,
    ...
    video_stream_encoder_->ConfigureEncoder(video_encoder_config.Copy(), kMaxPayloadLength, nullptr);
...
class VideoStreamEncoderWithRealEncoderTest : public VideoStreamEncoderTest,
...
public:
    ...
    {
        VideoStreamEncoderTest::SetUp();
        RtcContext env = CreateRtcContext(&field_trials_);
        std::unique_ptr<VideoEncoder> encoder;
        switch (codec_type_)
        {
            case kVideoCodecVP8: encoder = CreateVp8Encoder(env); break;
            case kVideoCodecVP9: encoder = CreateVp9Encoder(env); break;
            case kVideoCodecAV1: encoder = CreateLibaomAv1Encoder(env); break;
            case kVideoCodecH264: encoder = CreateH264Encoder(env); break;
...
protected:
    void ConfigureEncoderAndBitrate(VideoCodecType codec_type, std::unique_ptr<VideoEncoder> encoder)
    {
        // Configure VSE to use the encoder.
        encoder_ = std::move(encoder);
        encoder_proxy_factory_ = std::make_unique<test::VideoEncoderProxyFactory>(encoder_.get(), &encoder_selector_);
        video_send_config_.encoder_settings.encoder_factory = encoder_proxy_factory_.get();
        VideoEncoderConfig video_encoder_config;
...
TEST_P(VideoStreamEncoderWithRealEncoderTest, HandlesLayerToggling)
{
    ...
    {
        if (codec_type_ == VideoCodecType::kVideoCodecVP9 || codec_type_ == VideoCodecType::kVideoCodecAV1)
        {
    ...
    video_stream_encoder_
        ->OnBitrateUpdatedAndWaitForManagedResources(kTwoLayerBitrate, kTwoLayerBitrate, kTwoLayerBitrate, 0, 0, 0);
    video_stream_encoder_->WaitUntilTaskQueueIsIdle();
    ...
    video_stream_encoder_->OnBitrateUpdatedAndWaitForManagedResources(DataRate::KilobitsPerSec(1800),
...
INSTANTIATE_TEST_SUITE_P(
    All,
    VideoStreamEncoderWithRealEncoderTest,
    ::testing::Values(kVP8DisallowConversion, kVP9DisallowConversion, kAV1AllowConversion, kH264AllowConversion),
    TestParametersVideoCodecAndAllowI420ConversionToString);
#else
INSTANTIATE_TEST_SUITE_P(All,
                         VideoStreamEncoderWithRealEncoderTest,
                         ::testing::Values(kVP8DisallowConversion, kVP9DisallowConversion, kAV1AllowConversion),
                         TestParametersVideoCodecAndAllowI420ConversionToString);
#endif
...
{
protected:
    void RunSimulcastTest(const std::vector<std::vector<VideoStream>> &configs, const int expected_num_init_encode)
    {
    ...
    {
        timestamp_ms_ += kFrameIntervalMs;
        video_source_.IncomingCapturedFrame(CreateFrame(timestamp_ms_, kWidth, kHeight));
        sink_.WaitForEncodedFrame(timestamp_ms_);
    }

    void ExpectEqual(const VideoCodec &actual, const std::vector<VideoStream> &expected) const
    {
        EXPECT_EQ(actual.numberOfSimulcastStreams, expected.size());
        for (size_t i = 0; i < actual.numberOfSimulcastStreams; ++i)
        {
            EXPECT_EQ(actual.simulcastStream[i].maxFramerate, expected[i].max_framerate);
            ...
            EXPECT_EQ(actual.simulcastStream[i].width, kWidth / expected[i].scale_resolution_down_by);
            EXPECT_EQ(actual.simulcastStream[i].height, kHeight / expected[i].scale_resolution_down_by);
            EXPECT_EQ(actual.simulcastStream[i].numberOfTemporalLayers, expected[i].num_temporal_layers);
...
TEST_F(ReconfigureSimulcastEncoderTest, ReconfiguredIfMaxFramerateChanges)
{
    ...
    RunSimulcastTest({{config_before_high, config_before_low}, {config_after_high, config_after_low}},
...
{
public:
    void RunTest(const std::vector<VideoStream> &configs, const int expected_num_init_encode)
    {
...
TEST_F(ReconfigureEncoderTest, UpdatesNumTemporalLayersFromScalabilityModeChanges)
{
    VideoStream config1 = DefaultConfig();
    VideoStream config2 = config1;
...
{
    class SuperLazyTaskQueue : public webrtc::TaskQueue
    {
    public:
    ...
    RtcContext env = CreateRtcContext(time_controller.GetClock());
    MockableSendStatisticsProxy stats_proxy(&env.clock(),
                                            VideoSendStream::Config(nullptr),
                                            VideoEncoderConfig::ContentType::kRealtimeVideo,
                                            env.field_trials());
    ...
    std::unique_ptr<VideoBitrateAllocatorFactory> bitrate_allocator_factory =
        CreateBuiltinVideoBitrateAllocatorFactory();
    VideoStreamEncoderSettings encoder_settings{VideoEncoder::Capabilities(/*loss_notification=*/false)};
    ...
    std::unique_ptr<webrtc::TaskQueue, webrtc::TaskQueueDeleter> encoder_queue(new SuperLazyTaskQueue());
    ...
    // simply be deleted.
    VideoStreamEncoder encoder(env,
                               1,
                               &stats_proxy,
                               encoder_settings,
...
TEST(VideoStreamEncoderFrameCadenceTest, ActivatesFrameCadenceOnContentType)
{
    ...
    FrameCadenceAdapterInterface::Callback *video_stream_encoder_callback = nullptr;
    EXPECT_CALL(*adapter_ptr, Initialize)
        .WillOnce(Invoke([&video_stream_encoder_callback](FrameCadenceAdapterInterface::Callback *callback)
                         { video_stream_encoder_callback = callback; }));
    TaskQueue *encoder_queue = nullptr;
    auto video_stream_encoder = factory.Create(std::move(adapter), &encoder_queue);

    // First a call before we know the frame size and hence cannot compute the
    // number of simulcast layers.
    EXPECT_CALL(*adapter_ptr,
                SetZeroHertzModeEnabled(
                    Optional(Field(&FrameCadenceAdapterInterface::ZeroHertzModeParams::num_simulcast_layers, Eq(0u)))));
    VideoEncoderConfig config;
    ...
    EXPECT_CALL(*adapter_ptr,
                SetZeroHertzModeEnabled(
                    Optional(Field(&FrameCadenceAdapterInterface::ZeroHertzModeParams::num_simulcast_layers, Gt(0u)))));
...
TEST(VideoStreamEncoderFrameCadenceTest, ForwardsFramesIntoFrameCadenceAdapter)
{
    auto adapter = std::make_unique<MockFrameCadenceAdapter>();
    auto *adapter_ptr = adapter.get();
    ...
    video_stream_encoder->SetSource(&video_source, webrtc::DegradationPreference::MAINTAIN_FRAMERATE);

    EXPECT_CALL(*adapter_ptr, OnFrame);
    auto buffer = rtc::make_ref_counted<NV12Buffer>(/*width=*/16, /*height=*/16);
    video_source.IncomingCapturedFrame(VideoFrame::Builder().set_video_frame_buffer(std::move(buffer)).build());
}
...
TEST(VideoStreamEncoderFrameCadenceTest, DeactivatesActivatesLayersOnBitrateChanges)
{
    auto adapter = std::make_unique<MockFrameCadenceAdapter>();
    auto *adapter_ptr = adapter.get();
    SimpleVideoStreamEncoderFactory factory;
    ...
    video_stream_encoder->ConfigureEncoder(video_encoder_config.Copy(), kMaxPayloadLength);
    // Ensure an encoder is created.
    PassAFrame(encoder_queue, video_stream_encoder_callback, /*ntp_time_ms=*/1);

    // Both layers enabled at 1 MBit/s.
    video_stream_encoder->OnBitrateUpdated(DataRate::KilobitsPerSec(1000),
    ...
    video_stream_encoder->OnBitrateUpdated(DataRate::KilobitsPerSec(200),
    ...
    video_stream_encoder->OnBitrateUpdated(DataRate::Zero(), DataRate::Zero(), DataRate::Zero(), 0, 0, 0);
...
TEST(VideoStreamEncoderFrameCadenceTest, UpdatesQualityConvergence)
{
    ...
    EXPECT_CALL(factory.GetMockFakeEncoder(), GetEncoderInfo).WillRepeatedly(Return(info));
    ...
    EXPECT_CALL(factory.GetMockFakeEncoder(), EncodeHook)
        .WillRepeatedly(Invoke(
            [](EncodedImage &encoded_image, ScopedRefPtr<EncodedImageBuffer> buffer)
            {
                ...
                encoded_image.qp_ = kVp8SteadyStateQpThreshold + (encoded_image.SimulcastIndex() == 0 ? 0 : 1);
                CodecSpecificInfo codec_specific;
                codec_specific.codecType = kVideoCodecVP8;
                return codec_specific;
            }));
...
TEST(VideoStreamEncoderFrameCadenceTest, RequestsRefreshFramesWhenCadenceAdapterInstructs)
{
    auto adapter = std::make_unique<MockFrameCadenceAdapter>();
    auto *adapter_ptr = adapter.get();
    ...
    EXPECT_CALL(*adapter_ptr, ProcessKeyFrameRequest)
        .WillOnce(Invoke([video_stream_encoder_callback] { video_stream_encoder_callback->RequestRefreshFrame(); }));
...
TEST(VideoStreamEncoderFrameCadenceTest, RequestsRefreshFrameForEarlyZeroHertzKeyFrameRequest)
{
    ...
    factory.GetTimeController()->AdvanceTime(
        TimeDelta::Seconds(1) * FrameCadenceAdapterInterface::kOnDiscardedFrameRefreshFramePeriod / kMaxFps);
}
...
public:
    VideoStreamEncoderFrameCadenceRestrictionTest()
        ...
        , video_stream_encoder_(factory_.Create(std::move(adapter_), &encoder_queue_))
    {
    }

    ~VideoStreamEncoderFrameCadenceRestrictionTest() { factory_.DepleteTaskQueues(); }
...
TEST_F(VideoStreamEncoderFrameCadenceRestrictionTest, UpdatesVideoSourceRestrictionsUnRestricted)
{
...
TEST_F(VideoStreamEncoderFrameCadenceRestrictionTest, UpdatesVideoSourceRestrictionsWithMaxFrameRateRestriction)
{
...
TEST_F(VideoStreamEncoderFrameCadenceRestrictionTest, UpdatesVideoSourceRestrictionsWithoutMaxFrameRateRestriction)
{
