...
TEST(SimulcastToSvc, ConvertsEncodedImage)
{
    ...
    EXPECT_EQ(image.spatialIndex(), utils::nullopt);
...
TEST(SimulcastToSvc, PredictsInternalStateCorrectlyOnFrameDrops)
{
    ...
    std::unique_ptr<ScalableVideoController> svc_controller = CreateScalabilityStructure(ScalabilityMode::kS3T3);
    ...
    for (int i = 0; i < 100; ++i)
    {
        ...
        for (int sid = 0; sid < 3; ++sid)
        {
            ...
            codec_specific.codecSpecific.VP9.num_spatial_layers = 3;
            codec_specific.codecSpecific.VP9.first_active_layer = 0;
            codec_specific.codecSpecific.VP9.temporal_idx = layer_config[sid].TemporalId();
            codec_specific.generic_frame_info = svc_controller->OnEncodeDone(layer_config[sid]);
