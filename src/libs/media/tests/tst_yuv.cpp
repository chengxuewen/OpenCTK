...
TestLibYuv::TestLibYuv()
    : source_file_(NULL), orig_frame_(), width_(352), height_(288), size_y_(width_ * height_), size_uv_(
    ((width_ + 1) / 2) * ((height_ + 1) / 2)), frame_length_(utils::calcBufferSize(VideoType::kI420, 352, 288)) {}

void TestLibYuv::SetUp()
{
    const std::string input_file_name = utils::ResourcePath("foreman_cif", "yuv");
    source_file_ = fopen(input_file_name.c_str(), "rb");
    ASSERT_TRUE(source_file_ != NULL) << "Cannot read file: " << input_file_name << "\n";

    std::shared_ptr<I420BufferInterface> buffer(utils::ReadI420Buffer(width_, height_, source_file_));

    orig_frame_ = utils::make_unique<VideoFrame>(VideoFrame::Builder()
...
// Reading YUV frame - testing on the first frame of the foreman sequence
    int j = 0;
    // std::string output_file_name = utils::outputPath() + "LibYuvTest_conversion.yuv";
    std::string output_file_name = "LibYuvTest_conversion.yuv";
    FILE *output_file = fopen(output_file_name.c_str(), "wb");
    ASSERT_TRUE(output_file != NULL);

    double psnr = 0.0;

    std::shared_ptr<I420Buffer> res_i420_buffer = I420Buffer::create(width_, height_);

    printf("\nConvert #%d I420 <-> I420 \n", j);
    std::unique_ptr<uint8_t[]> out_i420_buffer(new uint8_t[frame_length_]);
    EXPECT_EQ(0, utils::ConvertFromI420(*orig_frame_, VideoType::kI420, 0, out_i420_buffer.get()));
    ...
    psnr = utils::I420PSNR(*orig_frame_->videoFrameBuffer()->getI420(), *res_i420_buffer);
    EXPECT_EQ(48.0, psnr);
    j++;
...
// Align the stride values for the output frame.
    ...
    res_i420_buffer = I420Buffer::create(width_, height_, stride_y, stride_uv, stride_uv);
    EXPECT_EQ(0, utils::ConvertFromI420(*orig_frame_, VideoType::kRGB24, 0, res_rgb_buffer2.get()));

    ret = libyuv::ConvertToI420(res_rgb_buffer2.get(), 0, res_i420_buffer.get()->MutableDataY(),
                                ...
                                utils::ConvertVideoType(VideoType::kRGB24));
    ...
    EXPECT_EQ(0, utils::ConvertFromI420(*orig_frame_, VideoType::kUYVY, 0, out_uyvy_buffer.get()));

    ret = libyuv::ConvertToI420(out_uyvy_buffer.get(), 0, res_i420_buffer.get()->MutableDataY(),
                                res_i420_buffer.get()->strideY(), res_i420_buffer.get()->MutableDataU(),
                                res_i420_buffer.get()->strideU(), res_i420_buffer.get()->MutableDataV(),
                                res_i420_buffer.get()->strideV(), 0, 0, width_, height_,
                                res_i420_buffer->width(), res_i420_buffer->height(), libyuv::kRotate0,
                                utils::ConvertVideoType(VideoType::kUYVY));
    ...
    EXPECT_EQ(0, utils::ConvertFromI420(*orig_frame_, VideoType::kYUY2, 0, out_yuy2_buffer.get()));

    ret = libyuv::ConvertToI420(out_yuy2_buffer.get(), 0, res_i420_buffer.get()->MutableDataY(),
                                res_i420_buffer.get()->strideY(), res_i420_buffer.get()->MutableDataU(),
                                res_i420_buffer.get()->strideU(), res_i420_buffer.get()->MutableDataV(),
                                res_i420_buffer.get()->strideV(), 0, 0, width_, height_,
                                res_i420_buffer->width(), res_i420_buffer->height(), libyuv::kRotate0,
                                utils::ConvertVideoType(VideoType::kYUY2));
    ...
    std::unique_ptr<uint8_t[]> out_rgb565_buffer(new uint8_t[width_ * height_ * 2]);
    EXPECT_EQ(0, utils::ConvertFromI420(*orig_frame_, VideoType::kRGB565, 0, out_rgb565_buffer.get()));

    ret = libyuv::ConvertToI420(out_rgb565_buffer.get(), 0, res_i420_buffer.get()->MutableDataY(),
                                res_i420_buffer.get()->strideY(), res_i420_buffer.get()->MutableDataU(),
                                res_i420_buffer.get()->strideU(), res_i420_buffer.get()->MutableDataV(),
                                res_i420_buffer.get()->strideV(), 0, 0, width_, height_,
                                res_i420_buffer->width(), res_i420_buffer->height(), libyuv::kRotate0,
                                utils::ConvertVideoType(VideoType::kRGB565));
...
// TODO(mikhal): Add psnr for RGB565, 1555, 4444, convert to ARGB.
    ...
    std::unique_ptr<uint8_t[]> out_argb8888_buffer(new uint8_t[width_ * height_ * 4]);
    EXPECT_EQ(0, utils::ConvertFromI420(*orig_frame_, VideoType::kARGB, 0, out_argb8888_buffer.get()));

    ret = libyuv::ConvertToI420(out_argb8888_buffer.get(), 0, res_i420_buffer.get()->MutableDataY(),
                                res_i420_buffer.get()->strideY(), res_i420_buffer.get()->MutableDataU(),
                                res_i420_buffer.get()->strideU(), res_i420_buffer.get()->MutableDataV(),
                                res_i420_buffer.get()->strideV(), 0, 0, width_, height_,
                                res_i420_buffer->width(), res_i420_buffer->height(), libyuv::kRotate0,
                                utils::ConvertVideoType(VideoType::kARGB));
...
//     std::string output_file_name = utils::outputPath() + "LibYuvTest_conversion.yuv";
    ...
    std::shared_ptr<I420Buffer> res_i420_buffer = I420Buffer::create(width_, height_, stride_y, stride_uv, stride_uv);
    std::unique_ptr<uint8_t[]> out_i420_buffer(new uint8_t[frame_length_]);
    EXPECT_EQ(0, utils::ConvertFromI420(*orig_frame_, VideoType::kI420, 0, out_i420_buffer.get()));
    int y_size = width_ * height_;
    int u_size = res_i420_buffer->chromaWidth() * res_i420_buffer->chromaHeight();
    int ret = libyuv::I420Copy(out_i420_buffer.get(), width_, out_i420_buffer.get() + y_size,
                               width_ >> 1, out_i420_buffer.get() + y_size + u_size, width_ >> 1,
                               res_i420_buffer.get()->MutableDataY(), res_i420_buffer.get()->strideY(),
                               res_i420_buffer.get()->MutableDataU(), res_i420_buffer.get()->strideU(),
                               res_i420_buffer.get()->MutableDataV(), res_i420_buffer.get()->strideV(),
...
TEST_F(TestLibYuv, NV12Scale2x2to2x2)
{
    ...
    utils::NV12Scale(tmp_buffer, src_y.data(),
              2, src_uv.data(),
              2, 2, 2, dst_y.data(),
              2, dst_uv.data(),
              2, 2, 2);
...
TEST_F(TestLibYuv, NV12Scale4x4to2x2)
{
    ...
    tmp_buffer.resize(src_chroma_width * src_chroma_height * 2 + dst_chroma_width * dst_chroma_height * 2);
    tmp_buffer.shrink_to_fit();

    utils::NV12Scale(tmp_buffer.data(), src_y, 4, src_uv, 4, 4, 4, dst_y.data(),
              2, dst_uv.data(),
              2, 2, 2);

    EXPECT_THAT(dst_y, ::testing::ElementsAre(Average(0, 1, 4, 5), Average(2, 3, 6, 7),
                                              Average(8, 9, 12, 13), Average(10, 11, 14, 15)));
    EXPECT_THAT(dst_uv, ::testing::ElementsAre(Average(0, 2, 4, 6), Average(1, 3, 5, 7)));
}
...
TEST(I420WeightedPSNRTest, SmokeTest)
{
    ...
    uint8_t ref_uv[] = {0};
    std::shared_ptr<I420Buffer> ref_buffer = I420Buffer::Copy(/*width=*/2, /*height=*/2, ref_y, /*stride_y=*/2, ref_uv,
        /*stride_u=*/1, ref_uv, /*stride_v=*/1);

    uint8_t test_y[] = {1, 1, 1, 1};
    uint8_t test_uv[] = {2};
    std::shared_ptr<I420Buffer> test_buffer = I420Buffer::Copy(
    ...
    EXPECT_NEAR(utils::I420WeightedPSNR(*ref_buffer, *test_buffer),
                (6.0 * psnr(1.0) + psnr(4.0) + psnr(4.0)) / 8.0,/*abs_error=*/0.001);
}
