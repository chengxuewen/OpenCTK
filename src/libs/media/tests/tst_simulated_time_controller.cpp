...
OCTK_CXX14_CONSTEXPR Timestamp kStartTime = Timestamp::Seconds(1000);
} // namespace
...
TEST(SimulatedTimeControllerTest, TaskIsStoppedOnStop)
{
    ...
    std::unique_ptr<TaskQueue, TaskQueueDeleter> task_queue =
        time_simulation.GetTaskQueueFactory()->CreateTaskQueue("TestQueue", TaskQueueFactory::Priority::NORMAL);
...
TEST(SimulatedTimeControllerTest, Example)
{
    ...
    public:
        ...
        void StartPeriodicTask(RepeatingTaskHandle *handle, TaskQueue *task_queue)
        {
    ...
    object->StartPeriodicTask(&handle, task_queue.get());
    auto handleMove1 = utils::makeMoveWrapper(std::move(handle));
    task_queue->PostTask([handleMove1]() mutable { handleMove1.move().Stop(); });
    auto objectMove = utils::makeMoveWrapper(std::move(object));
    task_queue->PostTask([objectMove] { });
}
...
TEST(SimulatedTimeControllerTest, DelayTaskRunOnTime)
{
    ...
    task_queue->PostDelayedTask([&] { delay_task_executed = true; }, TimeDelta::Millis(10));
...
TEST(SimulatedTimeControllerTest, SkipsDelayedTaskForward)
{
    GlobalSimulatedTimeController sim(kStartTime);
    auto main_thread = sim.GetMainThread();
    OCTK_CXX14_CONSTEXPR auto duration_during_which_nothing_runs = TimeDelta::Seconds(2);
    OCTK_CXX14_CONSTEXPR auto shorter_duration = TimeDelta::Seconds(1);
    MockFunction<void()> fun;
    EXPECT_CALL(fun, Call).WillOnce(
        Invoke([&] { ASSERT_EQ(sim.GetClock()->CurrentTime(), kStartTime + duration_during_which_nothing_runs); }));
