...
struct DummyExperiment
{
    ...
    {
        ParseFieldTrial({&enabled, &factor, &retries, &size, &ping, &hash}, field_trial);
    }
};
...
{
    FieldTrialConstrained<double> low("low", 10, utils::nullopt, 100);
    FieldTrialConstrained<double> high("high", 10, 5, utils::nullopt);
...
{
    FieldTrialOptional<int> max_count("c", utils::nullopt);
    ...
    FieldTrialOptional<unsigned> max_size("c", utils::nullopt);
...
{
    FieldTrialEnum<CustomEnum> my_enum(
        "e",
        CustomEnum::kDefault,
        {{"default", CustomEnum::kDefault}, {"red", CustomEnum::kRed}, {"blue", CustomEnum::kBlue}});
