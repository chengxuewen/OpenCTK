...
VideoSourceRestrictions::VideoSourceRestrictions()
    : max_pixels_per_frame_(utils::nullopt)
    , target_pixels_per_frame_(utils::nullopt)
    , max_frame_rate_(utils::nullopt)
{
}

VideoSourceRestrictions::VideoSourceRestrictions(Optional<size_t> max_pixels_per_frame,
...
{
    OCTK_DCHECK(!max_pixels_per_frame_.has_value() ||
                max_pixels_per_frame_.value() < static_cast<size_t>(std::numeric_limits<int>::max()));
    OCTK_DCHECK(!max_frame_rate_.has_value() || max_frame_rate_.value() < std::numeric_limits<int>::max());
...
const Optional<size_t> &VideoSourceRestrictions::max_pixels_per_frame() const { return max_pixels_per_frame_; }

const Optional<size_t> &VideoSourceRestrictions::target_pixels_per_frame() const { return target_pixels_per_frame_; }

const Optional<double> &VideoSourceRestrictions::max_frame_rate() const { return max_frame_rate_; }

void VideoSourceRestrictions::set_max_pixels_per_frame(Optional<size_t> max_pixels_per_frame)
{
    max_pixels_per_frame_ = std::move(max_pixels_per_frame);
}

void VideoSourceRestrictions::set_target_pixels_per_frame(Optional<size_t> target_pixels_per_frame)
{
    target_pixels_per_frame_ = std::move(target_pixels_per_frame);
}

void VideoSourceRestrictions::set_max_frame_rate(Optional<double> max_frame_rate)
{
    max_frame_rate_ = std::move(max_frame_rate);
}
...
void VideoSourceRestrictions::UpdateMin(const VideoSourceRestrictions &other)
{
    if (max_pixels_per_frame_.has_value())
    {
        max_pixels_per_frame_ =
            std::min(*max_pixels_per_frame_, other.max_pixels_per_frame().value_or(std::numeric_limits<size_t>::max()));
    }
    else
    {
    ...
    if (target_pixels_per_frame_.has_value())
    {
        target_pixels_per_frame_ =
            std::min(*target_pixels_per_frame_,
                     other.target_pixels_per_frame().value_or(std::numeric_limits<size_t>::max()));
    }
    else
    {
    ...
    {
        max_frame_rate_ =
            std::min(*max_frame_rate_, other.max_frame_rate().value_or(std::numeric_limits<double>::max()));
    }
    else
    {
...
bool DidRestrictionsIncrease(VideoSourceRestrictions before, VideoSourceRestrictions after)
{
    ...
    bool same_resolution = before.max_pixels_per_frame() == after.max_pixels_per_frame();
    bool same_framerate = before.max_frame_rate() == after.max_frame_rate();

    return (decreased_resolution && decreased_framerate) || (decreased_resolution && same_framerate) ||
...
bool DidRestrictionsDecrease(VideoSourceRestrictions before, VideoSourceRestrictions after)
{
    ...
    return (increased_resolution && increased_framerate) || (increased_resolution && same_framerate) ||
...
bool DidIncreaseResolution(VideoSourceRestrictions restrictions_before, VideoSourceRestrictions restrictions_after)
{
    ...
    if (!restrictions_after.max_pixels_per_frame().has_value())
        return true;
    return restrictions_after.max_pixels_per_frame().value() > restrictions_before.max_pixels_per_frame().value();
}
...
bool DidIncreaseFrameRate(VideoSourceRestrictions restrictions_before, VideoSourceRestrictions restrictions_after)
{
    ...
    if (!restrictions_after.max_frame_rate().has_value())
        return true;
    return restrictions_after.max_frame_rate().value() > restrictions_before.max_frame_rate().value();
}
