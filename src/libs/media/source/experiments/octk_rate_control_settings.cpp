...
const char kCongestionWindowDefaultFieldTrialString[] = "QueueSize:350,MinBitrate:30000,DropFrame:true";

const char kUseBaseHeavyVp8Tl3RateAllocationFieldTrialName[] = "WebRTC-UseBaseHeavyVP8TL3RateAllocation";
...
{
    return StructParametersParser::Create("QueueSize",
                                          &queue_size_ms, //
                                          "MinBitrate",
                                          &min_bitrate_bps,
                                          "InitWin",
...
// static
CongestionWindowConfig CongestionWindowConfig::Parse(StringView config)
{
...
std::unique_ptr<StructParametersParser> VideoRateControlConfig::Parser()
{
    ...
    return StructParametersParser::Create("pacing_factor",
                                          &pacing_factor, //
                                          "alr_probing",
                                          &alr_probing, //
                                          "vp8_qp_max",
...
RateControlSettings::RateControlSettings(const FieldTrialsView &key_value_config)
{
    std::string congestion_window_config = key_value_config.Lookup(CongestionWindowConfig::kKey);
    if (congestion_window_config.empty())
    {
        congestion_window_config = kCongestionWindowDefaultFieldTrialString;
    }
    congestion_window_config_ = CongestionWindowConfig::Parse(congestion_window_config);
    video_config_.vp8_base_heavy_tl3_alloc = key_value_config.IsEnabled(
        kUseBaseHeavyVp8Tl3RateAllocationFieldTrialName);
    video_config_.Parser()->Parse(key_value_config.Lookup(VideoRateControlConfig::kKey));
}
...
{
    return congestion_window_config_.queue_size_ms.value_or(kDefaultAcceptedQueueMs);
}

bool RateControlSettings::UseCongestionWindowPushback() const
{
    return congestion_window_config_.queue_size_ms && congestion_window_config_.min_bitrate_bps;
}
...
uint32_t RateControlSettings::CongestionWindowMinPushbackTargetBitrateBps() const
{
    return congestion_window_config_.min_bitrate_bps.value_or(kDefaultMinPushbackTargetBitrateBps);
}

Optional<DataSize> RateControlSettings::CongestionWindowInitialDataWindow() const
{
    return congestion_window_config_.initial_data_window;
}

Optional<double> RateControlSettings::GetPacingFactor() const
{
...
Optional<int> RateControlSettings::LibvpxVp8QpMax() const
{
    if (video_config_.vp8_qp_max && (*video_config_.vp8_qp_max < 0 || *video_config_.vp8_qp_max > 63))
    {
        OCTK_WARNING() << "Unsupported vp8_qp_max_ value, ignored.";
        return utils::nullopt;
    }
    return video_config_.vp8_qp_max;
}

Optional<int> RateControlSettings::LibvpxVp8MinPixels() const
{
