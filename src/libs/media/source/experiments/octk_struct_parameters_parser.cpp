 ...
 */
#include <private/octk_struct_parameters_parser_p.hpp>
...
namespace
{
size_t FindOrEnd(StringView str, size_t start, char delimiter)
{
    size_t pos = str.find(delimiter, start);
    pos = (pos == StringView::npos) ? str.length() : pos;
    return pos;
}
} // namespace
...
template <typename T>
inline void StringEncode(std::string *sb, Optional<T> val)
{
    if (val)
        StringEncode(sb, *val);
}
...
template <typename T>
bool TypedParser<T>::Parse(StringView src, void *target)
{
...
template class TypedParser<Optional<double>>;
template class TypedParser<Optional<int>>;
template class TypedParser<Optional<unsigned>>;

template class TypedParser<DataRate>;
template class TypedParser<DataSize>;
template class TypedParser<TimeDelta>;
template class TypedParser<Optional<DataRate>>;
template class TypedParser<Optional<DataSize>>;
template class TypedParser<Optional<TimeDelta>>;
} // namespace detail

StructParametersParser::StructParametersParser(std::vector<detail::MemberParameter> members)
    : members_(std::move(members))
{
}

void StructParametersParser::Parse(StringView src)
{
    size_t i = 0;
    while (i < src.length())
    {
        ...
        StringView key(src.substr(i, key_end - i));
        ...
        for (auto &member : members_)
        {
            if (key == member.key)
            {
                ...
                {
                    OCTK_WARNING() << "Failed to read field with key: '" << key << "' in trial: \"" << src << "\"";
                }
                break;
            }
        }
        ...
        {
            OCTK_INFO() << "No field with key: '" << key << "' (found in trial: \"" << src << "\")";
        }
    }
}
