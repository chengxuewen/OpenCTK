...
struct CongestionWindowConfig
{
    ...
    Optional<int> queue_size_ms;
    Optional<int> min_bitrate_bps;
    Optional<DataSize> initial_data_window;
    bool drop_frame_only = false;
    std::unique_ptr<StructParametersParser> Parser();
    static CongestionWindowConfig Parse(StringView config);
};

struct VideoRateControlConfig
{
    static constexpr char kKey[] = "WebRTC-VideoRateControl";
    Optional<double> pacing_factor;
    bool alr_probing = false;
    Optional<int> vp8_qp_max;
    Optional<int> vp8_min_pixels;
    bool trust_vp8 = true;
    bool trust_vp9 = true;
...
public:
    ...
    Optional<DataSize> CongestionWindowInitialDataWindow() const;

    Optional<double> GetPacingFactor() const;
    bool UseAlrProbing() const;

    Optional<int> LibvpxVp8QpMax() const;
    Optional<int> LibvpxVp8MinPixels() const;
