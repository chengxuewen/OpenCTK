...
#ifndef _OCTK_BALANCED_DEGRADATION_SETTINGS_HPP
#define _OCTK_BALANCED_DEGRADATION_SETTINGS_HPP
...
public:
    ...
    struct CodecTypeSpecific
    {
        ...
        {
            return qp_low == o.qp_low && qp_high == o.qp_high && fps == o.fps && kbps == o.kbps &&
                   kbps_res == o.kbps_res;
        }

        Optional<int> GetQpLow() const;
        Optional<int> GetQpHigh() const;
        Optional<int> GetFps() const;
        Optional<int> GetKbps() const;
        Optional<int> GetKbpsRes() const;
    ...
    struct Config
    {
        ...
        {
            return pixels == o.pixels && fps == o.fps && kbps == o.kbps && kbps_res == o.kbps_res &&
                   fps_diff == o.fps_diff && vp8 == o.vp8 && vp9 == o.vp9 && h264 == o.h264 && av1 == o.av1 &&
                   generic == o.generic;
        }
    ...
    bool CanAdaptUpResolution(VideoCodecType type, int pixels, uint32_t bitrate_bps) const;

    // Gets the min framerate diff from `configs_` based on `pixels`.
    Optional<int> MinFpsDiff(int pixels) const;

    // Gets QpThresholds for the codec `type` based on `pixels`.
    Optional<VideoEncoder::QpThresholds> GetQpThresholds(VideoCodecType type, int pixels) const;

private:
    Optional<Config> GetMinFpsConfig(int pixels) const;
    Optional<Config> GetMaxFpsConfig(int pixels) const;
...
#endif // _OCTK_BALANCED_DEGRADATION_SETTINGS_HPP
