...
void ChainDiffCalculator::Reset(const std::vector<bool> &chains)
{
    ...
    for (size_t i = 0; i < chains.size(); ++i)
    {
        if (chains[i])
        {
            last_frame_in_chain_[i] = utils::nullopt;
        }
    }
}

std::vector<int> ChainDiffCalculator::ChainDiffs(int64_t frame_id) const
{
...
std::vector<int> ChainDiffCalculator::From(int64_t frame_id, const std::vector<bool> &chains)
{
    auto result = ChainDiffs(frame_id);
    if (chains.size() != last_frame_in_chain_.size())
    {
        OCTK_ERROR() << "Insconsistent chain configuration for frame#" << frame_id << ": expected "
                     << last_frame_in_chain_.size() << " chains, found " << chains.size();
    }
