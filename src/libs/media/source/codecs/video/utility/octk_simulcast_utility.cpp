...
uint32_t SimulcastUtility::SumStreamMaxBitrate(int streams, const VideoCodec &codec)
{
...
bool SimulcastUtility::ValidSimulcastParameters(const VideoCodec &codec, int num_streams)
{
    // Check resolution.
    ...
    {
        if (codec.width * codec.simulcastStream[i].height != codec.height * codec.simulcastStream[i].width)
        {
            return false;
        }
    }
    ...
    for (int i = 1; i < num_streams; ++i)
    {
        if (fabs(codec.simulcastStream[i].maxFramerate - codec.simulcastStream[i - 1].maxFramerate) > 1e-9)
        {
            return false;
        }
    }
    ...
    {
        if (codec.simulcastStream[i].numberOfTemporalLayers != codec.simulcastStream[i + 1].numberOfTemporalLayers)
            return false;
    }
    return true;
}
...
{
    return codec.mode == VideoCodecMode::kScreensharing && codec.legacy_conference_mode &&
           (codec.codecType == kVideoCodecVP8 || codec.codecType == kVideoCodecH264);
}

bool SimulcastUtility::IsConferenceModeScreenshare(const VideoEncoderConfig &encoder_config)
{
    return encoder_config.content_type == VideoEncoderConfig::ContentType::kScreen &&
...
int SimulcastUtility::NumberOfTemporalLayers(const VideoCodec &codec, int spatial_id)
{
    ...
    else
    {
        switch (codec.codecType)
        {
            ...
            case kVideoCodecGeneric: break;
        }
    }

    if (codec.numberOfSimulcastStreams > 0)
    {
        OCTK_DCHECK_LT(spatial_id, codec.numberOfSimulcastStreams);
        num_temporal_layers = std::max(num_temporal_layers,
                                       static_cast<int>(codec.simulcastStream[spatial_id].numberOfTemporalLayers));
    }
