...
std::vector<int64_t> FrameDependenciesCalculator::FromBuffersUsage(int64_t frame_id,
                                                                   ArrayView<const CodecBufferUsage> buffers_usage)
{
    std::vector<int64_t> dependencies;
    OCTK_DCHECK_GT(buffers_usage.size(), 0);
    for (const CodecBufferUsage &buffer_usage : buffers_usage)
    {
        OCTK_CHECK_GE(buffer_usage.id, 0);
    ...
    for (const CodecBufferUsage &buffer_usage : buffers_usage)
    {
        ...
        if (buffer.frame_id == utils::nullopt)
        {
            OCTK_ERROR() << "Odd configuration: frame " << frame_id << " references buffer #" << buffer_usage.id
                         << " that was never updated.";
            continue;
        }
        direct_depenendencies.insert(*buffer.frame_id);
        indirect_depenendencies.insert(buffer.dependencies.begin(), buffer.dependencies.end());
    }
    ...
    std::set_difference(direct_depenendencies.begin(),
                        direct_depenendencies.end(),
                        indirect_depenendencies.begin(),
                        indirect_depenendencies.end(),
    ...
    for (const CodecBufferUsage &buffer_usage : buffers_usage)
    {
        ...
        buffer.dependencies.assign(direct_depenendencies.begin(), direct_depenendencies.end());
    }
