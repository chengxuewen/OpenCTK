...
#include "octk_frame_delay_variation_kalman_filter.hpp"
#include "octk_rtt_filter.hpp"
...
class JitterEstimator {
 public:
  ...
  struct Config {
    ...
    static Config ParseAndValidate(StringView field_trial);
    ...
    std::optional<double> max_frame_size_percentile = utils::nullopt;

    // The length of the percentile filters' window, in number of frames.
    std::optional<int> frame_size_window = utils::nullopt;
    ...
    std::optional<double> num_stddev_delay_clamp = utils::nullopt;
    ...
    std::optional<double> num_stddev_delay_outlier = utils::nullopt;
    ...
    //
    // Increasing this value rejects fewer samples.
    std::optional<double> num_stddev_size_outlier = utils::nullopt;
    ...
    std::optional<double> congestion_rejection_factor = utils::nullopt;
