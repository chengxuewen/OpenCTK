...
#include "octk_jitter_estimator_p.hpp"
...
void JitterEstimator::Reset() {
  ...
  last_update_time_ = utils::nullopt;
  prev_estimate_ = utils::nullopt;
  prev_frame_size_ = utils::nullopt;
  avg_noise_ms_ = 0.0;
  var_noise_ms2_ = 4.0;
  alpha_count_ = 1;
...
TimeDelta JitterEstimator::CalculateEstimate() {
  ...
  if (ret < kMinJitterEstimate) {
    ...
    OCTK_DCHECK_GE(ret, kMinJitterEstimate);
...
Frequency JitterEstimator::GetFrameRate() const {
  ...
  // Sanity check.
  OCTK_DCHECK_GE(fps, Frequency::Zero());
