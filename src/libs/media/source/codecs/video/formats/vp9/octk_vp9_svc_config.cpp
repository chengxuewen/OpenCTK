...
size_t GetLimitedNumSpatialLayers(size_t width, size_t height)
{
    ...
    const size_t min_width = is_landscape ? kMinVp9SpatialLayerLongSideLength : kMinVp9SpatialLayerShortSideLength;
    const size_t min_height = is_landscape ? kMinVp9SpatialLayerShortSideLength : kMinVp9SpatialLayerLongSideLength;
...
{
    num_spatial_layers = std::min(num_spatial_layers, kMaxNumLayersForScreenSharing);
    ...
    for (size_t sl_idx = 0; sl_idx < num_spatial_layers; ++sl_idx)
    {
        ...
        spatial_layer.maxFramerate = std::min(kMaxScreenSharingLayerFramerateFps[sl_idx], max_framerate_fps);
        spatial_layer.numberOfTemporalLayers = 1;
        spatial_layer.minBitrate = static_cast<int>(kMinScreenSharingLayerBitrateKbps[sl_idx]);
        spatial_layer.maxBitrate = static_cast<int>(kMaxScreenSharingLayerBitrateKbps[sl_idx]);
        spatial_layer.targetBitrate = static_cast<int>(kTargetScreenSharingLayerBitrateKbps[sl_idx]);
...
std::vector<SpatialLayer> ConfigureSvcNormalVideo(size_t input_width,
                                                  ...
                                                  Optional<ScalableVideoController::StreamLayersConfig> config)
{
    OCTK_DCHECK_LT(first_active_layer, num_spatial_layers);

    // Limit number of layers for given resolution.
    size_t limited_num_spatial_layers = GetLimitedNumSpatialLayers(input_width, input_height);
    if (limited_num_spatial_layers < num_spatial_layers)
    {
        OCTK_WARNING() << "Reducing number of spatial layers from " << num_spatial_layers << " to "
                       << limited_num_spatial_layers << " due to low input resolution.";
    ...
    if (config)
    {
        ...
        for (size_t sl_idx = 0; sl_idx < num_spatial_layers; ++sl_idx)
        {
            required_divisiblity = utils::lcm(required_divisiblity, config->scaling_factor_den[sl_idx]);
        }
    }
    ...
    std::vector<SpatialLayer> spatial_layers;
    for (size_t sl_idx = first_active_layer; sl_idx < num_spatial_layers; ++sl_idx)
    {
        SpatialLayer spatial_layer = {0};
        ...
        if (config)
        {
            spatial_layer.width = input_width * config->scaling_factor_num[sl_idx] / config->scaling_factor_den[sl_idx];
        ...
        int min_bitrate = static_cast<int>((600. * std::sqrt(num_pixels) - 95000.) / 1000.);
        min_bitrate = std::max(min_bitrate, 0);
        spatial_layer.minBitrate = std::max(static_cast<size_t>(min_bitrate), kMinVp9SvcBitrateKbps);
        spatial_layer.maxBitrate = static_cast<int>((1.6 * num_pixels + 50 * 1000) / 1000);
        spatial_layer.targetBitrate = (spatial_layer.minBitrate + spatial_layer.maxBitrate) / 2;
        spatial_layers.push_back(spatial_layer);
    }
...
{
    OCTK_DCHECK_EQ(codec.codecType, kVideoCodecVP9);

    Optional<ScalabilityMode> scalability_mode = codec.GetScalabilityMode();
    OCTK_DCHECK(scalability_mode.has_value());

    bool requested_single_spatial_layer = ScalabilityModeToNumSpatialLayers(*scalability_mode) == 1;

    // Limit number of spatial layers for given resolution.
    int limited_num_spatial_layers = GetLimitedNumSpatialLayers(codec.width, codec.height);
    if (limited_num_spatial_layers < ScalabilityModeToNumSpatialLayers(*scalability_mode))
    {
        ScalabilityMode limited_scalability_mode = LimitNumSpatialLayers(*scalability_mode, limited_num_spatial_layers);
        OCTK_WARNING() << "Reducing number of spatial layers due to low input resolution: "
    ...
    if (!info.has_value())
    {
        OCTK_WARNING() << "Failed to create structure " << ScalabilityModeToString(*scalability_mode);
        return {};
    }

    // TODO(bugs.webrtc.org/11607): Add support for screensharing.
    std::vector<SpatialLayer> spatial_layers = GetSvcConfig(codec.width,
                                                            codec.height,
                                                            codec.maxFramerate,
                                                            ...
                                                            codec.GetScalabilityMode() ? info : utils::nullopt);
    OCTK_DCHECK(!spatial_layers.empty());
...
std::vector<SpatialLayer> GetSvcConfig(size_t input_width,
                                       ...
                                       Optional<ScalableVideoController::StreamLayersConfig> config)
{
    OCTK_DCHECK_GT(input_width, 0);
    OCTK_DCHECK_GT(input_height, 0);
    OCTK_DCHECK_GT(num_spatial_layers, 0);
    OCTK_DCHECK_GT(num_temporal_layers, 0);

    if (is_screen_sharing)
    {
        return ConfigureSvcScreenSharing(input_width, input_height, max_framerate_fps, num_spatial_layers);
    }
    else
    {
        return ConfigureSvcNormalVideo(input_width,
                                       input_height,
                                       max_framerate_fps,
                                       first_active_layer,
                                       num_spatial_layers,
