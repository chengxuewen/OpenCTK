...
int GetLimitedNumSpatialLayers(int width, int height)
{
    ...
    const int min_width = is_landscape ? kMinAv1SpatialLayerLongSideLength : kMinAv1SpatialLayerShortSideLength;
    const int min_height = is_landscape ? kMinAv1SpatialLayerShortSideLength : kMinAv1SpatialLayerLongSideLength;
...
Optional<ScalabilityMode> BuildScalabilityMode(int num_temporal_layers, int num_spatial_layers)
{
    std::string name;
    name += "L" + std::to_string(num_spatial_layers) + "T" + std::to_string(num_temporal_layers);
...
std::vector<ScalabilityMode> LibaomAv1EncoderSupportedScalabilityModes()
{
    ...
    {
        if (ScalabilityStructureConfig(scalability_mode) != utils::nullopt)
        {
...
bool LibaomAv1EncoderSupportsScalabilityMode(ScalabilityMode scalability_mode)
{
    ...
    return ScalabilityStructureConfig(scalability_mode) != utils::nullopt;
}

bool SetAv1SvcConfig(VideoCodec &video_codec, int num_temporal_layers, int num_spatial_layers)
{
    OCTK_DCHECK_EQ(video_codec.codecType, kVideoCodecAV1);

    Optional<ScalabilityMode> scalability_mode = video_codec.GetScalabilityMode();
    if (!scalability_mode.has_value())
    {
        scalability_mode = BuildScalabilityMode(num_temporal_layers, num_spatial_layers);
        if (!scalability_mode)
        {
            OCTK_WARNING() << "Scalability mode is not set, using 'L1T1'.";
    ...
    ScalabilityMode reduced = LimitNumSpatialLayers(*scalability_mode,
                                                    GetLimitedNumSpatialLayers(video_codec.width, video_codec.height));
    if (*scalability_mode != reduced)
    {
        OCTK_WARNING() << "Reduced number of spatial layers from " << ScalabilityModeToString(*scalability_mode)
                       << " to " << ScalabilityModeToString(reduced);
        scalability_mode = reduced;
    }

    std::unique_ptr<ScalableVideoController> structure = CreateScalabilityStructure(*scalability_mode);
    if (structure == nullptr)
    {
        OCTK_WARNING() << "Failed to create structure " << static_cast<int>(*scalability_mode);
        return false;
    }
    ...
    for (int sl_idx = 0; sl_idx < info.num_spatial_layers; ++sl_idx)
    {
        ...
        spatial_layer.width = video_codec.width * info.scaling_factor_num[sl_idx] / info.scaling_factor_den[sl_idx];
        spatial_layer.height = video_codec.height * info.scaling_factor_num[sl_idx] / info.scaling_factor_den[sl_idx];
    ...
    if (requested_single_spatial_layer)
    {
        ...
        spatial_layer.targetBitrate = (video_codec.minBitrate + video_codec.maxBitrate) / 2;
        return true;
    }
    ...
    for (int sl_idx = 0; sl_idx < info.num_spatial_layers; ++sl_idx)
    {
        ...
        int min_bitrate_kbps = (480.0 * std::sqrt(num_pixels) - 95000.0) / 1000.0;
