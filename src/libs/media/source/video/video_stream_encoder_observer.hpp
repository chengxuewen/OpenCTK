...
#ifndef _OCTK_VIDEO_STREAM_ENCODER_OBSERVER_HPP
#define _OCTK_VIDEO_STREAM_ENCODER_OBSERVER_HPP

#include <openctk/media/video_adaptation_counters.hpp>
#include <openctk/media/video_bitrate_allocation.hpp>
#include <openctk/media/video_adaptation_reason.hpp>
#include <openctk/media/video_codec.hpp>
#include <openctk/media/video_encoder_config.hpp>
...
public:
    ...
    virtual void OnEncodedFrameTimeMeasured(int encode_duration_ms, int encode_usage_percent) = 0;
};
...
public:
    ...
    {
        AdaptationSettings()
            : resolution_scaling_enabled(false)
            , framerate_scaling_enabled(false)
        {
        }

        AdaptationSettings(bool resolution_scaling_enabled, bool framerate_scaling_enabled)
    ...
    virtual void OnSendEncodedImage(const EncodedImage &encoded_image, const CodecSpecificInfo *codec_info) = 0;

    virtual void OnEncoderImplementationChanged(EncoderImplementation implementation) = 0;
    ...
    virtual void OnAdaptationChanged(VideoAdaptationReason reason,
                                     const VideoAdaptationCounters &cpu_steps,
                                     const VideoAdaptationCounters &quality_steps) = 0;
    virtual void ClearAdaptationStats() = 0;

    virtual void UpdateAdaptationSettings(AdaptationSettings cpu_settings, AdaptationSettings quality_settings) = 0;
    ...
    virtual void OnBitrateAllocationUpdated(const VideoCodec &codec, const VideoBitrateAllocation &allocation) { }
...
#endif // _OCTK_VIDEO_STREAM_ENCODER_OBSERVER_HPP
