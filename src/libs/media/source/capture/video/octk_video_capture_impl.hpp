...
#ifndef _OCTK_VIDEO_CAPTURE_MAIN_SOURCE_VIDEO_CAPTURE_IMPL_H_
#define _OCTK_VIDEO_CAPTURE_MAIN_SOURCE_VIDEO_CAPTURE_IMPL_H_
...
class RTC_EXPORT VideoCaptureImpl : public VideoCaptureModule {
 ...
 protected:
  ...
  char* _deviceUniqueId OCTK_ATTRIBUTE_GUARDED_BY(api_checker_);
  Mutex api_lock_;
  // Should be set by platform dependent code in StartCapture.
  VideoCaptureCapability _requestedCapability OCTK_ATTRIBUTE_GUARDED_BY(api_checker_);
 ...
 private:
  ...
  int64_t _lastProcessTimeNanos OCTK_ATTRIBUTE_GUARDED_BY(capture_checker_);
  // last time the frame rate callback function was called.
  int64_t _lastFrameRateCallbackTimeNanos OCTK_ATTRIBUTE_GUARDED_BY(capture_checker_);

  rtc::VideoSinkInterface<VideoFrame>* _dataCallBack OCTK_ATTRIBUTE_GUARDED_BY(api_lock_);
  RawVideoSinkInterface* _rawDataCallBack OCTK_ATTRIBUTE_GUARDED_BY(api_lock_);

  int64_t _lastProcessFrameTimeNanos OCTK_ATTRIBUTE_GUARDED_BY(capture_checker_);
  // timestamp for local captured frames
  int64_t _incomingFrameTimesNanos[kFrameRateCountHistorySize] OCTK_ATTRIBUTE_GUARDED_BY(
      capture_checker_);
  // Set if the frame should be rotated by the capture module.
  VideoRotation _rotateFrame OCTK_ATTRIBUTE_GUARDED_BY(api_lock_);

  // Indicate whether rotation should be applied before delivered externally.
  bool apply_rotation_ OCTK_ATTRIBUTE_GUARDED_BY(api_lock_);
};
}  // namespace videocapturemodule
}  // namespace webrtc
#endif  // _OCTK_VIDEO_CAPTURE_MAIN_SOURCE_VIDEO_CAPTURE_IMPL_H_
