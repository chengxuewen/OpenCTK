...
class TestVideoCaptureCallback
 ...
 public:
  ...
  void OnFrame(const webrtc::VideoFrame& videoFrame) override {
    webrtc::Mutex::Locker locker(&capture_lock_);
 ...
 private:
  ...
  std::shared_ptr<webrtc::VideoFrameBuffer> last_frame_;
...
class VideoCaptureTest : public ::testing::Test {
 public:
  ...
  std::shared_ptr<VideoCaptureModule> OpenVideoCaptureDevice(
      unsigned int device,
    ...
    std::shared_ptr<VideoCaptureModule> module(
...
TEST_F(VideoCaptureTest, DISABLED_TestTwoCameras) {
  ...
  std::shared_ptr<VideoCaptureModule> module1(
  ...
  std::shared_ptr<VideoCaptureModule> module2(
