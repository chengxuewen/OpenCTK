...
void PipeWireNode::OnNodeInfo(void* data, const pw_node_info* info) {
  ...
  if (info->change_mask & PW_NODE_CHANGE_MASK_PROPS) {
    ...
    vid = vid_str ? rtc::stringToNumber<int>(vid_str) : utils::nullopt;
    pid = pid_str ? rtc::stringToNumber<int>(pid_str) : utils::nullopt;
...
void PipeWireSession::init(VideoCaptureOptions::Callback* callback, int fd) {
  {
    webrtc::Mutex::Locker locker(&callback_lock_);
    callback_ = callback;
  }
  ...
  if (fd != kInvalidPipeWireFd) {
    ...
    portal_notifier_ = utils::make_unique<CameraPortalNotifier>(this);
    portal_ = utils::make_unique<CameraPortal>(portal_notifier_.get());
    portal_->start();
  }
}
