...
class CameraPortalPrivate {
 ...
 private:
  ...
  CameraPortal::PortalNotifier* notifier_ OCTK_ATTRIBUTE_GUARDED_BY(&notifier_lock_) =
      nullptr;
...
CameraPortalPrivate::~CameraPortalPrivate() {
  {
    webrtc::Mutex::Locker locker(&notifier_lock_);
    notifier_ = nullptr;
  }
...
CameraPortal::CameraPortal(PortalNotifier* notifier)
    : private_(utils::make_unique<CameraPortalPrivate>(notifier)) {}
