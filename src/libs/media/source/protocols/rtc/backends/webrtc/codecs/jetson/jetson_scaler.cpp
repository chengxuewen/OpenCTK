...
std::unique_ptr<JetsonScaler> JetsonScaler::Create(int src_width, int src_height, int dst_width, int dst_height)
{
...
JetsonScaler::~JetsonScaler()
{
    ...
    CaptureTask task;
    while (capturing_tasks_.try_dequeue(task))
    {
        // Return the unused dma fd back to free buffers
        free_buffers_.enqueue(task.dst_dma_fd);
    }

    int fd = -1;
    while (free_buffers_.try_dequeue(fd))
    {
        int ret = NvBufSurf::NvDestroy(fd);
        if (ret < 0)
        {
            OCTK_LOGGING_ERROR(WEBRTC_LOGGER(), "codecs-jetson:Failed to Destroy NvBuffer");
        }
    }

    OCTK_LOGGING_DEBUG(WEBRTC_LOGGER(), "codecs-jetson:~JetsonScaler");
}
...
{
    worker_ = std::make_unique<Worker>("NvTransform", [this]() { CaptureBuffer(); });
    worker_->Run();
}

void JetsonScaler::EmplaceBuffer(V4L2FrameBufferRef frame_buffer, std::function<void(V4L2FrameBufferRef)> on_capture)
{
    if (abort_)
    {
        return;
    ...
    int dst_dma_fd = -1;
    if (free_buffers_.try_dequeue(dst_dma_fd))
    {
        return;
    }
    ...
    if (ret < 0)
    {
        OCTK_LOGGING_ERROR(WEBRTC_LOGGER(),
                           "codecs-jetson:NvTransform failed to tranform from fd(%d) to fd(%d)",
                           frame_buffer->GetDmaFd(),
                           dst_dma_fd);
        free_buffers_.enqueue(dst_dma_fd);
        return;
    }
    ...
    capturing_tasks_.enqueue(std::move(task));
}

void JetsonScaler::CaptureBuffer()
{
    CaptureTask task;
    if (capturing_tasks_.try_dequeue(task))
    {
        task.callback();
    }
}
