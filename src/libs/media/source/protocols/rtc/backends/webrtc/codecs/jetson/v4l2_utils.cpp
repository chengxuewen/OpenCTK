...
int V4L2Util::OpenDevice(const char *file)
{
    ...
    OCTK_LOGGING_DEBUG(WEBRTC_LOGGER(), "codecs-jetson:Successfully opened file %s (fd: %d)", file, fd);
    return fd;
}
...
{
    close(fd);
    OCTK_LOGGING_DEBUG(WEBRTC_LOGGER(), "codecs-jetson:fd(%d) is closed!", fd);
}
...
bool V4L2Util::QueryCapabilities(int fd, v4l2_capability *cap)
{
    ...
    {
        OCTK_LOGGING_ERROR(WEBRTC_LOGGER(), "codecs-jetson:fd(%d) query capabilities: %s", fd, strerror(errno));
        return false;
    }
    return true;
}
...
bool V4L2Util::InitBuffer(int fd, V4L2BufferGroup *gbuffer, v4l2_buf_type type, v4l2_memory memory, bool has_dmafd)
{
    ...
    OCTK_LOGGING_DEBUG(WEBRTC_LOGGER())
        .format("codecs-jetson:fd({}) driver '{}' on card '{}' in {} mode",
                fd,
                cap.driver,
                cap.card,
...
bool V4L2Util::DequeueBuffer(int fd, v4l2_buffer *buffer)
{
    ...
    {
        OCTK_LOGGING_ERROR(WEBRTC_LOGGER(), "codecs-jetson:fd(%d) dequeue buffer: %s", fd, strerror(errno));
        return false;
    }
    return true;
}
...
bool V4L2Util::QueueBuffer(int fd, v4l2_buffer *buffer)
{
    if (ioctl(fd, VIDIOC_QBUF, buffer) < 0)
    {
        OCTK_LOGGING_ERROR(WEBRTC_LOGGER(),
                           "codecs-jetson:fd(%d) queue buffer(%u): %s\n",
                           fd,
                           buffer->type,
                           strerror(errno));
        return false;
...
bool V4L2Util::SubscribeEvent(int fd, uint32_t type)
{
    ...
    {
        OCTK_LOGGING_ERROR(WEBRTC_LOGGER(),
                           "codecs-jetson:fd(%d) does not support VIDIOC_SUBSCRIBE_EVENT(%d)",
                           fd,
                           type);
        return false;
    }
...
bool V4L2Util::SetFps(int fd, v4l2_buf_type type, uint32_t fps)
{
    ...
    {
        OCTK_LOGGING_ERROR(WEBRTC_LOGGER(), "codecs-jetson:fd(%d) set fps(%d): %s", fd, fps, strerror(errno));
        return false;
    }
    return true;
}

bool V4L2Util::SetFormat(int fd, V4L2BufferGroup *gbuffer, uint32_t width, uint32_t height, uint32_t &pixel_format)
{
    v4l2_format fmt = {};
    ...
    OCTK_LOGGING_DEBUG(WEBRTC_LOGGER(),
                       "codecs-jetson:fd(%d) original formats: %s(%dx%d)",
                       gbuffer->fd,
    ...
    {
        OCTK_LOGGING_ERROR(WEBRTC_LOGGER(),
                           "codecs-jetson:fd(%d) set format(%s) : %s",
                           fd,
                           V4L2Util::FourccToString(fmt.fmt.pix_mp.pixelformat).c_str(),
                           strerror(errno));
        return false;
    ...
    OCTK_LOGGING_DEBUG(WEBRTC_LOGGER(),
                       "codecs-jetson:fd(%d) latest format: %s(%dx%d)",
                       gbuffer->fd,
                       V4L2Util::FourccToString(fmt.fmt.pix_mp.pixelformat).c_str(),
                       fmt.fmt.pix_mp.width,
                       fmt.fmt.pix_mp.height);
    ...
    {
        OCTK_LOGGING_ERROR(WEBRTC_LOGGER(),
                           "codecs-jetson:fd(%d) input size (%dx%d) doesn't match driver's output size (%dx%d): %s",
                           fd,
                           width,
                           height,
                           fmt.fmt.pix_mp.width,
...
bool V4L2Util::SetCtrl(int fd, uint32_t id, int32_t value)
{
    ...
    {
        OCTK_LOGGING_ERROR(WEBRTC_LOGGER(), "codecs-jetson:fd(%d) set ctrl(%d): %s", fd, id, strerror(errno));
        return false;
    }
    return true;
}
...
bool V4L2Util::SetExtCtrl(int fd, uint32_t id, int32_t value)
{
    ...
    {
        OCTK_LOGGING_ERROR(WEBRTC_LOGGER(), "codecs-jetson:fd(%d) set ext ctrl(%d): %s", fd, id, strerror(errno));
        return false;
    }
    return true;
}
...
bool V4L2Util::StreamOn(int fd, v4l2_buf_type type)
{
    ...
    {
        OCTK_LOGGING_ERROR(WEBRTC_LOGGER(), "codecs-jetson:fd(%d) turn on stream: %s", fd, strerror(errno));
        return false;
    }
    return true;
}
...
bool V4L2Util::StreamOff(int fd, v4l2_buf_type type)
{
    ...
    {
        OCTK_LOGGING_ERROR(WEBRTC_LOGGER(), "codecs-jetson:fd(%d) turn off stream: %s", fd, strerror(errno));
        return false;
    }
    return true;
}
...
void V4L2Util::UnMap(V4L2BufferGroup *gbuffer)
{
    for (int i = 0; i < gbuffer->num_buffers; i++)
    {
        ...
        {
            OCTK_LOGGING_DEBUG(WEBRTC_LOGGER(), "codecs-jetson:close (%d) dmafd", gbuffer->buffers[i].dmafd);
            close(gbuffer->buffers[i].dmafd);
        }
        if (gbuffer->buffers[i].start != nullptr)
        {
            OCTK_LOGGING_DEBUG(WEBRTC_LOGGER(), "codecs-jetson:unmapped (%d) buffers", gbuffer->fd);
...
bool V4L2Util::MMap(int fd, V4L2BufferGroup *gbuffer)
{
    for (int i = 0; i < gbuffer->num_buffers; i++)
    {
        ...
        {
            OCTK_LOGGING_ERROR(WEBRTC_LOGGER(), "codecs-jetson:fd(%d) query buffer: %s", fd, strerror(errno));
            return false;
        }
        ...
        if (gbuffer->has_dmafd)
        {
            ...
            {
                OCTK_LOGGING_ERROR(WEBRTC_LOGGER(), "codecs-jetson:fd(%d) export buffer: %s", fd, strerror(errno));
                return false;
            }
            buffer->dmafd = expbuf.fd;
            OCTK_LOGGING_DEBUG(WEBRTC_LOGGER(),
                               "codecs-jetson:fd(%d) export dma at fd(%d)",
                               gbuffer->fd,
                               buffer->dmafd);
        }
        ...
        OCTK_LOGGING_DEBUG(WEBRTC_LOGGER(),
                           "codecs-jetson:fd(%d) query buffer at %p (length: %d)",
                           gbuffer->fd,
                           buffer->start,
                           buffer->length);
    }
...
bool V4L2Util::AllocateBuffer(int fd, V4L2BufferGroup *gbuffer, int num_buffers)
{
    ...
    {
        OCTK_LOGGING_ERROR(WEBRTC_LOGGER(), "codecs-jetson:fd(%d) request buffer: %s", fd, strerror(errno));
        return false;
    }
