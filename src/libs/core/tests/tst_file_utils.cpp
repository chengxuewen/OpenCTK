...
std::string Path(StringView path)
{
...
void CleanDir(StringView dir, size_t *num_deleted_entries)
{
    OCTK_DCHECK(num_deleted_entries);
    *num_deleted_entries = 0;
    Optional<std::vector<std::string>> dir_content = ReadDirectory(dir);
...
void WriteStringInFile(StringView what, StringView file_path)
{
...
protected:
    ...
    {
        original_working_dir_ = utils::WorkingDir();
    }
...
TEST_F(FileUtilsTest, OutputPathFromUnchangedWorkingDir)
{
    ...
    std::string result = utils::OutputPath();
...
TEST_F(FileUtilsTest, RandomOutputPathFromUnchangedWorkingDir)
{
    ...
    std::string result = utils::OutputPathWithRandomDirectory();
...
{
    std::string temp_filename = utils::TempFilename(utils::OutputPath(), "TempFilenameTest");
    ASSERT_TRUE(utils::FileExists(temp_filename)) << "Couldn't find file: " << temp_filename;
    remove(temp_filename.c_str());
}

TEST_F(FileUtilsTest, GenerateTempFilename)
{
    std::string temp_filename = utils::GenerateTempFilename(
        utils::OutputPath(), "TempFilenameTest");
    ASSERT_FALSE(utils::FileExists(temp_filename))
...
{
    std::string directory = test::OutputPathWithRandomDirectory() + "fileutils-unittest-empty-dir";
// Make sure it's removed if a previous test has failed:
    remove(directory.c_str());
    ASSERT_TRUE(utils::CreateDir(directory));
    remove(directory.c_str());
}
...
{
    ASSERT_EQ(0, chdir(kPathDelimiter.data()));
    std::string resource = utils::ResourcePath("whatever", "ext");
#if !defined(OCTK_OS_IOS)
...
// Create a file with some dummy data in.
    std::string temp_filename = utils::TempFilename(
        utils::OutputPath(), "fileutils_unittest");
    FILE *file = fopen(temp_filename.c_str(), "wb");
    ASSERT_TRUE(file != NULL) << "Failed to open file: " << temp_filename;
    ASSERT_GT(fprintf(file, "%s", "Dummy data"), 0)
                        << "Failed to write to file: " << temp_filename;
    fclose(file);
    ASSERT_GT(utils::GetFileSize(temp_filename), 0u);
    remove(temp_filename.c_str());
}

TEST_F(FileUtilsTest, GetFileSizeNonExistingFile)
{
    ASSERT_EQ(0u, utils::GetFileSize("non-existing-file.tmp"));
}

TEST_F(FileUtilsTest, DirExists)
{
// Check that an existing directory is recognized as such.
    ASSERT_TRUE(utils::DirExists(utils::OutputPath()))
...
// Checks.
    Optional<std::vector<std::string>> dir_content =
