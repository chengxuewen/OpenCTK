...
void AddSparseSample(StringView name, int sample)
{
    OCTK_HISTOGRAM_COUNTS_SPARSE_100(name, sample);
}
void AddSampleWithVaryingName(int index, StringView name, int sample)
{
...
TEST_F(MetricsTest, RtcHistogramPercent_AddSample)
{
    ...
    OCTK_HISTOGRAM_PERCENTAGE(kName, kSample);
...
TEST_F(MetricsTest, RtcHistogramEnumeration_AddSample)
{
    ...
    OCTK_HISTOGRAM_ENUMERATION(kName, kSample, kSample + 1);
    EXPECT_EQ(1, metrics::NumSamples(kName));
    EXPECT_EQ(1, metrics::NumEvents(kName, kSample));
    EXPECT_THAT(metrics::Samples(kName), ElementsAre(Pair(kSample, 1)));
}
...
TEST_F(MetricsTest, RtcHistogramBoolean_AddSample)
{
    ...
    const int kSample = 0;
    OCTK_HISTOGRAM_BOOLEAN(kName, kSample);
    EXPECT_EQ(1, metrics::NumSamples(kName));
    EXPECT_EQ(1, metrics::NumEvents(kName, kSample));
    EXPECT_THAT(metrics::Samples(kName), ElementsAre(Pair(kSample, 1)));
}
...
TEST_F(MetricsTest, RtcHistogramCountsSparse_AddSample)
{
    ...
    OCTK_HISTOGRAM_COUNTS_SPARSE_100(kName, kSample);
    EXPECT_EQ(1, metrics::NumSamples(kName));
    EXPECT_EQ(1, metrics::NumEvents(kName, kSample));
    EXPECT_THAT(metrics::Samples(kName), ElementsAre(Pair(kSample, 1)));
}
...
TEST_F(MetricsTest, RtcHistogramCounts_AddSample)
{
    ...
    OCTK_HISTOGRAM_COUNTS_100(kName, kSample);
    EXPECT_EQ(1, metrics::NumSamples(kName));
    EXPECT_EQ(1, metrics::NumEvents(kName, kSample));
    EXPECT_THAT(metrics::Samples(kName), ElementsAre(Pair(kSample, 1)));
}
...
TEST_F(MetricsTest, RtcHistogramCounts_AddMultipleSamples)
{
    ...
    {
        OCTK_HISTOGRAM_COUNTS_200(kName, i);
...
{
    EXPECT_DEATH(OCTK_HISTOGRAMS_COUNTS_1000(-1, "Name", kSample), "");
