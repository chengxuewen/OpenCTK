...
{
    std::unique_ptr<char, AlignedFreeDeleter> scoped(static_cast<char *>(utils::alignedMalloc(size, alignment)));
    if (scoped.get() == NULL)
    {
        return false;
    }
...
TEST(AlignedMalloc, GetRightAlign)
{
    ...
    const char *misaligned_ptr = reinterpret_cast<const char *>(misaligned_address);
    const char *realigned_ptr = utils::getRightAlign(misaligned_ptr, alignment);
