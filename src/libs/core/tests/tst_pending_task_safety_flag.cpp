...
TEST(PendingTaskSafetyFlagTest, PendingTaskSuccess)
{
    ...
    {
    public:
        Owner() : tq_main_(TaskQueue::Current()) { OCTK_DCHECK(tq_main_); }
        ~Owner()
        {
...
TEST(PendingTaskSafetyFlagTest, PendingTaskDropped)
{
    ...
    {
    public:
        explicit Owner(bool *stuff_done) : tq_main_(TaskQueue::Current()), stuff_done_(stuff_done)
        {
            OCTK_DCHECK(tq_main_);
            *stuff_done_ = false;
        }
        ~Owner() { OCTK_DCHECK(tq_main_->IsCurrent()); }

        void DoStuff()
        {
            OCTK_DCHECK(!tq_main_->IsCurrent());
            tq_main_->PostTask(SafeTask(safety_.flag(), [this]() { *stuff_done_ = true; }));
        }
    ...
    tq1.SendTask([&owner, &stuff_done]() {
        owner = utils::make_unique<Owner>(&stuff_done);
    });
    ASSERT_TRUE(owner);
