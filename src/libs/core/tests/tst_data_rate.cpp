...
TEST(DataRateTest, ConstExpr)
{
    ...
    OCTK_CXX14_CONSTEXPR DataRate kDataRateBps = DataRate::BitsPerSec(kValue);
    OCTK_CXX14_CONSTEXPR DataRate kDataRateKbps = DataRate::KilobitsPerSec(kValue);
    OCTK_CXX14_CONSTEXPR_ASSERT(kDataRateBps.bps<double>() == kValue);
    OCTK_CXX14_CONSTEXPR_ASSERT(kDataRateBps.bps_or(0) == kValue);
    OCTK_CXX14_CONSTEXPR_ASSERT(kDataRateKbps.kbps_or(0) == kValue);
}
...
#endif  // GTEST_HAS_DEATH_TEST && !!defined(OCTK_OS_ANDROID) && OCTK_DCHECK_IS_ON
}
}  // namespace test
