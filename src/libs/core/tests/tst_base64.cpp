                      ...
                      size_t szdest)
{
    ...
    EXPECT_TRUE(Base64::DecodeFromArray(src, szsrc, Base64::DO_LAX, &unescaped, nullptr));
...
TEST(Base64, EncodeDecodeBattery)
{
    ...
    for (i = 0; i < sizeof(base64_tests) / sizeof(base64_tests[0]); ++i)
    {
        ...
        OCTK_TRACE() << "B64: " << base64_tests[i].cyphertext;

        const unsigned char *unsigned_plaintext = reinterpret_cast<const unsigned char *>(base64_tests[i].plaintext);
        ...
        memset(encode_buffer, 0, sizeof(encode_buffer));
        encode_length = Base64Escape(unsigned_plaintext, base64_tests[i].plain_length, encode_buffer, cypher_length);
        ...
        decode_length = Base64Unescape(encode_buffer, cypher_length, decode_buffer, sizeof(decode_buffer));
        ...
        if (first_equals)
        {
            ...
            *first_equals = '\0';
            EXPECT_NE(0U, Base64Unescape(encode_buffer, first_equals - encode_buffer, &decoded2));
...
{
    OCTK_TRACE() << "Testing specific base64 file";

    char unescaped[64 * 1024];

// unescape that massive blob above
    size_t size = Base64Unescape(SpecificTest, sizeof(SpecificTest), unescaped, sizeof(unescaped));
                ...
                Base64::DecodeFlags flags)
{
    ...
    bool success = Base64::DecodeFromArray(encoded, encoded_len, flags, &result, &consumed);
    size_t unparsed = encoded_len - consumed;
    EXPECT_EQ(expect_unparsed, unparsed) << "\"" << encoded << "\" -> \"" << decoded << "\"";
