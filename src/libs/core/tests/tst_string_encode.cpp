...
public:
    ...
    char data_[10];
    StringView data_view_{data_, sizeof(data_)};
    char decoded_[11];
    size_t dec_res_;
};
...
{
    std::string encoded = utils::hex_encode(data_view_);
    EXPECT_EQ("80818283848586878889", encoded);
    dec_res_ = utils::hex_decode(ArrayView<char>(decoded_), encoded);
...
{
    std::string encoded = utils::hex_encode_with_delimiter(data_view_, ':');
    EXPECT_EQ("80:81:82:83:84:85:86:87:88:89", encoded);
    dec_res_ = utils::hex_decode_with_delimiter(ArrayView<char>(decoded_), encoded, ':');
    ASSERT_EQ(sizeof(data_), dec_res_);
    ASSERT_EQ(0, memcmp(data_, decoded_, dec_res_));
}
...
{
    std::string encoded = utils::hex_encode("");
...
{
    std::string encoded = utils::hex_encode_with_delimiter("", ':');
    EXPECT_TRUE(encoded.empty());
    dec_res_ = utils::hex_decode_with_delimiter(ArrayView<char>(decoded_), encoded, ':');
    ASSERT_EQ(0U, dec_res_);
}
...
{
    dec_res_ = utils::hex_decode_with_delimiter(ArrayView<char>(decoded_, 4), "0123456789", 0);
    ASSERT_EQ(0U, dec_res_);
...
{
    dec_res_ = utils::hex_decode_with_delimiter(ArrayView<char>(decoded_), "axyz", 0);
    ASSERT_EQ(0U, dec_res_);
}
...
{
    dec_res_ = utils::hex_decode_with_delimiter(ArrayView<char>(decoded_), "012", 0);
    ASSERT_EQ(0U, dec_res_);
}
...
{
    dec_res_ = utils::hex_decode_with_delimiter(ArrayView<char>(decoded_, 4), "01::23::45::67", ':');
    ASSERT_EQ(0U, dec_res_);
}
...
TEST(TokenizeTest, CountSubstrings)
{
    ...
    EXPECT_EQ(5ul, utils::tokenize("one two three four five", ' ', &fields));
    fields.clear();
    EXPECT_EQ(1ul, utils::tokenize("one", ' ', &fields));
    ...
    EXPECT_EQ(1ul, utils::tokenize("  one  ", ' ', &fields));
    fields.clear();
    EXPECT_EQ(0ul, utils::tokenize(" ", ' ', &fields));
}
...
TEST(TokenizeTest, CompareSubstrings)
{
    ...
    utils::tokenize("find middle one", ' ', &fields);
...
TEST(TokenizeFirstTest, NoLeadingSpaces)
{
    ...
    ASSERT_TRUE(utils::tokenize_first("A &*${}", ' ', &token, &rest));
    ASSERT_STREQ("A", token.c_str());
    ASSERT_STREQ("&*${}", rest.c_str());

    ASSERT_TRUE(utils::tokenize_first("A B& *${}", ' ', &token, &rest));
    ASSERT_STREQ("A", token.c_str());
...
TEST(TokenizeFirstTest, LeadingSpaces)
{
    ...
    ASSERT_TRUE(utils::tokenize_first("     A B C", ' ', &token, &rest));
...
TEST(TokenizeFirstTest, SingleToken)
{
    ...
    ASSERT_FALSE(utils::tokenize_first("ABC", ' ', &token, &rest));

    ASSERT_TRUE(utils::tokenize_first("ABC    ", ' ', &token, &rest));
...
{
    EXPECT_EQ(5ul, utils::split("one,two,three,four,five", ',').size());
    EXPECT_EQ(1ul, utils::split("one", ',').size());

    // Empty fields between commas count.
    EXPECT_EQ(5ul, utils::split("one,,three,four,five", ',').size());
    EXPECT_EQ(3ul, utils::split(",three,", ',').size());
    EXPECT_EQ(1ul, utils::split("", ',').size());
}

// Tests comparing substrings.
TEST(SplitTest, CompareSubstrings)
{
    std::vector<StringView> fields = utils::split("find,middle,one", ',');
    ASSERT_EQ(3ul, fields.size());
    ASSERT_EQ("middle", fields.at(1));

    // Empty fields between commas count.
    fields = utils::split("find,,middle,one", ',');
...
{
    std::vector<StringView> fields = utils::split("a.b.c", '.');
    ASSERT_EQ(3ul, fields.size());
    EXPECT_EQ("a", fields[0]);
    EXPECT_EQ("b", fields[1]);
    EXPECT_EQ("c", fields[2]);

    fields = utils::split("..c", '.');
    ASSERT_EQ(3ul, fields.size());
...
{
    EXPECT_EQ(utils::toString(true), "true");
    EXPECT_EQ(utils::toString(false), "false");

    const char *c = "message";
    EXPECT_EQ(utils::toString(c), c);
    EXPECT_EQ(utils::toString(std::string(c)), c);
    char nonconst_c[] = "message";
    EXPECT_EQ(utils::toString(nonconst_c), c);
    EXPECT_EQ(utils::toString(&nonconst_c[0]), c);

    EXPECT_EQ(utils::toString(short{-123}), "-123");
    EXPECT_EQ(utils::toString((unsigned short)123), "123");
    EXPECT_EQ(utils::toString(int{-123}), "-123");
    EXPECT_EQ(utils::toString((unsigned int)123), "123");
    EXPECT_EQ(utils::toString((long int)-123), "-123");
    EXPECT_EQ(utils::toString((unsigned long int)123), "123");
    EXPECT_EQ(utils::toString((long long int)-123), "-123");
    EXPECT_EQ(utils::toString((unsigned long long int)123), "123");
    EXPECT_EQ(utils::toString(0.5), "0.5");
    int i = 10;
    EXPECT_EQ(utils::StringFormat("%p", &i), utils::toString(&i));
}
...
{
    T value;
    EXPECT_TRUE(utils::FromString(s, &value));
    EXPECT_EQ(value, t);
}
...
{
    T value;
    EXPECT_FALSE(utils::FromString(s, &value)) << "[" << s << "]";
}
