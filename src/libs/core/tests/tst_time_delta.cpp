...
TEST(TimeDeltaTest, ConstExpr)
{
    ...
    OCTK_STATIC_ASSERT(TimeDelta() == kTimeDeltaZero);
    OCTK_STATIC_ASSERT_X(kTimeDeltaZero.IsZero(), "");
    OCTK_STATIC_ASSERT_X(kTimeDeltaPlusInf.IsPlusInfinity(), "");
    OCTK_STATIC_ASSERT_X(kTimeDeltaMinusInf.IsMinusInfinity(), "");
    OCTK_STATIC_ASSERT_X(kTimeDeltaPlusInf.ms_or(-1) == -1, "");

    OCTK_STATIC_ASSERT_X(kTimeDeltaPlusInf > kTimeDeltaZero, "");

    OCTK_CXX14_CONSTEXPR TimeDelta kTimeDeltaMinutes = TimeDelta::Minutes(kValue);
    OCTK_CXX14_CONSTEXPR TimeDelta kTimeDeltaSeconds = TimeDelta::Seconds(kValue);
    OCTK_CXX14_CONSTEXPR TimeDelta kTimeDeltaMs = TimeDelta::Millis(kValue);
    OCTK_CXX14_CONSTEXPR TimeDelta kTimeDeltaUs = TimeDelta::Micros(kValue);

    OCTK_CXX14_CONSTEXPR_ASSERT(kTimeDeltaMinutes.seconds_or(0) == kValue * 60);
    OCTK_CXX14_CONSTEXPR_ASSERT(kTimeDeltaSeconds.seconds_or(0) == kValue);
    OCTK_CXX14_CONSTEXPR_ASSERT(kTimeDeltaMs.ms_or(0) == kValue);
    OCTK_CXX14_CONSTEXPR_ASSERT(kTimeDeltaUs.us_or(0) == kValue);
}
