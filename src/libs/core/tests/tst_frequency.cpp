 ...
 */
#include <openctk/core/frequency.hpp>
...
TEST(FrequencyTest, ConstExpr)
{
    ...
    OCTK_STATIC_ASSERT(Frequency() == kFrequencyZero);
    OCTK_STATIC_ASSERT(kFrequencyZero.IsZero());
    OCTK_STATIC_ASSERT(kFrequencyPlusInf.IsPlusInfinity());
    OCTK_STATIC_ASSERT(kFrequencyMinusInf.IsMinusInfinity());

    OCTK_STATIC_ASSERT(kFrequencyPlusInf > kFrequencyZero);
}
