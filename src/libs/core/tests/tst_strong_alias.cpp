...
template <> int GetExampleValue<int>(int index) { return 5 + index; }
template <> uint64_t GetExampleValue<uint64_t>(int index) { return 500U + index; }

template <> std::string GetExampleValue<std::string>(int index) { return std::string('a', index); }
...
TYPED_TEST(StrongAliasTest, ValueAccessesUnderlyingValue)
{
    ...
    static_assert(std::is_const<typename std::remove_reference<decltype(const_alias.value())>::type>::value,
...
TYPED_TEST(StrongAliasTest, CanBeConstructedFromMoveOnlyType)
{
    ...
    FooAlias a(utils::make_unique<TypeParam>(GetExampleValue<TypeParam>(0)));
    EXPECT_EQ(*a.value(), GetExampleValue<TypeParam>(0));

    auto bare_value = utils::make_unique<TypeParam>(GetExampleValue<TypeParam>(1));
...
TYPED_TEST(StrongAliasTest, MutableOperatorArrow)
{
    ...
    FooAlias a(utils::make_unique<TypeParam>());
    EXPECT_TRUE(a.value());
...
TYPED_TEST(StrongAliasTest, MutableOperatorStar)
{
    ...
    FooAlias a(utils::make_unique<TypeParam>());
    FooAlias b(utils::make_unique<TypeParam>());
    EXPECT_TRUE(*a);
    EXPECT_TRUE(*b);
...
TYPED_TEST(StrongAliasTest, IsDefaultConstructible)
{
    ...
    static_assert(std::is_trivially_default_constructible<FooAlias>::value ==
