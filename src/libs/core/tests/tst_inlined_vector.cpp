...
using IntVec = InlinedVector<int, 8>;
...
using RefCountedVec = InlinedVector<RefCounted, 8>;
...
using DynamicVec = InlinedVector<Dynamic, 8>;
...
{
    InlinedVector<NoDefaultCtor, 1> v(10, NoDefaultCtor(2));
    (void)v;
}
// TEST(InlinedVectorTest, NoCopy) //TODO:not support
// {
//     InlinedVector<NoCopy, 1> v(10);
//     (void)v;
// }
// TEST(InlinedVectorTest, NoAssign) //TODO:not support
// {
//     InlinedVector<NoAssign, 1> v(10);
//     (void)v;
// }
TEST(InlinedVectorTest, MoveOnly)
{
    InlinedVector<MoveOnly, 2> v;
...
TEST(InlinedVectorTest, Noexcept)
{
    ...
    EXPECT_TRUE((std::is_nothrow_move_constructible<
        InlinedVector<MoveOnly, 2>>::value));
    ...
    EXPECT_EQ(absl::default_allocator_is_nothrow::value,
              (std::is_nothrow_move_constructible<InlinedVector<MoveCanThrow, 2>>::value));
}

TEST(InlinedVectorTest, EmplaceBack)
{
#if OCTK_CC_CPP17_OR_GREATER
    InlinedVector<std::pair<std::string, int>, 1> v;
...
TEST(InlinedVectorTest, ShrinkToFitEdgeCases)
{
    ...
    {
        InlinedVector<std::string, 2> v(100);
        v.resize(0);
        v.shrink_to_fit();
...
public:
    ...
    {
        p_ = utils::make_unique<int>(*other.p_);
        return *this;
    }
...
TEST(AliasingTest, Emplace)
{
    for (int i = 2; i < 20; ++i)
    {
        InlinedVector<NotTriviallyDestructible, 10> vec;
        for (int j = 0; j < i; ++j)
        {
...
TYPED_TEST_P(InstanceTest, Swap)
{
    ...
    using InstanceVec = InlinedVector<Instance, 8>;
    for (int l1 = 0; l1 < 20; l1++)
    {
        SCOPED_TRACE(l1);
        for (int l2 = 0; l2 < 20; l2++)
        {
            ...
            else if (a.size() <= inlined_capacity && b.size() <= inlined_capacity)
            {
                ...
                // EXPECT_EQ((tracker.moves() ? tracker.moves() : tracker.copies()), max_len - min_len);
            }
            else
            {
                // TODO::FIX
                ...
                // EXPECT_EQ((tracker.moves() ? tracker.moves() : tracker.copies()), min_len);
            }
...
TYPED_TEST_P(InstanceTest, CountConstructorsDestructors)
{
    ...
    for (int len = 0; len < 20; len++)
    {
        ...
        EXPECT_GE(tracker.copies() + tracker.moves(), len);  // More due to reallocation.
        tracker.ResetCopiesMovesSwaps();
...
TEST(CountElemAssign, SimpleTypeWithInlineBacking)
{
    for (size_t original_size = 0; original_size <= 5; ++original_size)
    {
        ...
        InlinedVector<int, 2> v(original_contents.begin(),
...
TYPED_TEST_P(InstanceTest, CountElemAssignInlineBacking)
{
    ...
    for (size_t original_size = 0; original_size <= 5; ++original_size)
    {
        ...
        InlinedVector<Instance, 2> v(original_contents.begin(),
                                     original_contents.end());
...
TEST(RangedConstructor, SimpleType)
{
    ...
    InlinedVector<int, 4> v(source_v.begin(), source_v.end());
    EXPECT_EQ(3, v.size());
    ...
    InlinedVector<int, 2> realloc_v(source_v.begin(), source_v.end());
...
void InstanceRangedConstructorTestForContainer()
{
    ...
    tracker.ResetCopiesMovesSwaps();
    InlinedVector<Instance, inlined_capacity> v(source_v.begin(),
                                                source_v.end());
    EXPECT_EQ(2, v.size());
...
TEST(RangedConstructor, ElementsAreConstructed)
{
    ...
    InlinedVector<std::string, 1> v(source_v.begin(), source_v.end());
    EXPECT_EQ("cat", v[0]);
    EXPECT_EQ("dog", v[1]);
}
...
TEST(RangedAssign, SimpleType)
{
    ...
    for (size_t original_size = 0; original_size <= 5; ++original_size)
    {
        ...
        for (size_t target_size = 0; target_size <= 5; ++target_size)
        {
            ...
            InlinedVector<int, 3> v(original_contents.begin(),
                                    original_contents.end());
...
void InstanceRangedAssignTestForContainer()
{
    ...
    for (size_t original_size = 0; original_size <= 5; ++original_size)
    {
        ...
        for (size_t target_size = 0; target_size <= 5; ++target_size)
        {
            ...
            InlinedVector<Instance, 3> v(original_contents.begin(),
                                         original_contents.end());
            v.assign(new_contents.begin(), new_contents.end());
...
{
    EXPECT_THAT((InlinedVector<int, 4>{4, 5, 6}),
                AllOf(SizeIs(3), CapacityIs(4), ElementsAre(4, 5, 6)));
}

TEST(InitializerListConstructor, SimpleTypeWithReallocationRequired)
{
    EXPECT_THAT((InlinedVector<int, 2>{4, 5, 6}),
                AllOf(SizeIs(3), CapacityIs(Gt(2)), ElementsAre(4, 5, 6)));
}

TEST(InitializerListConstructor, DisparateTypesInList)
{
    EXPECT_THAT((InlinedVector<int, 2>{-7, 8ULL}), ElementsAre(-7, 8));

    EXPECT_THAT((InlinedVector<std::string, 2>{"foo", std::string("bar")}),
                ElementsAre("foo", "bar"));
}

TEST(InitializerListConstructor, ComplexTypeWithInlineBacking)
{
    EXPECT_THAT((InlinedVector<CopyableMovableInstance, 1>{
...
{
    EXPECT_THAT(
        (InlinedVector<CopyableMovableInstance, 1>{
...
TEST(InitializerListAssign, SimpleTypeFitsInlineBacking)
{
    for (size_t original_size = 0; original_size <= 4; ++original_size)
    {
        ...
        InlinedVector<int, 2> v1(original_size, 12345);
        const size_t original_capacity_v1 = v1.capacity();
        v1.assign({3});
        EXPECT_THAT(v1, AllOf(SizeIs(1), CapacityIs(original_capacity_v1), ElementsAre(3)));

        InlinedVector<int, 2> v2(original_size, 12345);
        const size_t original_capacity_v2 = v2.capacity();
        v2 = {3};
        EXPECT_THAT(v2, AllOf(SizeIs(1), CapacityIs(original_capacity_v2), ElementsAre(3)));
    }
}
...
TYPED_TEST_P(InstanceTest, InitializerListAssign)
{
    ...
    {
        SCOPED_TRACE(original_size);
        InlinedVector<Instance, 2> v(original_size, Instance(12345));
...
TEST(AllocatorSupportTest, Constructors)
{
    ...
    using AllocVec = InlinedVector<int, 4, MyAlloc>;
    const int ia[] = {0, 1, 2, 3, 4, 5, 6, 7};
    int64_t allocated = 0;
    MyAlloc alloc(&allocated);
    {
        AllocVec OCTK_ATTRIBUTE_UNUSED v;
    }
    {
        AllocVec OCTK_ATTRIBUTE_UNUSED v(alloc);
    }
    {
        // AllocVec OCTK_ATTRIBUTE_UNUSED v(ia, ia + OCTK_ARRAY_SIZE(ia), alloc);
    }
    {
        AllocVec OCTK_ATTRIBUTE_UNUSED v({1, 2, 3}, alloc);
    }

    AllocVec v2;
    {
        AllocVec OCTK_ATTRIBUTE_UNUSED v(v2, alloc);
    }
    {
        AllocVec OCTK_ATTRIBUTE_UNUSED v(std::move(v2), alloc);
    }
}
...
TEST(AllocatorSupportTest, CountAllocations)
{
    ...
    MyAlloc alloc(&allocated);
    {
        AllocVec OCTK_ATTRIBUTE_UNUSED v(ia, ia + 4, alloc);
...
TEST(AllocatorSupportTest, SwapBothAllocated)
{
    ...
    int64_t allocated2 = 0;
    {
        ...
        AllocVec v1(ia1, ia1 + OCTK_ARRAY_SIZE(ia1), a1);
        AllocVec v2(ia2, ia2 + OCTK_ARRAY_SIZE(ia2), a2);
...
TEST(AllocatorSupportTest, ScopedAllocatorWorksInlined)
{
    ...
    using AllocVec = InlinedVector<StdVector, 1, ScopedAlloc>;
...
TEST(AllocatorSupportTest, SizeAllocConstructor)
{
    ...
    using AllocVec = InlinedVector<int, inlined_size, Alloc>;
...
struct MinimumAllocator
{
    ...
    struct rebind
    {
        using other = MinimumAllocator<U>;
    };
...
TEST(InlinedVectorTest, MinimumAllocatorCompilesUsingTraits)
{
    ...
    InlinedVector<T, 1, MinimumAllocator<T>> vec;
    vec.emplace_back();
    vec.resize(0);
}
