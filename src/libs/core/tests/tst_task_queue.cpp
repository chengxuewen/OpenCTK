/*
 *  Copyright 2016 The WebRTC Project Authors. All rights reserved.
 *
 *  Use of this source code is governed by a BSD-style license
 *  that can be found in the LICENSE file in the root of the source
 *  tree. An additional intellectual property rights grant can be found
 *  in the file PATENTS.  All contributing project authors may
 *  be found in the AUTHORS file in the root of the source tree.
 */

#include <octk_task_queue_for_test.hpp>
#include <octk_task_queue_factory.hpp>
#include <octk_function_view.hpp>
#include <octk_scope_guard.hpp>
#include <octk_task_event.hpp>
#include <octk_task_queue.hpp>
#include <octk_date_time.hpp>
#include <octk_memory.hpp>
#include <octk_sleep.hpp>

#include <gmock/gmock.h>
#include <gtest/gtest.h>

#if defined(OCTK_OS_WIN)
// clang-format off
#   include <windows.h>  // Must come first.
#   include <mmsystem.h>
// clang-format on
#endif

#include <memory>
#include <utility>
#include <vector>

using namespace octk;

namespace
{

// Noop on all platforms except Windows, where it turns on high precision
// multimedia timers which increases the precision of TimeMillis() while in
// scope.
class EnableHighResTimers
{
public:
#if !defined(OCTK_OS_WIN)
    EnableHighResTimers() {}
#else
    EnableHighResTimers() : mEnabled(timeBeginPeriod(1) == TIMERR_NOERROR) {}
    ~EnableHighResTimers()
    {
        if (mEnabled)
        {
            timeEndPeriod(1);
        }
    }

private:
    const bool mEnabled;
#endif
};
}  // namespace

// This task needs to be run manually due to the slowness of some of our bots.
// TODO(tommi): Can we run this on the perf bots?
TEST(TaskQueueTest, PostDelayedHighRes)
{
    EnableHighResTimers high_res_scope;

    static const char kQueueName[] = "PostDelayedHighRes";
    Event event;
    TaskQueueForTest queue(kQueueName, TaskQueueFactory::Priority::HIGH);

    uint32_t start = DateTime::TimeMillis();
    queue.PostDelayedTask(
        [&event, &queue] {
            EXPECT_TRUE(queue.IsCurrent());
            event.Set();
        },
        TimeDelta::Millis(3));
    EXPECT_TRUE(event.Wait(TimeDelta::Seconds(1)));
    uint32_t end = DateTime::TimeMillis();
// These tests are a little relaxed due to how "powerful" our test bots can
// be.  Most recently we've seen windows bots fire the callback after 94-99ms,
// which is why we have a little bit of leeway backwards as well.
    EXPECT_GE(end - start, 3u);
    EXPECT_NEAR(end - start, 3, 3u);
}

TEST(TaskQueueStdlib, AvoidsSpammingLogOnInactivity)
{
    std::string log_output;
    auto task_queue = utils::createDefaultTaskQueueFactory()->CreateTaskQueue(
        "test", TaskQueueFactory::Priority::NORMAL);
    auto wait_duration = Event::defaultWarnDuration() + TimeDelta::Seconds(1);
    utils::usleep(wait_duration.ms());
    EXPECT_EQ(log_output.length(), 0u);
    task_queue = nullptr;
}
